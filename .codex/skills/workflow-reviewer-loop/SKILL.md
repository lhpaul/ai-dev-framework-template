...
4. Run Step 7 (automated review), Step 7b (regression label for implementation PRs), and Step 8 (CI) using the scripts in `scripts/development-workflow/`. Run Step 7 to completion before Step 7b and Step 8. Run `pr-review-loop.sh` and `pr-ci-loop.sh` in the foreground of the same turn — never start either in the background and end your turn to wait for it. The completion notification goes to your dispatcher, not to you, so doing so parks you permanently, indistinguishable from a dead runner. If you must background one, poll it yourself in-turn until it returns before continuing.
