...
- **Tracker status update silently skipped for project items with null `content`** (`workflow-lib.sh`, #401): Both `update_tracker_status_best_effort` and `get_tracker_status_for_issue` used `item.get('content', {})` in their Python3 item-matching blocks, which returns `None` (not `{}`) when `content` is explicitly `null` in JSON — issues added mid-session via the GitHub Projects API can produce this shape. Calling `.get('number')` on `None` raised `AttributeError`, causing Python3 to exit non-zero; the `|| true` guard silenced it and `item_json` was empty, triggering the "not found" warning or a silent empty-string return. Fixed by replacing `item.get('content', {})` with `(item.get('content') or {})` in both functions so `None` maps to an empty dict. Added defensive skip for items where `content.number` is `0`, `null`, or missing. Added a stderr debug log per skipped item (in `update_tracker_status_best_effort`) and a one-retry fallback that re-fetches the project item list when the first pass finds no match, covering cases where an item was added to the project board after the initial fetch.
