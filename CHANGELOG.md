...
- **`claude-code-action-reviewer.sh`: remove `inputs.pr_number` verification that always failed; add run-name PR scoping for concurrent dispatch** (#806, #808) — the Phase 2 poll loop verified each candidate Actions run by fetching `.inputs.pr_number` from the individual run endpoint, but the GitHub Actions Runs API always returns `inputs: null` regardless of what was passed at dispatch time (confirmed empirically). The check always evaluated to "skip this cycle", causing the loop to spin until `MAX_WAIT` (default 600 s) before falling through to Phase 3. The net result was correct (Phase 3's unresolved-thread query is already scoped to the correct PR number), but every PR using `claude-code-action` incurred a ~10-minute wall-clock penalty per reviewer-loop invocation. Fix: removed the `inputs.pr_number` lookup and the `EXCLUDED_RUN_IDS_JSON` exclusion list entirely. To prevent incorrect run selection under concurrent/parallel dispatch (where two PRs trigger the review workflow within the same poll window), `claude-code-review.yml` now sets `run-name: "Claude Code Review — PR #${{ inputs.pr_number }}"`, and the Phase 2 jq filter additionally matches on the run's `.name` field containing `PR #<pr_number>` (word-boundary guarded) alongside the timestamp and workflow-file filters. Unlike `.inputs` (always null in the Runs API), `.name` is populated from `run-name` and reliably identifies which PR triggered the run. Updated unit tests in `scripts/development-workflow/tests/test-claude-code-action-reviewer.sh` adding Area 5 concurrent-dispatch behavioral tests: given runs for two different PRs in the same poll window, the filter selects this PR's run (not the most-recent); only this PR's run present → selected; only the other PR's run present → null (no match); word-boundary guard prevents PR #808 from matching PR #8080.
