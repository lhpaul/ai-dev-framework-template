...
- **`pr-review-loop.sh`: detect CodeRabbit rate-limit and pause banners and escalate instead of returning false-clean** (#688): when CodeRabbit posts a rate-limit comment or a "Reviews paused" banner and cannot complete its review before the poll window ends, the script now emits `RESULT=escalate/REASON=rate_limit_max_retries` (exit 2) instead of the previous `RESULT=skipped/REASON=no_review` (exit 0, treated as clean). This prevents the reviewer loop from marking a PR ready when CodeRabbit was still rate-limited or paused. The rate-limit and pause retrigger actions are corrected from `@coderabbitai review` to `@coderabbitai resume`, which is the proper command to un-pause CodeRabbit after a rate-limit or auto-pause event. The new `rate_limit_max_retries` escalation reason is mapped to `timed out` in `normalize_platform_verdict` for consistent summary reporting. A new test case covers the new verdict mapping.
- **`pr-review-loop.sh`: preserve `phase_after_clean_platforms` in `--pre-after-clean-only` mode** (#693): `filter_phase_after_clean_platforms` was called unconditionally after `filter_pre_after_clean_platforms`, which had already removed phase platforms from `platforms`. The second filter found no matching entries and cleared `phase_after_clean_platforms`, causing `PHASE_AFTER_CLEAN_PLATFORM_LIST` telemetry to be empty and the post-review summary to omit the configured phase list. Fixed by skipping `filter_phase_after_clean_platforms` when `--pre-after-clean-only` is active.
