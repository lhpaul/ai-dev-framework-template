...
- **`haystack-reviewer.sh`: recover from non-zero haystack CLI exit when stdout contains valid JSON** (#800) — some haystack CLI versions return a non-zero exit code even when `haystack triage --json --no-wait` produces a valid completed-analysis payload (observed in Batch 70 and Batch 71: analysis visible on haystackeditor.com, but wrapper reported `UNAVAILABLE`). Previously, any non-zero exit from the haystack CLI caused the script to immediately emit `RESULT=skipped REASON=unavailable` without inspecting stdout, silently dropping completed findings. The script now inspects stdout before giving up: if stdout is valid JSON with `status=pending`, the poll-retry loop continues (same path as exit-0 + pending); if stdout is valid JSON with a completed result, findings parsing proceeds; only if stdout is empty or non-JSON AND exit is non-zero does the script report UNAVAILABLE. This eliminates the silent-swallow pattern and ensures completed analyses are always surfaced regardless of CLI exit code. Added 7 new unit tests in Area 8 of `test-haystack-reviewer.sh` covering each sub-case: completed+no-findings, completed+blocking-finding, pending+retry, status=none, empty stdout, invalid JSON stdout, and zero-exit baseline.
