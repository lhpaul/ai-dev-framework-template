...
- **Tracker status update silently skipped for project items with null `content`** (`workflow-lib.sh`, #401): `update_tracker_status_best_effort`'s Python3 item-matching block used `item.get('content', {})`, which returns `None` (not `{}`) when `content` is explicitly `null` in JSON — issues added mid-session via the GitHub Projects API can produce this shape. Calling `.get('number')` on `None` raised `AttributeError`, causing Python3 to exit non-zero; the `|| true` guard silenced it and `item_json` was empty, triggering the "not found" warning. Fixed by replacing `item.get('content', {})` with `(item.get('content') or {})` so `None` maps to an empty dict. Added defensive skip with a stderr debug log for items where `content.number` is `0`, `null`, or missing. Added a one-retry fallback that re-fetches the project item list when the first pass finds no match, covering cases where an item was added to the project board after the initial fetch.
