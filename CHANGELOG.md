...
- **Workflow shell guard lint**: add a diff-based lint check for new `scripts/development-workflow/**/*.sh` lines that suppress `gh`, `git`, `curl`, or `haystack` failures with `|| true` without an explicit `workflow-shell-guard` rationale. The ShellCheck workflow now runs the guard and its harness for workflow-script changes.
