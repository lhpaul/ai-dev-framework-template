...
- **Step 8a blocking gate: verify all review threads resolved via GraphQL before ready-for-human-review** (`91-orchestrate-work-protocol.md`, #425): Agents were applying `ready-for-human-review` without executing the GraphQL `reviewThreads` query, asserting reviews were "clean" based on self-tracked thread state rather than querying the API. Added a mandatory GraphQL verification gate immediately before Check 4 that runs the `reviewThreads` query inline, counts unresolved bot-authored threads, and exits with code 4 if any are found. The gate emits a `⛔ STOP` message and requires the agent to address findings and re-enter Step 8a from the beginning — `ready-for-human-review` cannot be applied until the GraphQL query confirms all threads are resolved. Maintains the distinct-exit-code contract (exit 4 = unresolved review threads).
