...
- **Protocol 90: detect stale local branches in Step 3.3 pre-batch environment check** (#653): Step 3.3 gains a new Check 3 that scans for two categories of stale local branches — workflow-prefix branches (`feature/`, `fix/`, `refactor/`, `hotfix/`, `spec/`, `implementation-plan/`) whose upstream PR has been merged, and `worktree-agent-*` branches that have no remote counterpart and no associated open or merged PR. Found branches are listed with suggested `git branch -D` cleanup commands and surfaced to the human before dispatch. The check is non-blocking (operational noise only; does not hold batch dispatch) and runs in parallel with Check 1 (stale worktrees) and Check 2 (integration branch sync). Addresses recurring stale-branch accumulation observed in Batches 17, 21, 34, 48, and 53.

- **Support long-running multi-item developments via a dedicated integration branch** (#628): adds the `develop-<slug>` integration-branch workflow, epic/label creation in the add-backlog-item protocol, orchestrator-level base-branch override in protocols 90 and 91, developer base-branch note in protocol 03, and the new `05b-graduate-development-protocol.md` graduation command.
- **Protocol 03: verify-before-add guidance for tickets claiming a missing feature** (#608): Fast Track Step 1 gains a mandatory "Verify-Before-Add" block that requires developers to independently reproduce a reported gap before writing any code when the ticket claims a feature is "missing" or "unavailable" without a stack trace or failing test. If the gap cannot be reproduced, the ticket is cancelled and documented. Includes a concrete example scenario (field projection misread as absent).
- **Script quality gates for `pr-review-loop.sh`** (#585): add a shell test harness at `scripts/development-workflow/tests/test-pr-review-loop.sh` covering `normalize_platform_verdict`, `check_unreplied_rest_comments`, and compare-mode analytics (`append_compare_metrics_row`); add a path-triggered CI workflow (`.github/workflows/test-pr-review-loop.yml`) that runs the harness on PRs touching `pr-review-loop.sh` or `workflow-lib.sh`; add script-coverage and downstream bug-review checklist items to the prepare-release protocol (Step 7.3); add a downstream script-bug tracking prompt to the retrospective protocol (Step 3a).
- **Prettier for markdown formatting** (#584): `package.json` gains a `prettier` dev dependency (v3.8.3) and a `"format": "prettier --write \"**/*.md\""` script; `.prettierrc.json` sets `proseWrap: "preserve"` to prevent prose paragraph reflowing. All `.md` files in the repository have been formatted with Prettier so downstream projects that run Prettier see no spurious diffs on `/sync-template` runs.
...
- **Enforce ready-for-regression label and reviewer-loop handoff via CI** (#613): add two GitHub Actions workflows — `apply-regression-label.yml` that auto-applies the `ready-for-regression` label to implementation PRs based on branch prefix (`feature/`, `fix/`, `refactor/`, `hotfix/`), and `reviewer-loop-guard.yml` that asserts the automated reviewer-loop summary comment is present before a PR can be treated as merge-eligible. A new integration doc (`docs/workflow/development-workflow/integrations/ci-enforcement.md`) covers default prefix configuration, downstream overrides, and branch-protection wiring.
- **Auto-remove `ready-for-regression` label on push** (#612): a new `.github/workflows/remove-regression-label-on-push.yml` workflow triggers on `pull_request: synchronize` events and removes the `ready-for-regression` label from the PR when new commits are pushed, preventing stale regression-readiness signals from persisting on code that has not yet been re-reviewed.
...
- **Ensure spec/plan/developer agents add issues to project board** (#656): adds `ensure_on_project_board` to `workflow-lib.sh` and integrates it across agent checklists (`.claude/agents`, `.cursor/agents`), skill workflows (`.codex/skills`), and development protocols (`01-generate-spec-protocol.md`, `02-generate-implementation-plan-protocol.md`, `03-implement-development-protocol.md`, `90-batch-orchestrate-work-protocol.md` Step 2.5) so issues are guaranteed to appear on the GitHub Projects board before their tracker status is updated.

### Fixed

- **Protocol 90/integrations: filter project board queries to open issues only to prevent GraphQL rate-limit exhaustion** (#655): Protocol 90 Step 1a gains a new "Rate-limit awareness for GitHub Projects pagination" subsection documenting why `gh project item-list` on large boards exhausts the 5 000-point GraphQL rate limit and providing two concrete alternatives — the recommended open-issue-first approach (`gh issue list --state open` to build a candidate set, then a single `gh project item-list` call with client-side cross-reference to open issues) and a simpler client-side terminal-status post-filter. Both Protocol 90 and `docs/workflow/development-workflow/integrations/github-projects.md` gain a rate-limit check pattern (`gh api rate_limit`) with warn (< 1 000 remaining) and pause (< 200 remaining) thresholds. The `github-projects.md` "Reading Project Items" section is updated with the same open-issue-first pattern and performance note.
- **Protocol 91/93: convert draft PR to non-draft before internal review gate when CodeRabbit is a listed reviewer** (#657): adds a "Draft-state pre-check" at the top of Protocol 91 Step 7a that detects when the PR is in draft state and any listed internal reviewer (e.g., CodeRabbit) skips draft PRs, then runs `gh pr ready` automatically before dispatching reviewers. This prevents silent reviewer-coverage gaps caused by `auto_review.drafts: false` in `.coderabbit.yaml` — CodeRabbit produces no skip notice when it bypasses a draft PR, making the omission invisible. The CodeRabbit reachability check is updated to clarify that the `drafts: false` restriction is resolved by the pre-check, not treated as an unreachability condition. Protocol 93 gains a matching "draft-state check" in its pre-flight section for standalone reviewer-loop invocations.

- **`sync-template`: auto-create PR and start reviewer loop after applying changes** (#630): Step 5 of the sync-template protocol (`.claude/commands/sync-template.md`, `.cursor/commands/sync-template.md`) is renamed from "Generate git instructions" to "Commit, push, and open PR" and now executes all git/PR steps automatically instead of printing them for the user to run manually. A new Step 6 ("Start the reviewer loop") runs the internal review gate (Step 7a), converts the PR to non-draft, runs `pr-review-loop.sh`, and applies `ready-for-regression` and `ready-for-human-review` labels before returning — completing the sync-template workflow end-to-end in a single invocation. The Codex `workflow-sync-template` skill is updated to reference Steps 5 and 6.
- **`sync-template`: "apply all" now walks through manual-review and optional-additive items inline** (#629): previously, "apply all" silently excluded items in the manual-review section and the optional-additive section, leaving users who chose "apply all" with an incomplete sync. The Step 3 confirmation prompt now offers two named options — "apply all" (walks through every section, presenting each manual-review or optional-additive item inline for confirm/skip) and "apply always-sync only" (skips non-always-sync items entirely). Step 4 is restructured with explicit sub-sections for always-sync, manual-review/optional-additive ("apply all" path), always-sync-only path, special-handling paths, and general rules. Special-handling paths (`.github/workflows/deploy.yml`, `e2e-regression.yml`, `e2e/`, `.claude/settings.json`, etc.) remain outside "apply all" and continue to require explicit per-path approval. The Codex `workflow-sync-template` skill is updated to reference the new Step 3/4 behavior.

- **`pr-review-loop.sh`: auto-reply to resolved CodeRabbit REST comments** (#616): `coderabbit_thread_gate_clean` now auto-replies to remaining unreplied outside-diff REST comments after all GraphQL review threads pass as resolved, instead of returning `RESULT=needs_fixes` and blocking the loop. A new `auto_reply_unreplied_rest_comments` helper fetches the IDs of unreplied root CodeRabbit REST comments (applying the same resolved-IDs exclusion as `check_unreplied_rest_comments`) and posts a brief acknowledgement reply (`Resolved — addressed in this PR.`) to each one via the GitHub `pulls/{pr}/comments/{id}/replies` endpoint. If any reply post fails the function falls back to `RESULT=needs_fixes` so the agent can address remaining comments manually. The test harness (`test-pr-review-loop.sh`) gains a new Area 5 covering the reply count, POST endpoint targeting, resolved-IDs exclusion, already-replied exclusion, and partial-failure exit code.

- **Protocol 93: document CodeRabbit summary comment update-in-place pattern** (#644): a new "CodeRabbit summary comment update-in-place pattern" section in `93-automated-reviewer-loop-protocol.md` explains that CodeRabbit edits its single summary/walkthrough comment in-place after each review cycle rather than posting new comments, making the comment timestamp an unreliable signal for review completion. Agents must not infer from comment age whether CodeRabbit has reviewed the current HEAD; the authoritative signals are the GraphQL thread audit (`isResolved` state) and the CodeRabbit SUCCESS commit status checked by `pr-review-loop.sh`.
- **`pr-review-loop.sh`: detect and auto-resume CodeRabbit auto-pause at start of review loop** (#651): adds a pre-poll pause-detection step in `run_coderabbit_review` that inspects the most recent CodeRabbit issue comment on the PR (without the `since_iso` timestamp filter used by the Phase 2 in-loop check) before the poll loop starts. If that comment matches the "Reviews paused" pattern, `@coderabbitai resume` is posted immediately and `since_iso` is reset to the pre-post timestamp so the resulting review is captured. The `coderabbit_retrigger_attempted` flag in Phase 2 is initialized from the pre-poll result to prevent a double resume. This closes the gap observed in Batch 52 (PR #650) where a pre-HEAD pause banner was invisible to the Phase 2 `since_iso`-filtered check, causing the loop to exit `skipped (no_review)` — treated as clean — without CodeRabbit having reviewed the latest commit.
- **Protocol 93: document CodeRabbit "Reviews paused" and silent non-trigger silence patterns** (#643): a new "CodeRabbit silence patterns" section in `93-automated-reviewer-loop-protocol.md` explains the two ways CodeRabbit can go silent after a push — auto-paused reviews ("Reviews paused" issue comment) and silent non-trigger (no activity at all) — and documents how `pr-review-loop.sh` handles each automatically via `@coderabbitai review` retriggers. Includes diagnostic steps for agents polling manually and criteria for when to escalate vs. wait. Protocol 91 Step 7 gains a brief cross-reference callout pointing to the new section.
- **Protocol 91: strengthen GraphQL reviewThreads gate to prevent agents bypassing it after `pr-review-loop.sh clean`** (#634): Step 7 result table `clean` action is updated to explicitly state that `clean` does NOT authorize applying `ready-for-human-review` directly and that the GraphQL pre-Check-4 gate in Step 8a is the authoritative thread-resolution check. The "Re-query reviewThreads after each push" section is updated with a callout warning that `clean` from the script does not mean all threads are resolved. Step 8a gains a Warning block at the top explaining that `pr-review-loop.sh clean` is not a sufficient readiness signal and that the full checklist — including the GraphQL pre-Check-4 gate — must be run every time.
- **Release post-merge cleanup: add Linear tracker support for `Merged` → `Released` transitions** (#627): `update_tracker_status_best_effort` and `get_tracker_status_for_issue` in `workflow-lib.sh` now detect `provider: linear` and emit a clear, actionable warning with per-issue guidance instead of the misleading "GITHUB_PROJECT_NUMBER not set" skip message. `prepare-release-post-merge-cleanup.sh` gains a Linear-provider fast path that prints per-issue manual action instructions (Linear MCP/API) and exits cleanly, preventing the spurious `UPDATED=0` hard failure that occurred on every Linear-tracker run. `post-merge-cleanup.sh` is covered automatically by the `workflow-lib.sh` change.
- **Reviewer loop: pre-post verification guard before `gh pr comment` / `gh pr review` calls** (#603): Protocol 93 gains a mandatory "Pre-post verification guard" step that must be completed before any agent-composed `gh pr comment` or `gh pr review` call that characterises a platform result. The guard requires (1) re-fetching the raw platform transcript immediately before composing the comment, (2) cross-checking every approval or pass claim against a specific transcript excerpt, and (3) falling back to a conservative "Review completed — see individual platform comments for details." message when the transcript is absent or ambiguous. The guard applies to both inline fix-pass summaries and the final reviewer-loop summary comment. A reference to the guard is added in the "PR feedback tracking and comments" bullet for fix commit comments.
- **Protocol 90: orchestrator done-report must query artifact state via `gh` CLI, not trust agent self-reports** (#604): Step 5.1 preamble is rewritten to make explicit that every field in the post-dispatch verification — labels, CHANGELOG presence, reviewer loop completion, CI status — must be sourced from independently queried `gh` CLI output, never from Work Item Runner self-reports. A new "CHANGELOG presence" row is added to the verification table: for `feature/*`, `fix/*`, `refactor/*`, and `hotfix/*` PRs the orchestrator must confirm `CHANGELOG.md` appears in the `files` array returned by `gh pr view --json files` before accepting the "ready" claim; missing CHANGELOG triggers agent redispatch. The `gh pr view` command in the step is updated to include `files` in its `--json` fields. Step 6 gains a "done-report source rule" callout requiring that every summary assertion be sourced from Step 5.1 artifact queries, not from agent claims.
- **Protocol 90/91: explicit item list scope guard — hard-refuse rule for out-of-scope items** (#605): Protocol 90 gains a new "Explicit Item List Scope Guard" section (before Step 1) that hard-refuses all artifact mutations (branch creation, PR labeling, tracker updates, subagent dispatch, CHANGELOG edits) on items outside an explicit list provided at dispatch time; out-of-scope items trigger a WARNING log entry and are reported in the Step 6 summary. Protocol 91 gains an equivalent "Explicit Item List Scope Guard" subsection in Step 1 with the same hard-refuse rule, WARNING log requirement, and Step 6 reporting obligation. Both protocols include a human-override clause that requires an explicit in-session instruction to expand scope.
- **`pr-review-loop.sh`: REST comment check skips comments whose GraphQL thread is already resolved** (#586): `check_unreplied_rest_comments` now receives the database IDs of root comments from resolved GraphQL threads (`isResolved=true`) and excludes them from the unreplied count. Previously, resolving a thread via the GitHub UI left the REST comment present without a non-bot reply, causing `coderabbit_thread_gate_clean` to incorrectly block the review loop. A new `get_resolved_thread_comment_ids` helper fetches the resolved thread IDs and the resolved-IDs list is passed to `check_unreplied_rest_comments` as an optional fourth argument.
- **`pr-review-loop.sh`: auto-trigger `@coderabbitai review` on silent non-trigger after push** (#587): CodeRabbit sometimes does not auto-trigger after a push commit and the polling loop would stall until `max_wait` expired. A new silent-non-trigger detection block posts `@coderabbitai review` once no activity is seen within `CODERABBIT_NO_TRIGGER_TIMEOUT` seconds (default: 600 s) and no "Reviews paused" or "rate limit" comment is present. Respects `CODERABBIT_RATE_LIMIT_MAX_RETRIES` as the combined retrigger cap. The elapsed timer resets after each trigger so the review has a full polling window to complete.
- **Protocol 90/91: fixer redispatches in parallel batches must use worktree isolation** (#589): Step 5 and Step 5.1 of Protocol 90 now explicitly require that any Work Item Runner redispatch — including fixer-agent passes triggered by review findings — carries `BATCH_CONTEXT=true` and the resolved `<worktree-path>` when the original batch used parallel dispatch. Protocol 91 Step 7 adds an equivalent fixer agent worktree isolation rule stating that fixer agents dispatched from a parallel-batch Work Item Runner must receive the same `BATCH_CONTEXT=true` and `WORKTREE_PATH` values. Without these values, fixer agents use main-repo file paths in `Read`/`Edit`/`Write` calls while committing via the worktree git context, leaving uncommitted files in the main working tree.
- **Protocol 90/91: `ready-for-regression` enforcement explicitly covers `refactor/*` PRs** (#590): Protocol 91 Step 7b adds a "`refactor/*` is not exempt" callout and names all implementation branch types (`feature/*`, `fix/*`, `refactor/*`, `hotfix/*`) in the `BATCH_CONTEXT=true` enforcement note. Protocol 90 Step 5.1 direct-apply rule prose now enumerates all four branch types and adds a matching "`refactor/*` is not exempt" guardrail note. Closes a gap where agents could infer `refactor/*` was exempt from the label based on context implying only `fix/*` Fast Track branches were covered.
- **Plan-writer cross-section consistency check extended to file paths and routes** (#591): the mandatory cross-section self-check in Protocol 02 Step 5.5 now covers file paths, directory names, and route/URL structures in addition to function names, constants, and decision index labels. The `tech-lead` agent files for Claude Code and Cursor are updated with an explicit reminder to apply this check before committing.
...
- **ShellCheck SC1007: replace `CDPATH= cd` with `CDPATH='' cd` across all workflow scripts**: the inline env-var form `CDPATH= cd` triggers ShellCheck SC1007; all 16 occurrences across 11 scripts are updated to the explicit empty-string form `CDPATH='' cd`.
- **`pr-review-loop.sh`: `normalize_platform_verdict` maps `skipped` to `unavailable` in compare mode**: previously `skipped` was mapped to `clean`, hiding missing reviews in per-platform metrics and summary output. `skipped` now maps to `unavailable` to distinguish absent reviews from confirmed-clean ones.
- **`pr-review-loop.sh`: bot accounts excluded from human-reply detection in REST thread check**: the `check_unreplied_rest_comments` filter `select(.user.login != $bot)` only excluded the primary review-bot login; other GitHub bot accounts (logins ending in `[bot]`) were counted as human replies. An additional `test("\\[bot\\]$")` predicate now excludes all bot accounts.
- **`pr-review-loop.sh`: "Metrics row appended" summary line is now conditional on success**: the compare-mode PR summary always claimed "_Metrics row appended to `retro-metrics-platforms.md`_" even when `append_compare_metrics_row` failed. The `_compare_metrics_appended` flag now tracks actual success, and the line is omitted on failure.
- **`pr-review-loop.sh`: compare-mode platform change detection uses names, not just count**: the previous check only compared the number of platform columns, so swapping one reviewer for another (same count) went undetected. The header row above the last separator row is now parsed to extract ordered platform names, which are compared against the current configuration.
- **`check-tracker-merge-mapping.sh`: `get_target_status` uses `awk` block extraction instead of `grep -A5`**: the previous `grep -A5` silently returned empty when `TARGET_STATUS` was more than 5 lines below the branch marker. The function now uses `awk` to extract the full clause block between consecutive branch markers before searching for the assignment.
- **Retrospective protocol: Step 3b subagent detection and fallback** (#593): Step 3b now includes an explicit subagent context note clarifying that the template cross-reference uses `gh` CLI exclusively (no MCP tools required); a filesystem-based YAML read command is provided so subagent runners can obtain `template.repository` without relying on memory or MCP tool availability. The 3b Gate is updated with explicit subagent guidance: skipping Step 3b because the agent assumed an empty `template.repository` without reading the YAML does not satisfy the gate — the agent must read the file and, if unreadable, mark all findings `check-unavailable` to proceed.
- **Item-orchestrator: cross-layer scope check before fast-track dispatch** (#601): Protocol 91 Step 2 gains a mandatory "Cross-layer scope check" sub-section that must run before any backlog item is classified as Fast Track. If the issue title, body, or any linked spec/plan contains concrete signals that the change spans more than one architectural layer (e.g., database schema + API endpoint + UI component, or data pipeline + storage + mapper + presentation), the item must be routed to the Full Pipeline instead of fast-tracked. Protocol 03 Path 3 criteria list gains a "No multi-layer scope signals" entry and a "Cross-layer scope check" explanation block. The Fast Track section of `docs/workflow/development-workflow/README.md` is updated with the new criterion and a pointer to the decision rule in Protocol 91.
- **Reviewer loop: mandatory post-push SHA verification before fix-pass completion** (#602): Protocol 93 adds a mandatory post-push SHA verification step in both the inline fix rule and the fixer agent batching rule. Immediately after every `git push`, the runner must compare `git rev-parse HEAD` against `gh pr view <pr_number> --json headRefOid` to confirm the push landed on the remote. If the SHAs do not match, the push is retried once; if the retry also fails, the runner reports BLOCKED and must not resolve review threads or apply readiness labels.
- **Protocol 03: mandatory test harness coverage checklist at self-review step** (#614): a new top-level `## Test Harness Coverage Checklist` section is added to `03-implement-development-protocol.md` requiring agents to verify edge-case coverage (empty input, whitespace-only input, boundary values, missing environment variables, concurrent execution, negative assertions) before self-approving any implementation that ships or modifies a test script or validation harness. Each path's verify/pre-commit step now references this checklist. Addresses the pattern from Batches 38 and 46 where CodeRabbit required multiple fix rounds to catch edge-case gaps that agents missed during self-review.
- **Shell script quality checklist extended to embedded protocol markdown snippets** (#635): the Shell Script Quality Checklist in `03-implement-development-protocol.md` gains a new item 8 that applies the same error-handling and wrong-branch-guard standards to shell code blocks (` ```bash ` / ` ```sh ` fenced blocks) embedded in protocol and documentation `.md` files. The checklist trigger sentence is broadened from `.sh`-only to include protocol `.md` changes with embedded shell blocks. The `developer` agent instruction files (`.claude/agents/developer.md`, `.cursor/agents/developer.md`) and the shell-scripting section of `docs/best-practices/1-general.md` are updated to reflect the extended scope. `REVIEW.md` gains a matching "Shell snippet safety in protocol files" bullet under the documentation PR checks.
- **`pr-review-loop.sh`: add SIGTERM/SIGINT traps to clean up lock directory on signal delivery** (#615): the single-instance lock guard previously registered only an `EXIT` trap; receiving `SIGTERM` (e.g., CI runner timeout, orchestrator kill) while the process was blocked inside a `sleep` or `gh api` call could leave `/tmp/pr-review-loop-<PR>.lockdir` on disk, preventing script restarts. Two additional traps are added — `TERM` and `INT` — that remove the lockdir and re-raise the original signal (re-raise pattern: `trap - SIG; kill -SIG $$`) so the parent process receives the correct death-by-signal exit status. The test harness gains a subprocess-based Area 4 that verifies SIGTERM cleanup, plus a code-inspection assertion that the INT trap is registered.
