...
- **Protocol 90: Step 5.1 verification table gains a CHANGELOG presence check** (#607): `gh pr view` command in Step 5.1 is updated to include `files` in its `--json` fields; a new "CHANGELOG presence" row is added to the verification table requiring that `CHANGELOG.md` appears in the PR's file set for `feature/*`, `fix/*`, `refactor/*`, and `hotfix/*` PRs before the orchestrator accepts a "ready" claim; `spec/*` and `implementation-plan/*` PRs are exempt per existing policy. Missing CHANGELOG triggers agent redispatch rather than silent acceptance.
