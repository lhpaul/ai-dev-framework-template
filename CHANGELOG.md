...
- **Worktree git switch guardrail** (issue #192): added an explicit "Critical: Worktree Git Discipline" block to Protocol 91 Step 3 enumerating prohibited commands (`switch`, `checkout`, `checkout -b`, `reset`, `restore`) against the main repo root when `BATCH_CONTEXT=true`, with required alternatives (`git -C <worktree-path>` or `cd <worktree-path> &&`). Added an optional pre-tool-use hook guidance note (marked "Optional — platform-specific") immediately after the Critical block. Upgraded Protocol 90 Step 5.2 to handle all four postcondition states: wrong-branch+clean (auto-correct and log as guardrail violation), wrong-branch+dirty (halt and escalate), correct-branch+clean (proceed normally), correct-branch+dirty (halt and escalate). Added a matching "Worktree git discipline" reminder to `.cursor/agents/item-orchestrator.md` for dual-agent consistency.
...
- **Codex reviewer runtime fallback in Step 7a** (issue #185): Protocol 91 Step 7a now includes a runtime-availability check sub-section before the reviewer dispatch map. Before dispatching any reviewer, the runner classifies each listed reviewer as `reachable` or `unreachable` based on runner-context identity (e.g., `codex` is unreachable from Claude Code subagent context). Under the default `warn` policy, unreachable reviewers are skipped with a PR warning comment naming the reviewer and runner context, and the gate proceeds with the reachable subset. If zero reviewers are reachable, the gate hard-fails regardless of policy and the PR stays in draft. A `fail-if-any-unavailable` strict policy is also documented. A Step 7a summary comment is now mandatory whenever the gate exits, listing the effective reviewer set, skipped reviewers (with reason), and the final verdict. Step 8c gains a clarifying parenthetical to distinguish the new Step 7a summary comment from the Step 7 "Automated Reviewer Loop Summary" check. `.ai-dev-workflow.yaml` gains a commented-out `internal_reviewers_unavailable_policy` annotation documenting the allowed values and default.
...
- **Worktree Write/Edit path discipline for item-orchestrator** (issue #193): item-orchestrator agents running in isolated git worktrees now receive an explicit reminder that all Write/Edit tool calls must target paths under the resolved worktree path, not the main repo root; same guardrail added to both Claude Code and Cursor agent prompts.
...
- **Contradictory log output and missing tracker status update in `post-merge-cleanup.sh`** (issue #184): unified the two separate issue-number extraction code paths (`fix|feature|hotfix|refactor` and `spec|implementation-plan`) into a single block that sets both `ISSUE_NUMBER` and `BRANCH_TYPE`, eliminating the spurious "No issue number detected" log line that appeared for `spec/*` and `implementation-plan/*` branches. Added a new `update_tracker_status` helper function (best-effort, non-fatal) that resolves the GitHub Projects project ID, Status field ID, option ID, and project item ID via `gh project` commands and GraphQL, then calls `updateProjectV2ItemFieldValue` to set the correct status: `Spec Ready` for `spec/*` merges, `Plan Ready` for `implementation-plan/*` merges, and `Merged` for `feature/*`/`fix/*`/`refactor/*`/`hotfix/*` merges. The helper also applies rollback prevention per Protocol 91 Step 10: if the project item's current status is already further along in the 11-stage workflow ordering, the update is skipped with a "skipping rollback" log rather than overwriting a more-advanced status. Log messages for spec and plan branches now emit distinct text ("Spec branch..." vs "Implementation plan branch...") for clarity. Updated `docs/ai/development-workflow/integrations/github-projects.md` to document all three status transitions in the Post-Merge Cleanup section.
