...
- **`reviewer-loop-guard.yml`: add grace-period polling before failing when no loop summary is found** (#733) — the guard previously failed immediately if the automated reviewer-loop summary comment was not yet present, requiring a manual re-run after `pr-review-loop.sh` finished. The guard now polls up to `GUARD_MAX_POLLS` times (default 3) with `GUARD_POLL_INTERVAL` seconds between each attempt (default 30) before posting a failure status, giving `pr-review-loop.sh` up to 90 seconds to post its summary without a false-failure race.
