...
- Template deployment workflow scaffold: `.github/workflows/deploy-template.yml` triggers on `develop` and `main`, maps to `develop`/`production` environments, and keeps deploy steps as explicit no-op placeholders for downstream repository customization.
