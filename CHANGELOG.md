...
- **Enforce capable model for item-orchestrator review fix loops (issue #109)**: enforced `sonnet` (balanced) tier as the minimum model for the `item-orchestrator` agent in Claude Code and Cursor agent definitions to prevent economy-tier models from silently failing review-fix reasoning cycles.
- **Retrospective protocol checks backlog before presenting suggestions (issue #130)**: `06-retrospective-protocol.md` now requires querying the issue tracker for related items (filtered by the `workflow` label) in Step 3 (analysis) — before presenting suggestions in Step 5 — to avoid duplicating existing backlog items or suggesting work that is already in flight.
- **Pre-implementation scope checklist added to developer agent (issue #110)**: protocol `03-implement-development-protocol.md` now includes an explicit pre-implementation scope checklist requiring agents to verify the target branch, confirm file scope matches the assigned issue, and flag any ambiguous cross-cutting changes before writing a single line of code. Prevents agents from accidentally touching files outside their assigned scope in parallel batch runs.
- **Label checklist gate before declaring ready-for-human-review (issue #107)**: protocol `91-orchestrate-work-protocol.md` Step 8a now enforces a mandatory hard-gate checklist (non-draft state, `ready-for-regression` on implementation PRs, absence of `needs-fixes`) that must pass before `ready-for-human-review` is applied. Prevents PRs from being declared ready while holding contradictory labels or missing regression coverage.
- **Post-dispatch PR verification step for orchestrator (issue #123)**: protocol `90-batch-orchestrate-work-protocol.md` Step 5.1 and protocol `91-orchestrate-work-protocol.md` Step 8c now require the orchestrator to independently verify actual PR state via `gh pr view` before reporting any PR as ready for human review. Verification covers base branch correctness, required labels, presence of the automated reviewer loop summary comment, and green CI checks. Trusting Work Item Runner self-reports without verification caused 50% of batch 1 PRs to have at least one issue requiring human correction.
