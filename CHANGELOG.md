...
- **Haystack local hooks: validate Conventional Commits and document the operating model** — `hooks/commit-msg` now enforces the repository Conventional Commits subject format and 72-character first-line limit while preserving standard merge, revert, fixup, squash, and amend exceptions. The hook supports hierarchical scopes such as `ui/button`, and the workflow test harness covers the hook's acceptance, rejection, prefix bypass, and length-boundary behavior. The Haystack integration guide now distinguishes local deterministic hooks, `.haystack` PR-review rules, and the `haystack triage` reviewer-loop integration, with a small hardening roadmap for future checks.
