...
- **Retrospective suggestion decoupled from terminal condition (issue #131)**: protocols `91-orchestrate-work-protocol.md` (Step 6) and `90-batch-orchestrate-work-protocol.md` (Step 6) no longer suggest a retrospective when a PR reaches `ready-for-human-review`. The retrospective is now suggested only after the human confirms the PR(s) have been merged (e.g., via `/post-merge-cleanup`, `/batch-merge`, or an explicit merge signal). This prevents premature retrospective prompts while work is still awaiting human review.
