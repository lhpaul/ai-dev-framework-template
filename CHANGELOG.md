...
- **`pr-review-loop.sh`: detect CodeRabbit rate-limit and escalate instead of returning false-clean** (#688): when CodeRabbit posts a rate-limit comment and cannot complete its review before the poll window ends, the script now emits `RESULT=escalate/REASON=rate_limit_max_retries` (exit 2) instead of the previous `RESULT=skipped/REASON=no_review` (exit 0, treated as clean). This prevents the reviewer loop from marking a PR ready when CodeRabbit was still rate-limited. Additionally, the rate-limit retry action was corrected from `@coderabbitai review` to `@coderabbitai resume`, which is the proper command to un-pause CodeRabbit after a rate-limit window. The new `rate_limit_max_retries` escalation reason is mapped to `timed out` in `normalize_platform_verdict` for consistent summary reporting. A new test case covers the new verdict mapping.
