...
- **`claude-code-review.yml`: fix missing `id-token: write` permission and deprecated `pr_number`/`model` inputs** (#766) — adds `id-token: write` to the `permissions` block (required for OIDC token auth in `claude-code-action` v1.0.133+); removes deprecated `pr_number` and `model` inputs from the `with:` block; moves `anthropic_api_key` from `env:` to `with:` as now supported by the action.
