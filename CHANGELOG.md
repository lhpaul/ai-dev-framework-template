...
- **`pr-review-loop.sh`: "Metrics row appended" summary line is now conditional on success**: the compare-mode PR summary always claimed "*Metrics row appended to `retro-metrics-platforms.md`*" even when `append_compare_metrics_row` failed. The `_compare_metrics_appended` flag now tracks actual success, and the line is omitted on failure.
