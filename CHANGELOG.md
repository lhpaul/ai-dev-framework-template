...
- **Carve out spec/plan stages from `TOOL_FIX=unknown` serialization rule** (#571): Protocol 90's `TOOL_FIX=unknown` handling paragraph previously stated "treat `unknown` the same as `yes`" unconditionally, causing items at the spec-writing or plan-writing stage (e.g., #562 and #563) to be incorrectly serialized into their own sub-batches. Adds a "Spec/plan stage carve-out" to Protocol 90 that treats `TOOL_FIX=unknown` as `no` when the item's tracker status is `Writing Spec` or `Writing Plan` (or its `NEXT_ACTION` is `write-plan`, `run-spec-review-and-open-pr`, or `run-plan-review-and-open-pr`). Updates `workflow-batch-plan.sh` to apply the same carve-out at the script level so the output reflects `TOOL_FIX=no` for spec/plan-stage items with no spec or plan documents, preventing unnecessary serialization when those items' PRs cannot invoke canonical tool files.

- **`pr-review-loop.sh`: detect CodeRabbit outside-diff comments as unresolved**: `coderabbit_thread_gate_clean` previously only used the GraphQL `reviewThreads` API, which misses review comments posted on lines outside the PR diff (`line == null`). These "outside-diff" comments are visible in the GitHub PR UI as unresolved findings but never create proper GitHub review threads. A new `check_unreplied_rest_comments` function queries the REST pulls-comments endpoint and counts root CodeRabbit comments that have no human reply; the thread gate now returns `RESULT=needs_fixes` with `REASON=coderabbit_unreplied_rest_comments` when any such comments exist. REST failures are non-fatal (logged as a warning; the GraphQL result is still honoured).
- **Clarify Step 3b heading and add mandatory completion gate in retrospective protocol** (#554): renames the Step 3b heading from "opt-in — skipped when not configured" to "runs when `template.repository` is configured; skipped otherwise" to prevent agents from treating the whole step as opt-in; adds a hard gate (Step 3b Gate) immediately before Step 3c that requires agents to return to Step 3b and complete it before classifying any findings when `template.repository` is set. Addresses two downstream misclassifications (Step 5.2 serial-dispatch residuals and missing `ready-for-regression` label both marked "Add to backlog" instead of `already-tracked` / `already-fixed`).
...
- **Close worktree branch-leak gap in stage subagent dispatch**: Claude Code subagents dispatched via the `Agent` tool start with an independent shell context — the worktree CWD guard sourced by the item-orchestrator does not propagate to dispatched subagents, allowing the `developer` subagent to run `git checkout develop` / `git checkout -b` from the main repo CWD and silently leak a branch-switch into the main working tree. Fixed by adding an explicit `BATCH_CONTEXT` branch-skip rule to `developer.md` agent system prompts (`.claude/`, `.cursor/`, Codex `workflow-implementer`), a mandatory Stage-agent handoff branch-skip requirement to `item-orchestrator.md` and the `workflow-item-orchestrator` Codex skill, a Guard scope limitation note and Stage-agent handoff branch-skip requirement block to Protocol 91 Step 3, and updating the Protocol 90 Step 5.2 escalation message to reference the correct root cause.
- **Move "Automated Reviewer Loop Summary" posting into `pr-review-loop.sh`** (#504): `pr-review-loop.sh` now posts the `### Automated Reviewer Loop Summary` PR comment automatically on `clean` and `escalate` exits, eliminating the recurring agent omission that caused PRs to reach `ready-for-human-review` without the Step 7 summary (recurred in PRs #460, #488, #503). For `needs_fixes` at `cycle >= max_cycles`, the orchestrator passes the new `--post-final-summary` flag on the last invocation and the script posts the summary automatically. The comment body matches the `Automated Reviewer Loop Summary|Reviewer Loop Summary|No blocking PR feedback` regex consumed by `workflow-next-action.sh` and Step 5.1 verification. Protocols 91 Step 7, 90 Step 5.1, 93, and 03 Step 9 are updated to reflect that the script now owns all three terminal exit summaries. The Step 7a Internal Review Gate Summary is unaffected and remains agent-owned.
- **Document hotfix backport PR reviewer-loop exemption** (#508): `03-implement-development-protocol.md` now explicitly states that running the full automated reviewer loop is optional for identical cherry-pick backport PRs; proceeding directly to merge is acceptable when automated reviewers post a clean result or no result; blocking findings must still be addressed before merge; backport PRs with conflict-resolution changes must run the full review loop.
- **Sync protocol now validates CI workflow presence and script references** (#513): `sync-template` skill (`.claude/skills/sync-template.md`, `.claude/commands/sync-template.md`, `.cursor/commands/sync-template.md`) adds Step 4.5 — a CI workflow health check that runs after applying template changes and before generating git instructions. The check verifies that `.github/workflows/` exists, that all workflow YAML files parse without errors (via `yamllint` or Python `yaml.safe_load` fallback), and that every `scripts/` path referenced in `run:` steps resolves to an actual file. Parse errors block the commit; missing script paths surface as warnings with an offer to copy from the template source when the gap was introduced by the sync.
- **Clarify Step 5.2 violation counter applies to parallel batches only** (#516): adds an explicit note to the `Recurrence tracking` block in Protocol 90 Step 5.2 stating that working-tree residuals from serial subagent dispatch are expected and do not increment the violation counter; only parallel-batch agents running in isolated worktrees can produce genuine Step 5.2 violations. Updates the `When Step 5.2 fires` header and the retrospective-notes tally bullet to reinforce the parallel-only scope, preventing orchestrators from inflating violation counts with normal serial-dispatch behaviour.
- **Add Shell Scripting section to general best practices** (#521): adds a `## Shell Scripting` section to `docs/best-practices/1-general.md` covering the five patterns most consistently missed in agent-authored scripts — fail-open error handling, input validation before external commands, grep pattern anchoring, pipe exit-code propagation with `pipefail`, and glob precision. Prevents the multi-round review cycles caused by standard bash hygiene issues (as seen in PR #517).
- **Surface PR-Agent advisory findings in reviewer loop summary** (#523): `pr-review-loop.sh` now collects advisory (non-blocking) labels from PR-Agent comments that return `clean` and includes them in the `### Automated Reviewer Loop Summary` comment under an "Advisory findings (non-blocking)" section with a link to the PR-Agent comment. The fix-loop behavior is unchanged — advisory-only labels still return `clean` and do not trigger a fix round. When no advisory findings are present the summary is unchanged.
- **Make `_pr_agent_classify` label-aware to stop false `needs_fixes` loops** (#518): the classifier in `pr-review-loop.sh` now inspects the bold `<strong>LABEL</strong>` tokens inside a `Recommended focus areas for review` section; advisory-only labels (`Possible Issue`, `Edge Case`, `Logic Gap`, `Documentation Inconsistency`) are non-blocking; hard-blocker (`Critical`, `Must Fix`, `Breaking Change`), security (`Security Concern`), and compatibility (`API Change`, `Backward Compatibility`) labels → `needs_fixes`; any unrecognized label or no parseable labels → `needs_fixes` (conservative). Prevents chore/sync PRs from looping on non-actionable suggestions.
- **`codex-github-reviewer.sh` async-arrival grace period** (#505): after the main poll budget is exhausted (all `--max-retriggers` attempts consumed), the script posts a single async-arrival trigger comment and waits one additional `POLL_INTERVAL` before declaring `TIMED_OUT`. Catches bot responses that arrive just after the polling window closes without extending the full `MAX_WAIT` budget, reducing false-unavailable verdicts for the recurring async-arrival pattern.
- **Document non-blocking treatment of advisory ShellCheck findings in PR-Agent config** (#511): adds a clarifying note to `.pr_agent.toml` explaining that context-specific workarounds flagged by PR-Agent under `Possible Issue` (e.g. ShellCheck SC2188 false-positives, intentional `pipefail` suppressions) are non-blocking and will not cause reviewer-loop churn; the underlying fix was delivered by #518.
...
- **Add pre-commit edge-case reasoning guidance to hotfix Path 4** (#509): `03-implement-development-protocol.md` Path 4 Step 4 now includes a required pre-commit checklist prompting the implementer to reason through edge cases before the first commit — specifically: inputs or response formats where the change behaves unexpectedly; for regex/string matching, testing both the false-positive case and the true-positive case including when both appear in the same string or line; for conditional filtering, verifying a suppression condition does not also suppress a genuine match.
- **Add mandatory branch-context verification after `git switch` in Protocol 91** (#520): adds a `Branch-context verification` block immediately after the worktree `cd` step in Protocol 91 Step 3; agents must run `git rev-parse --abbrev-ref HEAD` and abort if the current branch does not match the expected branch. Prevents the Batch 33 incident pattern where RTK-filtered `git switch` output suppressed the confirmation message and commits landed on the wrong branch.
- **Document ShellCheck false-positive suppression pattern** (#512): adds a `## Shell Scripting` section to `docs/best-practices/1-general.md` covering when `# shellcheck disable=` directives are appropriate (e.g., intentional glob patterns in `case` arms, deliberate word-splitting), the correct line-level suppression syntax with a mandatory inline explanation comment, and four rules governing scope and justification. Prevents rework caused by undocumented or unexplained disable directives in workflow scripts. <!-- markdown-heuristic-disable COUNT001 -->
- **Require mechanism citation for behavioral guarantees in implementation plans** (#515): `REVIEW.md` plan review checklist now requires that every behavioral guarantee (e.g., "at most once per run", "bounded", "idempotent") cites the specific enforcement mechanism (flag, guard clause, constraint, lock); a missing mechanism citation is a blocking finding.
- **Add base-branch guard to all `gh pr create` steps in Protocol 03** (#536): every PR-creation step in `03-implement-development-protocol.md` (all four paths and the hotfix backport) now includes a mandatory pre-create `git merge-base --is-ancestor` check and a post-create `gh pr view --json baseRefName` assertion; if the assertion fails the agent must close the malformed PR and report the failure rather than leaving it open against the wrong base.
- **Restrict CodeRabbit review to PR diff and add cross-file expansion guidance** (#537): `.coderabbit.yaml` now sets `changed_files_only: true` under `reviews:` so CodeRabbit only comments on files that appear in the PR diff, preventing scope inflation from tangentially-related file suggestions. `93-automated-reviewer-loop-protocol.md` adds a "Cross-file expansion" section instructing agents to defer out-of-scope reviewer suggestions to a new backlog issue rather than addressing them inline, with an exception for cross-cutting consistency issues introduced by the PR itself.
- **Add comprehensive pre-flight diagnostic and dry-run mode to sync-template** (#538): `sync-template` (`.claude/commands/sync-template.md`, `.claude/skills/sync-template.md`, `.cursor/commands/sync-template.md`, `.codex/skills/workflow-sync-template/SKILL.md`) adds Step 0.5 — a read-only diagnostic phase that runs immediately after the template source is resolved and before any files are modified. The diagnostic detects all four failure categories in one pass: (1) file-level conflict risks in always-sync files, (2) CI configuration issues including missing workflow files and script reference gaps, (3) CHANGELOG structural defects including missing `[Unreleased]` section, missing link reference definitions, and duplicate section headers, and (4) protocol file incompatibilities including `.ai-dev-workflow.yaml` validity and unconfigured review platform references. The diagnostic produces a categorized report and a resolution plan so the agent can sequence the full fix upfront rather than discovering issues progressively. A new `--dry-run` flag runs only Step 0.5 and stops without modifying any files, allowing agents to plan the full resolution sequence before committing to a sync.
- **Add CHANGELOG link reference definition lint check** (#539): `check-changelog-duplicate-headers.sh` now also verifies that every `## [X.Y.Z]` versioned section heading and `## [Unreleased]` (when versioned sections exist) has a corresponding `[X.Y.Z]: https://...` link reference definition line at the bottom of the file, following Keep a Changelog convention. The check exits non-zero when any definition is missing, preventing broken links in rendered CHANGELOGs. `docs/best-practices/2-version-control.md` is updated to document this requirement explicitly.
- **Harden `GITHUB_PROJECT_OWNER` resolution in `workflow-lib.sh`** (#549): replaces the silent `gh repo view` fallback for `GITHUB_PROJECT_OWNER` in `get_tracker_status_for_issue` and `update_tracker_status_best_effort` with a new `workflow_resolve_github_project_owner` helper that implements a three-tier fallback chain: (1) `GITHUB_PROJECT_OWNER` env var, (2) `gh repo view` API call, (3) parse owner from `git remote get-url origin` (SSH or HTTPS). When sourced directly in a subagent shell where `GITHUB_TOKEN` is absent, `gh repo view` fails silently — the git-remote fallback is reliable in all cloned-repo contexts regardless of `gh` authentication state. A visible warning is emitted to stderr only when all three tiers fail, replacing the previous silent skip behavior that caused tracker updates to be lost across two consecutive batches (Batch 37 PR #547, Batch 38 issue #528).
- **Require immediate commit and push of `retro-metrics.md` in retrospective protocol** (#552): Step 6 of `06-retrospective-protocol.md` now explicitly requires committing and pushing `docs/workflow/retro-metrics.md` to `develop` immediately after appending the metrics row, before the session ends or any branch switch occurs. Prevents silent loss of the metrics row when the agent switches branches (as occurred in Batch 37 — the row was appended but not committed before the Batch 38 implementation run switched away from `develop`, causing the row to be discarded).
- **Add intra-file content-duplication check to documentation PR review pass** (#553): `REVIEW.md` Pass 2 (Code Quality) gains a new "Additional checks for documentation PRs" block with two checks: (1) intra-file content duplication — new sections must not reproduce tables or lists already present elsewhere in the same file (flag as `important`, recommend cross-referencing the canonical location); (2) wording consistency — procedural instructions in a new section must not contradict the flow described in sibling sections of the same file (flag as `important`). Addresses the Batch 38 pattern where the internal reviewer missed a duplicate severity-classification table and contradictory troubleshooting wording in PR #550. <!-- markdown-heuristic-disable COUNT001 -->
- **Auto-file upstream issues for `contribute-upstream` retrospective findings** (#555): adds mandatory Step 3e to `06-retrospective-protocol.md`: for every finding classified `contribute-upstream` in Step 3b, the retrospective agent automatically creates a GitHub issue on `template.repository` (with the `workflow` label) before presenting findings to the human. The filed issue URL is printed in the Step 4 output alongside the `contribute-upstream` label, giving the human a direct link for follow-up. The Step 5 "Contribute upstream" action is updated to note that auto-filing already occurred and to clarify the remaining optional role (adding context to the filed issue). Addresses the pattern where `contribute-upstream` findings from downstream retrospectives were identified but not filed on the template repository during the session.
- **Suppress redundant intermediate reviewer loop summary comments** (#570): `pr-review-loop.sh`'s `_post_review_summary` now checks for an existing script-posted summary comment (identified by the `*Posted automatically by \`pr-review-loop.sh\`.*` marker) and updates it in place via `gh api PATCH` rather than always creating a new comment. This prevents multiple identical "clean — no blocking findings" summary comments from accumulating on the PR timeline when the orchestrator invokes the script multiple times across fix cycles (e.g. when async bot reviewers post new findings after a clean exit triggers a re-run).
- **Fix missing `ready-for-regression` label on Fast Track `fix/*` PRs in parallel batch dispatch** (#556): expands Path 3 (Fast Track) Step 9 in `03-implement-development-protocol.md` to inline the full two-phase pre-label ordering gate (Phase 1: Steps 1.1–1.3; Phase 2: Steps 2.1–2.2) previously only present in Path 1 Step 9, so agents following only the Fast Track section see the explicit `ready-for-regression` requirement without a cross-reference lookup. Adds a `BATCH_CONTEXT=true` mandatory-step note to Protocol 91 Step 7b to make the label requirement prominent in the compressed parallel-dispatch path. Adds a Step 7b regression-label assertion to `pr-review-loop.sh`'s `_post_review_summary` function: when the reviewer loop exits `clean` for an implementation branch and `ready-for-regression` is not yet applied, the auto-posted summary comment includes a `Step 7b WARNING` with the apply command, surfacing the gap immediately rather than waiting for the orchestrator's Step 5.1 enforcement.
...
[Unreleased]: https://github.com/lhpaul/ai-dev-framework-template/compare/v0.25.1...HEAD
