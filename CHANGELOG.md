...
- **Document ShellCheck false-positive suppression pattern** (#512): adds a `## Shell Scripting` section to `docs/best-practices/1-general.md` covering when `# shellcheck disable=` directives are appropriate (e.g., intentional glob patterns in `case` arms, deliberate word-splitting), the correct line-level suppression syntax with a mandatory inline explanation comment, and four rules governing scope and justification. Prevents rework caused by undocumented or unexplained disable directives in workflow scripts.
