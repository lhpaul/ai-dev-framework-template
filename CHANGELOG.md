...
- **`ready-for-regression` label no longer dropped by fix commits after the reviewer loop has run** (#805) — a `synchronize` event from any post-review-loop commit silently removed the `ready-for-regression` label, requiring manual re-application before CI could run on the final commit (recurred in 6+ batches). Fixed by two complementary changes: (1) `pr-review-loop.sh` now auto-restores the `ready-for-regression` label at the start of every invocation for `feature/*`, `fix/*`, `refactor/*`, and `hotfix/*` branches — if the label was dropped by a push, the next reviewer loop run re-applies it before any platform reviews; (2) `remove-ready-for-regression-on-push.yml` now skips label removal when an "Automated Reviewer Loop Summary" comment is already present on the PR, meaning the loop has run at least once and the label was deliberately applied by the orchestrator. The removal workflow retains its original behavior (remove the label) on first-push / before the loop has run, preserving the intended gate for initial commits. Added 4 unit tests in `test-pr-review-loop.sh` (Area 11) covering the new auto-restore logic.
