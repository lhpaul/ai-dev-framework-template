...
- **Move "Automated Reviewer Loop Summary" posting into `pr-review-loop.sh`** (#504): `pr-review-loop.sh` now posts the `### Automated Reviewer Loop Summary` PR comment automatically on `clean` and `escalate` exits, eliminating the recurring agent omission that caused PRs to reach `ready-for-human-review` without the Step 7 summary (recurred in PRs #460, #488, #503). The comment body matches the `Automated Reviewer Loop Summary|Reviewer Loop Summary|No blocking PR feedback` regex consumed by `workflow-next-action.sh` and Step 5.1 verification. Protocols 91 Step 7, 90 Step 5.1, 93, and 03 Step 9 are updated to reflect that the `clean`/`escalate` summary is now script-owned; the `needs_fixes` at `cycle >= max_cycles` summary remains agent-owned. The Step 7a Internal Review Gate Summary is unaffected and remains agent-owned.
