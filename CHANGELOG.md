...
- **`claude-code-action` review platform** — `pr-review-loop.sh` now recognizes `claude-code-action` as a review platform. Add it to `review.platforms` in `.ai-dev-workflow.yaml` to integrate a GitHub Actions-based Claude Code review into the automated reviewer loop. Companion script `scripts/development-workflow/claude-code-action-reviewer.sh` dispatches a `workflow_dispatch` event, polls for the resulting Actions run to complete, and exits with the same three-code contract used by `codex-github-reviewer.sh` (0 = APPROVED, 1 = NEEDS_REVISION, 2/3 = TIMED_OUT/UNAVAILABLE).
