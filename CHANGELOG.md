...
- **PR-Agent configuration race condition** (#499): PR-Agent merges TOML settings (`model`, `fallback_models`, `model_weak`, `pr_actions`, `auto_describe`, `auto_improve`) *after* its initial fallback / action-filter checks, so a TOML-only configuration silently falls back to the built-in OpenAI default (`gpt-5.4` / `gpt-5.4-mini`) and fails with `dummy_key` auth errors on every PR. Pin all relevant settings as `config.*` and `github_action_config.*` env vars in `.github/workflows/pr-agent.yml` so they take effect before the fallback fires; `model_weak` is also added to `.pr_agent.toml` as defense-in-depth. Updates `docs/workflow/development-workflow/integrations/pr-agent.md` to document that env vars are the load-bearing pin and TOML serves as documentation.
