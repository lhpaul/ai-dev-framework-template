...
- **Workflow shell guard lint and after-clean readiness gate**: add a diff-based lint check for new `scripts/development-workflow/**/*.sh` lines that suppress `gh`, `git`, `curl`, or `haystack` failures with `|| true` without an explicit `workflow-shell-guard` rationale. The ShellCheck workflow now runs the guard and its harness for workflow-script changes. `pr-review-loop.sh` also converts draft PRs to ready before dispatching the first `review.phase_after_clean` platform, so Haystack and other ready-only reviewers do not run against draft PRs.
