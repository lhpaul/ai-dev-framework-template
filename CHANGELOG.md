...
- fix(item-orchestrator): item-orchestrator agents running in isolated git worktrees now receive an explicit reminder that all Write/Edit tool calls must target paths under the resolved worktree path, not the main repo root; same guardrail added to both Claude Code and Cursor agent prompts (#193)
