...
- **`ready-for-regression` label no longer dropped by fix commits after the reviewer loop has run** (#805) — a `synchronize` event from any post-review-loop commit silently removed the `ready-for-regression` label, requiring manual re-application before CI could run on the final commit (recurred in 6+ batches). Fixed by two complementary changes: (1) `pr-review-loop.sh` now auto-restores the `ready-for-regression` label at the start of every invocation for `feature/*`, `fix/*`, `refactor/*`, and `hotfix/*` branches via a new `restore_regression_label_if_missing()` function — if the label was dropped by a push, the next reviewer loop run re-applies it before any platform reviews; `gh pr edit` failures are now surfaced to stderr (not silently swallowed), and the label-presence check now properly propagates `gh` API failures as a fail-open skip instead of masking them; (2) `remove-ready-for-regression-on-push.yml` now skips label removal when an "Automated Reviewer Loop Summary" comment posted by a bot account (`user.login` ending with `[bot]`) is already present on the PR — the author-scope filter prevents a human comment quoting the loop-summary phrase from triggering a false skip; the `gh api ... | jq` pipeline now captures `gh` failures explicitly so an API error cannot be misread as "no summary comment found"; the removal workflow retains its original behavior (remove the label) on first-push / before the loop has run, preserving the intended gate for initial commits. Replaced 4 grep-only source-inspection tests in `test-pr-review-loop.sh` (Area 11) with 6 behavioral tests that invoke the new function directly with a mocked `gh`, asserting label restore on absent label, no-op on present label, no action on non-implementation branches, fail-open on `gh` view failure, hotfix branch coverage, and function placement before the harness return point.
