...
- **Protocol 90 Step 5.1: verify reviewer-loop summary recency via `updated_at`, not `created_at`** — `pr-review-loop.sh` maintains a single "Automated Reviewer Loop Summary" comment and updates it in place, so a freshness check using `created_at` (which reflects the comment's first post) produces false "stale / incomplete run" verdicts for PRs whose summary was legitimately refreshed on a later commit or rebase. The Stale/Incomplete PR Detection section now documents comparing the summary comment's `updated_at` (fetched via REST, since `gh pr view --json comments` does not expose it) against the latest commit timestamp.
