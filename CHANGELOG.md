...
- **Protocol 90 Step 5.1: verify reviewer-loop summary recency via `updated_at`, not `created_at`** (#823) — `pr-review-loop.sh` maintains a single "Automated Reviewer Loop Summary" comment and updates it in place, so a freshness check using `created_at` (which reflects the comment's first post) produces false "stale / incomplete run" verdicts for PRs whose summary was legitimately refreshed on a later commit or rebase. The Stale/Incomplete PR Detection section now documents fetching the summary comment's `updated_at` via REST (since `gh pr view --json comments` does not expose it), converting it to epoch seconds, and comparing it against the latest commit timestamp.
- **Haystack reviewer loop now surfaces review-policy verdicts from `haystack pr-status`** (#818) — `haystack-reviewer.sh` no longer relies only on `haystack triage --json` findings. It also reads `haystack pr-status --json` and emits policy metadata when Haystack marks a PR as `needsHumanReview` / `analysisVerdict: needs-review`. `pr-review-loop.sh` keeps these verdicts advisory when there are no blocking findings, but the PR summary now displays `haystack (needs-review: policy)` instead of incorrectly reading as `haystack (clean)`. Clean `needs-assignment` states are visible without blocking deterministic workflow progress, and the summary now includes the Haystack bucket, `needsHumanReview`, and a disposition such as `blocking`, `policy-human-review`, `advisory-only`, or `good-to-merge`.
- **`ready-for-regression` label no longer dropped by fix commits after the reviewer loop has run** (#805) — a `synchronize` event from any post-review-loop commit silently removed the `ready-for-regression` label, requiring manual re-application before CI could run on the final commit (recurred in 6+ batches). Fixed by two complementary changes: (1) `pr-review-loop.sh` now auto-restores the `ready-for-regression` label at the start of every invocation for `feature/*`, `fix/*`, `refactor/*`, and `hotfix/*` branches via a new `restore_regression_label_if_missing()` function; the restore is gated on the presence of an "Automated Reviewer Loop Summary" comment on the PR — when the summary comment exists, the label was previously applied by the loop and dropped by a push (restore is correct); when the summary comment is absent, the loop has never run and the label was never applied (restore is suppressed, preventing any human intentional removal before the loop's first run from being overridden); if the comment-presence query fails, the function fails open (restores) and emits a WARN; the marker match is author-agnostic (`### Automated Reviewer Loop Summary`, not scoped to `[bot]`) to cover both local-run and CI-run cases; (2) `remove-regression-label-on-push.yml` now skips label removal when an "Automated Reviewer Loop Summary" comment exists on the PR — the same canonical marker ensures symmetry with the restore gate; the `gh api ... | jq` pipeline captures `gh` failures explicitly so an API error cannot be misread as "no summary comment found"; the removal workflow now also fails open toward NOT removing the label on comments API failure (previously it treated any API failure as "no summary comment found" and removed the label, defeating the protection on transient API errors — the fix sets `LOOP_COMMENT=1` on failure so only a successful API response showing zero summary comments triggers removal); the removal workflow retains its original behavior on first-push / before the loop has run when the API succeeds, preserving the intended gate for initial commits. Updated 6 behavioral tests in `test-pr-review-loop.sh` (Area 11) and added 3 new assertions for the summary-comment gate: label absent + summary present → restore; label absent + summary absent → no restore; comments API failure → fail-open restore with WARN. Total: 91 tests (up from 85).
- **Workflow shell guard lint and after-clean readiness gate**: add a diff-based lint check for new `scripts/development-workflow/**/*.sh` lines that suppress `gh`, `git`, `curl`, or `haystack` failures with `|| true` without an explicit `workflow-shell-guard` rationale, including backslash-continued commands. The ShellCheck workflow now validates pull-request base-ref input, runs the guard, and runs its harness for workflow-script changes. `pr-review-loop.sh` also converts draft PRs to ready before dispatching the first `review.phase_after_clean` platform, so Haystack and other ready-only reviewers do not run against draft PRs.
- **Haystack local hooks and review routing: validate Conventional Commits, document the operating model, and route Haystack after draft cleanup** — `hooks/commit-msg` now enforces the repository Conventional Commits subject format and 72-character first-line limit while preserving standard merge, quoted/unquoted revert, fixup, squash, and amend exceptions. The hook supports hierarchical scopes such as `ui/button`, and the workflow test harness covers the hook's acceptance, rejection, prefix bypass, input-handling, and length-boundary behavior. Haystack is now configured in `review.phase_after_clean` so the reviewer loop marks draft implementation PRs ready before invoking `haystack triage`, avoiding draft-only `pending_timeout` escalations. The Haystack integration guide now distinguishes local deterministic hooks, `.haystack` PR-review rules, and the `haystack triage` reviewer-loop integration, with a small hardening roadmap for future checks.
- **Codex command-style skill aliases**: add repo-scoped `.agents/skills/` discovery support and aliases for Claude Code command names such as `/add-backlog-item`, `/run-work`, `/run-item-work`, `/run-reviewer-loop`, `/batch-merge`, `/post-merge-cleanup`, `/prepare-release`, `/graduate-development`, `/retrospective`, and `/sync-template`. The installer now links skills into current Codex skill discovery paths (`~/.agents/skills`) while retaining legacy `~/.codex/skills` compatibility, with test coverage for install ordering, per-alias installation, non-symlink skip behavior, and missing source validation.
- **`claude-code-action-reviewer.sh`: remove `inputs.pr_number` verification that always failed; add run-name PR scoping for concurrent dispatch** (#806, #808) — the Phase 2 poll loop verified each candidate Actions run by fetching `.inputs.pr_number` from the individual run endpoint, but the GitHub Actions Runs API always returns `inputs: null` regardless of what was passed at dispatch time (confirmed empirically). The check always evaluated to "skip this cycle", causing the loop to spin until `MAX_WAIT` (default 600 s) before falling through to Phase 3. The net result was correct (Phase 3's unresolved-thread query is already scoped to the correct PR number), but every PR using `claude-code-action` incurred a ~10-minute wall-clock penalty per reviewer-loop invocation. Fix: removed the `inputs.pr_number` lookup and the `EXCLUDED_RUN_IDS_JSON` exclusion list entirely. To prevent incorrect run selection under concurrent/parallel dispatch (where two PRs trigger the review workflow within the same poll window), `claude-code-review.yml` now sets `run-name: "Claude Code Review — PR #${{ inputs.pr_number }}"`, and the Phase 2 jq filter uses a two-tier selection: when any candidate run in the window has a `"PR #N"`-style name (indicating the new workflow is deployed on the default branch), the filter requires `.name | test("PR #<pr_number>\b")` to be true; when no candidate has that pattern (old workflow during the deployment transition), it falls back to timestamp-only selection for backward compatibility. Unlike `.inputs` (always null in the Runs API), `.name` is populated from `run-name` and reliably identifies which PR triggered the run. Updated unit tests in `scripts/development-workflow/tests/test-claude-code-action-reviewer.sh` adding Area 5 (6 new behavioral tests): concurrent dispatch where the most-recent run belongs to another PR (must select this PR's run), only this PR's run present, only the other PR's run present (null), word-boundary guard (PR #8080 does not match PR #808), two retries for same PR (most recent), and backward-compat fallback (generic names → newest selected).
- **`reviewer-loop-guard.yml`: replace fixed-count grace period with explicit comment-existence poll** (#790) — the previous grace-period approach (3 polls × 30 s = 90 s) proved insufficient across batches 67–69 and recurred on PRs #808–#809: the guard fired before `pr-review-loop.sh` had finished posting its summary comment. The guard now polls every `GUARD_POLL_INTERVAL` seconds (default: 30) until either the summary comment is found or `GUARD_MAX_WAIT` seconds (default: 600) have elapsed. This eliminates the timing race by construction — the guard waits for the actual comment to exist rather than assuming it will appear within a fixed window. The old `GUARD_MAX_POLLS` / `GUARD_POLL_INTERVAL` pair is replaced by the new `GUARD_MAX_WAIT` / `GUARD_POLL_INTERVAL` pair for clearer configuration semantics.
- **`haystack-triage.md` / `haystack-reviewer.sh`: document recurring `keep-changelog-unreleased-structure-canonical` false positive and add agent "do not act" guard** (#807) — Haystack's `Rules violation` category fires a false positive on PRs whose CHANGELOG entry is correctly formatted under `[Unreleased]` → `### Fixed` (rule `keep-changelog-unreleased-structure-canonical`). This has recurred in Batches 34, 35, 55, and PR #801. The existing documentation only covered the hotfix backport PR case; this adds explicit guidance for the broader regular PR pattern. `haystack-triage.md`: the `"Rules violation"` section is expanded with a visible agent warning block ("do not attempt to fix this"), explanation of why the LLM checker misfires on diffs, and a two-condition dismissal checklist; the hotfix backport sub-case is preserved under a nested heading; a new troubleshooting entry is added for the regular PR case; the severity mapping table link target is updated. `haystack-reviewer.sh`: the header comment for `Rules violation` is expanded to enumerate both false positive scenarios and includes an explicit IMPORTANT notice so agents reading the script source also receive the do-not-fix signal. Option A (Haystack per-repo rule suppression) was investigated but is not supported by the CLI schema — Haystack's `pr-rules.yml` has no `enabled`/`disabled` field per rule; the `haystack init --help` and `triage` command expose no suppression flags. Option B (documentation guard) is the implemented fix.
- **`haystack-reviewer.sh`: recover from non-zero CLI exit and treat `status=error`/incomplete states as transient (never clean)** (#800) — two related false-result bugs fixed: (1) some haystack CLI versions return a non-zero exit code even when `haystack triage --json --no-wait` produces a valid completed-analysis payload (observed in Batch 70/71); any non-zero exit now inspects stdout before reporting `UNAVAILABLE` — valid completed JSON proceeds to findings parsing, valid `status=pending` or `status=error` JSON enters the poll-retry loop, and only empty/non-JSON stdout with a non-zero exit reports `UNAVAILABLE`. (2) The `status` case block previously treated any unknown/unrecognised status value (including `status=error` and "Rating synthesis not available") as a terminal completed state via the `*)` catch-all, proceeding to findings parsing on an incomplete analysis and emitting a false `RESULT=clean`. Empirically (confirmed 2026-06-02), a genuinely completed analysis has **no `status` field** in its JSON payload — the `*)` catch-all has been replaced by an explicit empty-string branch (no status field = completed, proceed to findings) and a `*)` transient branch (any non-empty, non-"none" status = still synthesizing, poll-retry). `status=error` and all other non-terminal status values now follow the same poll-retry path as `status=pending`; budget exhaustion on any transient state emits `RESULT=skipped REASON=pending_timeout` — never `RESULT=clean`. Added 5 new unit tests in Area 9 of `test-haystack-reviewer.sh` (68 total): `status=error` → retry → findings surfaced; `status=error` persisting until timeout → `pending_timeout`; completed-0-findings → clean (the only clean path); `status=error` → retry → 0 findings → clean; non-zero-exit + `status=error` → retry → findings surfaced.
- **Backport bug fixes from downstream sync review (leasity-exchanges-prototype #279)**: eleven issues discovered during a downstream template sync are applied to the template source. `pr-review-loop.sh`: `run_codex_github_review()` now escalates instead of silently proceeding when the Phase-1 `check_unresolved_threads` call fails; Copilot `CHANGES_REQUESTED` now counts actual inline review comments via the reviews-comments API instead of hardcoding `1`; Copilot poll loop now logs a `WARN` when `review_state` is empty under `set +e` instead of silently falling through; `unlock` subcommand now exits 1 with an actionable `ERROR` message instead of `WARN`-and-continue when lock PID/cmd metadata files are unreadable; summary-comment fallback path now uses `--body-file` with a temp file instead of `--body "$var"` to avoid shell metacharacter injection; `head -1` → `head -n 1` (POSIX) in `run_haystack_review()`. `batch-merge.sh`: `head -5` → `head -n 5` (POSIX). `claude-code-action-reviewer.sh`: jq run-poll filter now adds `sort_by(.created_at) | reverse` before `first` for deterministic selection of the most recent run; epoch→ISO8601 conversion now includes a `python3` fallback for platforms with neither BSD `date -r` nor GNU `date -d`. `05b-graduate-development-protocol.md`: `head -40` → `head -n 40` (POSIX). `91-orchestrate-work-protocol.md`: fix exit-code reference for late-arriving async bot threads (`exit 5` → `exit 6`) in the interpretation table, matching the exit-code contract table and the inline code snippet.
- **`haystack-reviewer.sh`: poll-retry when `status=pending` instead of mapping to `UNAVAILABLE`** (#795) — `haystack triage --no-wait` exits immediately when the Haystack cloud analysis is not yet complete (typically 2–4 minutes after a PR push). Previously, a `status=pending` response was mapped identically to `status=none` (permanent no-analysis) and emitted `RESULT=skipped REASON=unavailable`, silently skipping findings whenever the reviewer loop ran in that timing window. The script now enters a poll-retry loop: it waits `HAYSTACK_POLL_INTERVAL` seconds (default: 15) and retries `haystack triage --no-wait` until the analysis completes or the overall `HAYSTACK_REVIEWER_TIMEOUT` budget (default: 120s) is exhausted. Budget exhaustion on a pending analysis now emits `RESULT=skipped REASON=pending_timeout` (distinct from `REASON=unavailable` and `REASON=timeout`), enabling callers to distinguish the three unavailability sub-cases. Also fixes `normalize_platform_verdict()` in `pr-review-loop.sh` to map `REASON=pending_timeout` to `'timed out'` (not `'unavailable'`) in compare-mode metrics. Added dedicated unit tests in `scripts/development-workflow/tests/test-haystack-reviewer.sh` covering the retry loop, pending_timeout exit, and distinct REASON values; added `verdict_escalate_pending_timeout` test in `test-pr-review-loop.sh`. Updated `haystack-triage.md` with revised timeout/poll-interval configuration table, updated exit-code contract table, and updated troubleshooting section replacing the timing-gap workaround guidance with the new automatic retry behaviour.
- **`pr-review-loop.sh`: capture and surface `haystack-reviewer.sh` stderr instead of discarding it** (#796) — `run_haystack_review()` previously called `haystack-reviewer.sh` with `2>/dev/null`, silently dropping the companion script's `INFO:` diagnostics. Callers could not distinguish between CLI-not-installed, auth failure, network error, and `status=pending` timing gaps — all were lumped into a single `RESULT=skipped REASON=unavailable` token. The caller now captures stderr to a temp file via `2>"$haystack_stderr_file"` and re-emits it to its own stderr before cleanup, making all diagnostic context visible to orchestrators and human operators.
- **`workflow-lib.sh`: guard `BASH_SOURCE[0]` against unset when sourced from non-Bash contexts** (#792) — `workflow_script_dir()` now uses `${BASH_SOURCE[0]:-$0}` so the script degrades gracefully when sourced from shells or orchestrator contexts (such as Claude Code) that do not populate `BASH_SOURCE`. Previously, sourcing the file directly caused an immediate `parameter not set` fatal error under `set -u`.
...
- **Protocol 03: require renamed-string downstream grep scan before committing** (#791) — all four implementation paths (Full Pipeline, Refactor, Fast Track, Hotfix) now include a "Renamed-string downstream scan" step in the pre-commit verification section. When any literal string, identifier, context name, or signal value is renamed, agents must run `grep -r "old_string" .` across the full repository before staging to surface downstream references in fixture files, test files, documentation, shell scripts, and CI workflows. This closes the gap that required three unplanned fix commits after PR #785 renamed a reviewer-loop-guard context string without scanning for downstream consumers.
