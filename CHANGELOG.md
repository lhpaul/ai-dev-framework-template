...
- **`claude-code-action-reviewer.sh`: remove `inputs.pr_number` verification that always failed** (#806) — the Phase 2 poll loop verified each candidate Actions run by fetching `.inputs.pr_number` from the individual run endpoint, but the GitHub Actions Runs API always returns `inputs: null` regardless of what was passed at dispatch time (confirmed empirically). The check always evaluated to "skip this cycle", causing the loop to spin until `MAX_WAIT` (default 600 s) before falling through to Phase 3. The net result was correct (Phase 3's unresolved-thread query is already scoped to the correct PR number), but every PR using `claude-code-action` incurred a ~10-minute wall-clock penalty per reviewer-loop invocation. Fix: removed the `inputs.pr_number` lookup and the `EXCLUDED_RUN_IDS_JSON` exclusion list entirely; the timestamp filter (`created_at >= POLL_AFTER_TIME`) now serves as the sole run-selection criterion. Added unit tests in `scripts/development-workflow/tests/test-claude-code-action-reviewer.sh` covering timestamp filtering, workflow-file path matching, null-inputs tolerance, and in-progress status passthrough.
