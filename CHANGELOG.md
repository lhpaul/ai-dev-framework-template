...
- **Unverified bash behavior claims in protocol documentation** (`03-implement-development-protocol.md`, `.claude/agents/developer.md`, `.cursor/agents/developer.md`): Protocol 03's Shell Script Quality Checklist lacked a rule requiring verification of bash/shell runtime behavior claims before committing protocol documentation. PR #397 (issue #388) required 5 Devin review cycles to correct inaccuracies introduced in protocol docs (wrong exit-code table row, incorrect `local VAR=$(cmd)` explanation, dead code after `return`, `trap PIPE` vs `trap EXIT` confusion). Added item 8 to the Shell Script Quality Checklist: any new bash/shell behavior claim in protocol documentation must be verified by running a scratch snippet or citing a specific authoritative reference (bash man page, POSIX spec) before committing. Updated developer agent key rules in `.claude/agents/developer.md` and `.cursor/agents/developer.md` to include item 8 in the checklist summary.
