...
- **`pr-review-loop.sh`: read `review.platforms` from PR target branch, not checkout state** (#756) — the script now resolves the PR's base branch via `gh pr view --json baseRefName` and reads `.ai-dev-workflow.yaml` from `origin/<base>` via `git show`, so platform coverage is consistent regardless of the operator's local checkout state. Falls back to the working-tree config if the remote ref is unavailable.
