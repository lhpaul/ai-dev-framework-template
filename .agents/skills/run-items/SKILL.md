...
17. After all in-scope PRs reach `ready-for-human-review`, inspect the effective
   ...
   through merge, cleanup, and tracker reconciliation. Delegated batch merge
   authority never authorizes force-pushing a PR branch; if a destructive PR
   branch update would be required, stop before mutation and route the exact
   operation through `scripts/development-workflow/workflow-branch-push-guard.sh`.
   Never use Protocol 94 auto-discovery from `/run-items`. If any stage does not allow merge, finish at the
