---
name: run-work
description: "Read-only portfolio scan: discovers what can advance and proposes the largest safe batch options. No execution — for bounded multi-item execution use $run-items; for single items use $run-item; for epics use $run-epic."
---
...
2. Run `./scripts/development-workflow/run-work-router.sh [--json]` (no targets — scan mode only).
3. For `no_target_scan`, follow Protocol 90 proposal mode via `workflow-orchestrator`
   to scan and propose. Do not dispatch execution in this mode.
4. For bounded multi-item execution use `$run-items`; for single items use `$run-item`;
