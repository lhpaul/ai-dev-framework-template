...
- `.agents/skills/`: repo-scoped Codex discovery path, including command-style aliases such as `/add-backlog-item`, `/run-work`, `/run-item-work`, `/run-reviewer-loop`, `/batch-merge`, `/post-merge-cleanup`, `/prepare-release`, `/graduate-development`, `/retrospective`, and `/sync-template`.
