...
The rows above are selected by the verdicts the axes carry, not by how many axes a question yields, so exactly one row applies to any question. A question that decomposes into a single axis is read by the same rows as a multi-axis one: a settled single axis the runner conforms to reaches Not an architecture decision, a settled single axis whose behavior departs reaches Departure reported, and a settled single axis whose every citation is Not yet implemented reaches Not an architecture decision. Only a single axis that is Genuinely open reaches the row for a question that does not separate.
