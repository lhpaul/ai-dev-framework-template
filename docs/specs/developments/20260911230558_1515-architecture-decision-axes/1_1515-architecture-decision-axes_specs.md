...
**Actor**: The runner driving the work item.
**Preconditions**: A question has been raised, the runner cannot resolve it alone, the axis decomposition finds the question does not separate into more than one independently answerable component, and the runner has found nothing in the workflow specification that bears on that one axis.

**Steps**:

1. The runner states the question and its source.
2. It performs the normal axis decomposition and finds this question does not separate into more than one independently answerable component, so it has one axis.
...
- One axis is a valid decomposition only where the question genuinely does not separate. A question with more than one independently answerable component — for example, choosing both a database and a deployment platform — decomposes into that many axes even where none of them has any governing specification line; each such axis is reported separately as Genuinely open with the reason No governing line, per Business Rules and the multi-axis outcome below, not folded into this use case's single axis.
- The report is not padded with invented axes to look thorough.
...
- **Stop message**: unchanged in its three existing elements. For a complete report — one carrying no citation whose conformance or substance could not be determined, per the malformed-input rule — the required human action names the genuinely open axes only, rather than the question as a whole. For an incomplete report under the malformed-input rule, the required human action instead supplies what that rule names — the missing question or its source, a redone decomposition, the missing conformance evidence, or confirmation of a citation's substance — since such a report may have no open axis yet to name. Where a malformed-input request is combined with genuinely open axes elsewhere in the same report, per the composition rule in the Decision-Gate Consistency Matrix, the required human action names both: the open axes to decide, and, separately, whatever the malformed-input rule requests — and that report, carrying an unresolved malformed-input request, is incomplete, not complete, regardless of how many axes it also settles or leaves open.
...
Every "citation" referenced anywhere in this matrix — in the Gate inputs, Triggers, Allowed outcomes, and Malformed or missing gate inputs tables below — is a citation offered as support for the runner's behavior or for a decision a human or reviewer will weigh, per Business Rules above. A workflow specification line named only to identify a source, without being offered as support, is outside this feature's scope and carries none of this matrix's declarations or verdicts.
...
| The runner is about to cite a workflow specification line as support for its behavior or a decision, in an escalation report or a reply on a review thread | Fires the conformance declaration alone, without requiring a full escalation report. A citation named only to identify a source, not offered as support, does not fire this trigger, per the scope note above |
...
| Every axis settled, no citation on those axes Departs, at least one Conforms (the rest, if any, Not yet implemented), the runner is not uncertain, and no reviewer or human has raised a citation's substance in question that the runner cannot resolve | Not an architecture decision, unless the runner disputes any citation's substance or a raised substance question remains unresolved | Applies the cited lines and continues; no human decision was required — unless the runner affirmatively judges any citation's substance wrong, whether that citation Conforms or is Not yet implemented, in which case it keeps that citation's own declaration as stated, opens a new axis — Genuinely open with the reason Governing line disputed and a proposed amendment — asking whether the line should change, and stops pending that decision instead of continuing; or, where a reviewer or human raised a citation's substance in question and the runner genuinely cannot resolve it, the malformed-input rule below governs instead of continuing | None, unless a new axis was opened, or the malformed-input rule applies — then decide that axis or answer the raised question |
| The runner's behavior departs on at least one settled axis's citation, whether or not every axis in the decomposition is settled | Departure reported when every axis is settled; where an axis outside the departure remains open, the mixed-axis row above supplies the report's outcome label instead, and this row's content still governs the departure — per the composition rule below | States each departure plainly. For each departed citation independently, the runner either conforms, or opens a new axis — Genuinely open with the reason Governing line disputed and a proposed amendment — asking whether that line should change; different departed citations in the same report can resolve differently, one corrected and another disputed | None, unless at least one new axis was opened — then decide each opened axis |
...
| Every axis settled, coverage certain, every citation on those axes is Not yet implemented, and no reviewer or human has raised a citation's substance in question that the runner cannot resolve | Not an architecture decision, unless the runner disputes a citation's substance or a raised substance question remains unresolved | Proceeds with the work as the cited lines already specify. There is no departure to declare, because the behavior does not exist yet to conform or depart — unless the runner affirmatively judges a citation's substance wrong even though nothing has been built yet, in which case it opens a new axis — Genuinely open with the reason Governing line disputed and a proposed amendment — asking whether the line should change, and stops pending that decision instead of proceeding; or, where a reviewer or human raised a citation's substance in question and the runner genuinely cannot resolve it, the malformed-input rule below governs instead of proceeding | None, unless a new axis was opened, or the malformed-input rule applies — then decide that axis or answer the raised question |
| A citation is offered in a reply on a review thread, outside an escalation report, and its conformance can be determined as Conforms or Not yet implemented | Declaration required        | Attaches that declaration to the citation in place — unless the runner affirmatively judges that citation's substance wrong, in which case it keeps the declaration as stated and opens a new axis — Genuinely open with the reason Governing line disputed and a proposed amendment — asking whether the line should change, per Business Rules above | None, unless a new axis was opened — then decide that axis |
| A citation is offered in a reply on a review thread, outside an escalation report, its conformance is Departs, and that citation's axis is Settled by specification | Declaration required, departure reported | States the departure plainly in the reply, before the reply answers anything else. For that citation, the runner either conforms, or opens a new axis — Genuinely open with the reason Governing line disputed and a proposed amendment — asking whether the line should change, per Business Rules above | None, unless a new axis was opened — then decide that axis |
| A citation is offered in a reply on a review thread, outside an escalation report, its conformance is Departs, and that citation's axis is already Genuinely open for a different reason | Declaration required        | States the departure plainly in the reply, as part of that already-open axis, per Business Rules above; correction or a substance dispute on this citation waits until the axis itself is decided | None beyond deciding the already-open axis                       |
| A citation is offered in a reply on a review thread, outside an escalation report, and the runner cannot determine its conformance | Declaration required, undetermined | States plainly, in that reply, that conformance could not be determined and why — using none of the three declarations, the same content the malformed-input rule requires of such a citation — without opening a full `architecture_decision` escalation for it, since this trigger fires the declaration alone and does not require a full report. Undetermined conformance and the substance judgment below are independent inputs: where the runner separately, and independently of the undetermined conformance, affirmatively judges that citation's substance wrong, it also opens a new axis — Genuinely open with the reason Governing line disputed and a proposed amendment — asking whether the line should change, per Business Rules above | None, unless a new axis was opened — then decide that axis — or unless the axis that citation bears on is later escalated in a full report, where the malformed-input rule governs it |

Exactly one row supplies the report's outcome label for any question — this is a claim about the label, not about content: per the composition rule below, more than one row's required content can govern the same report when more than one specific reason applies at once, even though only one row's label does. Three rows above leave every axis settled on their own: the runner conforms on every citation, every citation on those axes is Not yet implemented, and the runner is unsure the cited lines fully answer the question. Those three read the same way whether the settled axis is the question's only axis or one among several, the first two both reaching Not an architecture decision either way. The departs row differs: its content — the correct-or-dispute remedy for each departed citation — governs any settled axis carrying a departure whether or not other axes remain open, but it supplies the report's own outcome label, Departure reported, only when every axis in the decomposition is settled; where an axis outside the departure is still open, the mixed-axis row above supplies the label instead, and the departs row's content composes with it, per the composition rule below. A citation set that mixes Conforms and Not yet implemented, with no Departs among them, is read by the conforms row: nothing has departed, and the all-Not-yet-implemented row is reserved for the case where nothing cited has been built at all.

The third of those three — the runner is unsure the cited lines fully answer the question — does not stay all-settled once it fires: recording that uncertainty adds a new axis, Genuinely open with the reason Coverage uncertain. That gives the question the same settled-and-open shape as "At least one axis settled, at least one open," and it is the uncertainty row, not the general one, that applies to it, because the uncertainty row names the reason the general row does not. The outcome is the same either way — Escalated, with the decision scope narrowed to the open axis by the settled ones — so naming the more specific row changes what the report says about that axis, not which outcome it reaches. This precedence holds as long as the uncertainty axis is the report's only reason for being open; where it coexists with an axis or citation governed by a different specific reason, the composition rule below applies instead.
...
The outcomes above assume each gate input in the Gate inputs table could actually be established. Where establishing one of them fails for a given axis or citation, none of the outcomes above apply to that axis or citation; the matching row below governs it instead, and every one of these rows fails closed — the escalation is reported as incomplete rather than presented as well-formed, and the runner never treats a missing or malformed input as grounds to skip the stop. This displacement operates at the level of the specific axis or citation whose gate input failed, not across the whole report: it does not by itself keep the outcome-table rows above from governing a different axis or citation in the same report whose gate inputs were established normally, exactly as the paragraph following the table below shows for the third and fourth rows. The first two rows are the exception in practice rather than in rule — where the question or its source cannot be established, or decomposition produces no axis, there is nothing else in that report for decomposition to have reached yet, so no other axis or citation exists to be governed by the rows above; the row still governs only what its own gate-input failure concerns, the same as the third and fourth rows, and reaches the whole report only because the failure itself is total.
...
| A reviewer or a human has put a covered, non-uncertain citation's substance in question, and the runner genuinely cannot resolve it either way — neither concluding the line's substance is wrong nor confirming it is right | Escalated, substance undetermined | Does not force the question into a new Genuinely open axis or a sixth reason value — deferred, Out of Scope item 10. States the question plainly, names who raised it, and states that the runner could not resolve it either way. This judgment is a gate input in its own right, distinct from coverage and from conformance, the same way conformance is distinct from coverage: the axis that citation settles keeps its Settled by specification coverage verdict, and the citation's conformance declaration, where determinable, is reported normally and is unaffected by the unresolved substance question | Confirm whether the cited line's substance is correct; the coverage verdict itself needs no decision |

These four rows are read in the order listed, because the second, third, and fourth presuppose what the first checks: decomposition has nothing to decompose until the question and its source are established, and a citation's conformance or substance has nothing to be undetermined about until an axis exists for the citation to settle. Where the question or its source cannot be established at all, that state is read by the first row alone, not by the second row's zero-axes case as well, even though no axis exists yet in either state.

None of these four rows produces "Not an architecture decision" or any other outcome that lets the run apply the cited lines and continue unaided. What's required from the operator differs by row: the first two require correcting the escalation's content — supplying the missing question or its source, or a redone decomposition — while the third and fourth require supplying evidence or an answer rather than deciding an axis, per each row's own Operator's next action above. None of the four is a case the runner resolves on its own.

A report can combine the third or fourth row with the settled-axis and open-axis outcomes above: a decomposition can have one axis whose citation's conformance is undetermined, or whose citation's substance question is undetermined, alongside other axes that are genuinely open (including an axis whose open reason is Governing lines conflict, if one of the conflicting citations is itself undetermined either way — the axis keeps its Genuinely open verdict and conflict content while the undetermined citation separately requests evidence or an answer), settled with a determined declaration, or subject to a departure. A single citation can even carry both an undetermined conformance and an undetermined substance judgment at once — the axis it settles keeps its Settled by specification coverage verdict either way, and the report separately requests what each applicable row needs: conformance evidence for the third row, confirmation of the line's substance for the fourth. Where a single citation triggers both at once, the report's outcome label is read from the third row — Escalated, conformance undetermined — because the third row precedes the fourth in the order established above; the fourth row's own request, confirming the line's substance, still applies to that same citation alongside it, exactly as the outcome table above composes more than one specific-reason row's content under a single outcome label. Where any of this applies, the report is incomplete rather than well-formed — the applicable row's own outcome governs — and the report states the genuinely open axes' requested decision, if any, alongside a separately reported request for whatever the third or fourth row requires; that request is never merged into the requested decision itself.
...
| A line reaches and covers the axis, the runner is not uncertain about coverage, a reviewer or human has asked whether the line's substance is right, and the runner genuinely cannot tell either way | The axis the line covers stays Settled by specification; the substance question is Escalated, substance undetermined, per the malformed-input rule (not a sixth reason — Out of Scope, item 10) | The report states the raised question plainly, who raised it, and that it could not be resolved; reported as incomplete rather than well-formed, and never treated as though no question had been asked |
...
- [ ] Every citation offered as support for the runner's behavior or for a decision, in a complete emitted escalation report, carries exactly one declaration: Conforms, Departs, or Not yet implemented. The one exception — a citation whose conformance cannot be determined — is covered by its own criterion below and does not count as a fourth declaration. A citation named only to identify a source, not offered as support, carries no declaration, per the criterion below and Business Rules above.
...
- [ ] Where a reviewer or a human has put a covered, non-uncertain citation's substance in question and the runner genuinely cannot resolve it either way — neither concluding the line's substance is wrong nor confirming it is right — the report does not force the question into a new Genuinely open axis or a sixth reason value (Out of Scope, item 10), and does not silently drop it either. It states the question plainly, names who raised it, and states that the runner could not resolve it, and the escalation is reported as incomplete rather than presented as well-formed, per the malformed-input rule. The axis that citation settles keeps its Settled by specification coverage verdict, and the citation's conformance declaration, where determinable, is reported normally; this judgment is a gate input distinct from both, the same way conformance is distinct from coverage.
...
- [ ] A citation the runner offers, as support for its behavior or for a decision, in a reply on a review thread carries the same declaration, visible in that reply — or, where its conformance cannot be determined, that reply states so plainly instead, using none of the three declarations, without opening a full `architecture_decision` escalation for that citation on that account. A citation named only to identify a source, not offered as support, carries no declaration and is outside this feature's scope, per Business Rules above.
...
- [ ] Where every axis is Settled by specification and every citation on those axes is declared Not yet implemented, the runner is not uncertain whether the cited lines fully answer the question, and the runner does not dispute any of those citations' substance, the runner proceeds with the work the cited lines specify and does not raise an `architecture_decision` escalation for that question — unless a reviewer or a human has raised an unresolved substance question about one of those citations that the runner cannot answer, in which case the malformed-input rule applies instead, per the criterion above.

### The requested decision covers only the open axes

- [ ] Where an emitted report contains at least one genuinely open axis, it states the decision requested of the human, and that decision names the genuinely open axes only. Where every axis is settled but a citation's conformance could not be determined, or a citation's substance question raised by a reviewer or human could not be resolved, the report requests no decision — since there is no open axis to decide — and instead separately states, as its own element, a request for the missing conformance evidence or a confirmation of the citation's substance, as applicable; the axis that citation was meant to settle stays Settled by specification and is never listed as genuinely open on that account.
- [ ] No axis marked Settled by specification appears in the requested decision.
- [ ] A report that contains at least one settled axis and at least one open axis asks the human about the open axis only, and the human can confirm that by reading the report alone. A separately reported conformance-evidence request or substance-confirmation request, where one applies, is additional to the requested decision and does not count as asking about a settled axis.
...
- [ ] The emitted stop still carries the exact stop condition name, the affected work item, and the concrete human action required. Where the report has at least one genuinely open axis, the required human action names those axes and, separately, any conformance evidence to supply for an axis whose citation's conformance could not be determined, or any substance confirmation to supply for a citation whose substance a reviewer or human raised and the runner could not resolve. Where the report is incomplete under the malformed-input rule and has no open axis to name — the question or its source could not be established, decomposition produced no axis, every axis is settled but a citation's conformance could not be determined, or every axis is settled but a raised citation's substance could not be resolved — the required human action instead supplies what that rule names: the missing question or its source, a redone decomposition, the missing conformance evidence, or confirmation of the citation's substance.
...
- [ ] Where a reviewer or a human has put a covered, non-uncertain citation's substance in question and the runner genuinely cannot resolve it either way, that question is not silently absorbed into "the runner does not dispute any citation's substance" above just because the runner formed no affirmative wrongness judgment, and it is not represented as a sixth Genuinely open reason (Out of Scope, item 10). It is reported per the malformed-input rule instead, exactly as an undetermined conformance is, per the criterion under *Citations carry a conformance declaration* — and the run stops under `architecture_decision` for it.
...
Both items below surfaced as review findings after the two fixes already applied for the same exhaustiveness/consistency class (per-axis scoping of the malformed-input rule at "Malformed or missing gate inputs" above, and the missing coverage-certainty exclusion on the all-Not-yet-implemented acceptance criterion under "Citations carry a conformance declaration" above). Per the human-approved terminal review plan for this item, a third finding of that same class stops further spec-stage fixing; these two are recorded here as known gaps rather than resolved in this spec, and the implementation-plan stage must force a resolution for each before implementation proceeds — either by adding the missing precision to the spec first, or by explicitly deciding and recording the intended behavior in the plan itself.

1. **Operational Visibility "Stop message" bullet does not restate the "raised" gate on the substance-undetermined malformed-input row.** Business Rule 188, the malformed-input table's fourth row (under "Malformed or missing gate inputs"), and Acceptance Criteria 401 and 427 all gate the substance-undetermined, incomplete-report treatment on a reviewer or human having *actually raised* the citation's substance in question — where nobody raised it, the citation's axis proceeds unquestioned under the settled-axis rules. The "Stop message" bullet under **Operational Visibility** describes the required human action for an incomplete report (including "confirmation of a citation's substance") without repeating that raised-question gate, so a reader of that bullet alone could conclude the incompleteness and confirmation-request treatment applies whenever a citation's substance is merely internally indeterminate to the runner, even where nobody raised it. The plan stage must either add the explicit "only where the question has actually been raised" qualifier to that bullet, or confirm and record that the composition rule already makes the gate unambiguous in context.

2. **"Citations carry a conformance declaration" acceptance criterion restricts the declaration requirement to complete reports.** The per-citation composition rule (at "Malformed or missing gate inputs" above, and its elaboration a few paragraphs below it) establishes that a citation whose own conformance or substance is undetermined does not prevent a different, determinable citation in the same report from being governed normally by the outcome-table rows, and Business Rule 189 requires every offered citation except the undetermined one to carry a declaration. The first acceptance criterion under "Citations carry a conformance declaration" instead states the declaration requirement as applying "in a complete emitted escalation report," which does not on its face cover a determinable citation sitting inside an otherwise-incomplete, mixed report. The plan stage must either restate that criterion to require the declaration per determinable citation regardless of whether the report as a whole is complete, or confirm and record that the composition rule already forces this reading without a criterion change.
