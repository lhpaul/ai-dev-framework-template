...
- [ ] Where every axis is Settled by specification and every citation on those axes is declared Not yet implemented, and the runner does not dispute any of those citations' substance, the runner proceeds with the work the cited lines specify and does not raise an `architecture_decision` escalation for that question — unless a reviewer or a human has raised an unresolved substance question about one of those citations that the runner cannot answer, in which case the malformed-input rule applies instead, per the criterion above.
