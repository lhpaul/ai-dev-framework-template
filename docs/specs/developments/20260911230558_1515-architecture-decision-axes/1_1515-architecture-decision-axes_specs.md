...
- Every citation offered anywhere in the runner's rationale carries a **conformance declaration**: Conforms, Departs, or Not yet implemented. This holds in escalation reports and in replies on review threads — anywhere the runner offers a citation as support for a decision a human or reviewer will weigh. The one exception is a citation whose conformance genuinely cannot be determined for behavior that already exists: that citation carries none of the three declarations, and the report is instead governed by the malformed-input rule in the Decision-Gate Consistency Matrix, which reports the escalation as incomplete rather than well-formed.
...
Departure handling composes with a mixed decomposition rather than being restricted to the all-settled case. Where the initial decomposition already produced at least one open axis, and the runner's behavior separately departs from a citation on one of the settled axes, the question's outcome is still read by the mixed-axis row above — "At least one axis settled, at least one open" — because that escalation is already required independent of any departure. The departs row's own content still governs how that settled axis is reported within the same escalation: the citation's declaration is Departs, stated plainly, and the runner either conforms or opens a new dispute axis, exactly as it would if every other axis had also been settled. Only where every axis in the decomposition is settled does the departure decide the outcome on its own — Departure reported, per the departs row above — because no other axis already requires escalation.
...
- [ ] Where a citation's conformance cannot be determined for behavior that already exists, the report declares neither Conforms, Departs, nor Not yet implemented for that citation. It states plainly that conformance could not be determined and why, reports the axis that citation was meant to settle as Genuinely open with the reason Coverage uncertain, and the escalation is reported as incomplete rather than presented as well-formed, per the malformed-input rule.
