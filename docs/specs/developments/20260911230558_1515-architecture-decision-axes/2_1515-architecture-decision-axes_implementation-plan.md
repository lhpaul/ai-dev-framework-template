...
| Every axis settled but some citation declares `Departs` | Not eligible for the no-escalation path | **Correction branch:** correct the behavior to conform where that is the obvious correction; the citation then conforms, so the no-escalation row above applies and the run continues (no stop). **Dispute branch:** otherwise raise the departure as its own separate axis — **Genuinely open** with the reason **Governing line disputed**, carrying a proposed amendment for what the line should become — while the covered axis stays **Settled by specification**; then stop (this branch only) | Protocol 91 stop-and-name section, canonical page | Spec: a `Departs` citation "is never presented as support for the current behavior" |
