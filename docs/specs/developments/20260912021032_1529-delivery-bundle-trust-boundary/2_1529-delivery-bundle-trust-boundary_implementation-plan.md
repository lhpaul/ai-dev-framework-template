...
file (GAP-14); and T25/T26 prove that a present, non-`str` JSON value for `ci_outcome` /
...
an uncaught `TypeError` from unguarded set membership (GAP-16). T25/T26's red capture is the
crash itself (the unmodified script raising `TypeError` and exiting non-zero for the wrong reason,
or terminating the test harness), not a clean rejection — the fix converts a crash into the
documented blocker, so "red" here means "the process aborts instead of returning the blocker",
distinct from every other non-exempt test's "the process exits 0 or names the wrong blocker".
...
9. **Add the red tests for `component-milestone-reconciliation.sh`** (T12-T18, T24, T25, and T26,
   including T15a and T15d; T15b, T15c, T13a, and T14a are the red-capture exemptions for this
   suite — T13a and T14a additionally regression-pin the pre-existing `pending`-value blocking
   behavior for the two `hub_input` flags). For T25/T26, the captured red state is the unmodified
   script raising an uncaught `TypeError` (unhashable type) instead of returning a blocker —
   capture that crash, not a clean non-zero exit with the wrong blocker name.
10. **Implement the reconciliation changes** (GAP-4, D5, D9, D6, GAP-14, GAP-16, `trust_basis`).
    Re-run green: T25/T26 now exit cleanly with `ci_outcome_invalid` / `deployment_outcome_invalid`
    instead of crashing.
