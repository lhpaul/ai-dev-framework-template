...
- [ ] `scripts/development-workflow/component-milestone-reconciliation.sh`
  - `load_json_file` (line 55): widen the existing `try`/`except` around `open()` and
    ...
    returning a documented blocker; on that exception, `fail` with a new outcome distinct from
    `invalid_json` (e.g. `{label}_unreadable`) so every caller of `--evidence-file` or
    `--delivery-manifest` gets the same stable, documented failure every other malformed-input
    case in this audit produces (GAP-17).
...
regular file (a directory) is now rejected cleanly with a stable `{label}_unreadable`-class
blocker rather than crashing `load_json_file` with an uncaught `OSError` (GAP-17). T25/T26's and
...
| T28 | `test-component-milestone-reconciliation.sh` | `inspect-component --evidence-file <path-to-a-directory>` on the hub path (a path that exists but is not a regular file, e.g. an actual directory) | `inspect-component` exits cleanly (no uncaught `OSError`/traceback, no non-zero crash exit) with a stable `{label}_unreadable`-class blocker distinct from `invalid_json` (GAP-17) |
