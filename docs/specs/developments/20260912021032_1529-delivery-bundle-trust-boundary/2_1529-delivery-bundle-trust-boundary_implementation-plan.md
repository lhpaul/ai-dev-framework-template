...
| GAP-16 | `component-milestone-reconciliation.sh` | `classify_component`'s hub path checks `ci_outcome` and `deployment_outcome` with Python set-membership (`ci not in {"passed", "not_applicable"}` at line 343 after the GAP-14 narrowing, and `deployment not in {"recorded", "not_applicable"}` at line 345), both read unguarded from `evidence.get(field)`. A `set`'s `in`/`not in` operator hashes its left operand; an evidence file supplying a non-scalar JSON value for either field (for example `ci_outcome: []` or `ci_outcome: {}`) is unhashable, so the check raises an uncaught Python `TypeError` instead of returning `blocked_component_outcome`/the documented blocker — crashing `inspect-component`/`apply-component` rather than rejecting the malformed record the way every other malformed-input case in this audit does. This is the same "narrows a set/enum without validating input shape first" defect class GAP-6 already closed for `evidence_state` (`evidence_state()`, lines 179-196, returns the raw value only when `isinstance(raw, str)`, before any closed-enum comparison runs) — `ci_outcome`/`deployment_outcome` never received that same type guard. `blocker_for_component` in `delivery-bundle-manifest.sh` (line 176-187) is **not** affected: its equivalent checks use tuple membership (`value in ("failed", "blocked")`, `component.get("ci_outcome") not in ("passed", "not_applicable", ...)`), and tuple `in` compares by equality, not by hashing, so an unhashable value such as `[]` simply compares unequal to every member and correctly falls through to `blocked_component_outcome` without raising — verified directly (`python3 -c 'print([] not in ("passed","not_applicable"))'` prints `True`, no exception) | In `classify_component`, before the `ci_outcome`/`deployment_outcome` membership checks (Table B), add the same `isinstance(value, str)` type guard `evidence_state()` already uses for `evidence_state` (GAP-6/D9): read the raw evidence value, and treat it as its own value only when it is a `str`; when the key is present with a non-string JSON value (a list, object, number, boolean, or `null` are all non-`str`), classify it as an explicit `ci_outcome_invalid` / `deployment_outcome_invalid` blocker rather than reaching the set-membership check with the raw value — an absent key still resolves to `ci_outcome_missing` / `deployment_outcome_missing` exactly as today, and a present, recognized, or unrecognized *string* still resolves exactly as GAP-14's narrowed Table B row already specifies. No `blocker_for_component` change is needed (see Defect column) |
| GAP-17 | `component-milestone-reconciliation.sh` | `load_json_file` (line 55) checks `os.path.exists(path)` before opening the file, but a path that exists and is not a regular file — for example a directory — passes that check and then makes `open(path, "r", encoding="utf-8")` (line 65) raise an `OSError` subclass (`IsADirectoryError`) that only `json.JSONDecodeError` is caught around; the exception propagates uncaught, crashing `inspect-component`/`apply-component` with a Python traceback for any caller of `--evidence-file` or `--delivery-manifest` (both routed through `load_json_file`) instead of returning the documented `evidence_not_found` / `invalid_json` outcome or any other stable blocker. Reproduced directly: `--evidence-file /tmp` (an existing directory) passes `os.path.exists`, then crashes at `open()`. This is the same "uncaught crash instead of a clean, documented failure" defect class GAP-16 already closed for a different code path in the same file (set-membership hashing) — but GAP-16's fix and this one produce differently-shaped output: GAP-16's `ci_outcome_invalid`/`deployment_outcome_invalid` are fields inside `classify_component`'s structured JSON result (with `mutation_allowed: false`), because `classify_component` has already parsed a valid record by the time it runs that check; `load_json_file` runs **before** any record exists to build a structured result from, so its failure mode is necessarily the same shape as the two blockers it already produces today, `invalid_json` and `{label}_not_found` — a bare `fail()` call, not a JSON payload | Wrap the `open()` call (not only `json.load`) in the existing `try` block, catching `OSError` alongside `json.JSONDecodeError`; on `OSError`, call `fail(f"{label}_unreadable", f"{label} file could not be read: {exc}")` with **no `payload` argument**, exactly like the existing `invalid_json` and `{label}_not_found` calls in the same function — this exits `1`, prints `ERROR_CODE={label}_unreadable message='...'` to stderr, and emits **no JSON to stdout**; it does **not** carry a `mutation_allowed` field, because `fail()` only emits JSON when called with an explicit `payload`, which none of `load_json_file`'s existing callers pass. A path that exists but cannot be opened as a regular file — a directory, a permission-denied file, or any other open-time OS failure — therefore produces the same stable, stderr-only failure shape every other `load_json_file` rejection already produces, rather than an uncaught traceback |
...
or is not a JSON string under `component_release_routed` routing; and, in the reverse direction,
when `selected_product_repo_key` is anything other than exactly JSON `null` under
`single_repo_release` routing.** Both directions of this field's guard are type checks on the
parsed JSON, not string-emptiness checks after the `jq -r '... // ""'` stringify pattern: that
pattern only substitutes `""` for `null` or `false`, so a JSON array or object (e.g. `[]`) is
truthy in `jq`, passes through unchanged, and is then rendered by `jq -r` as its JSON text
form — the non-empty bash string `"[]"` — which would otherwise satisfy a plain
"resolves non-empty" check despite never being a legitimate string identifier (T6i pins this
for the forward direction; T6f already pinned the equivalent ambiguity for the reverse
direction's `""` case). The `routing_outcome` closed-enum precondition must run before
...
- [ ] `scripts/development-workflow/component-release-evidence.sh`
  ...
  - In the same block, refuse emission when `selected_product_repo_key` resolves empty **or is
    not a JSON string** while `routing_outcome` is `component_release_routed`; exit `1` with
    `target binding is missing required identity field: selected_product_repo_key`. Implement
    this forward guard the same way as the reverse guard below — as a type check on the parsed
    JSON (`jq -e '(.selected_product_repo_key | type) == "string" and (.selected_product_repo_key
    | length) > 0'`), not as a reuse of the `jq -r '.selected_product_repo_key // ""'` pattern
    already used at line 168 for the branch-pattern check. That pattern only substitutes `""`
    for `null` or `false`; a JSON array or object such as `[]` is truthy in `jq`, so `// ""`
    passes it through unchanged, and `jq -r` then renders it as its JSON text form — the
    non-empty bash string `"[]"` — which would satisfy a plain "resolves non-empty" check despite
    never being a legitimate identifier value. The type check closes that gap.
...
- [ ] `scripts/development-workflow/delivery-bundle-manifest.sh`
  ...
  - `blocker_for_component`: narrow the `ci_outcome` acceptance check (line 184) from
    ...
    non-blocking, `passed`-equivalent outcome (GAP-14). No type guard is needed here for GAP-16:
    `blocker_for_component`'s `ci_outcome`/`deployment_outcome` checks (lines 176-187) use tuple
    membership throughout (`value in (None, "")`, `value in ("failed", "blocked")`, the final
    `not in ("passed", "not_applicable")` / `not in ("recorded", "not_applicable")`), and tuple
    `in`/`not in` compares by equality rather than hashing, so a non-`str` value such as `[]`
    already falls through this loop without raising and reaches the trailing
    `blocked_component_outcome` return — verified directly against the running code, not assumed.
- [ ] `scripts/development-workflow/component-milestone-reconciliation.sh`
  - `load_json_file` (line 55): widen the existing `try`/`except` around `open()` and
    `json.load()` to also catch `OSError`, not only `json.JSONDecodeError`. A path that passes
    `os.path.exists` but is not an openable regular file — a directory, a permission-denied
    file, or any other open-time OS failure — currently raises an uncaught `OSError` subclass
    (e.g. `IsADirectoryError`) that crashes the process with a Python traceback rather than
    returning a documented blocker; on that exception, call `fail(f"{label}_unreadable",
    f"{label} file could not be read: {exc}")` with **no `payload` argument**, matching exactly
    how the existing `invalid_json` and `{label}_not_found` calls in this same function are
    written (neither passes `payload` either). This exits `1`, prints
    `ERROR_CODE={label}_unreadable message='...'` to stderr, and emits **no JSON to stdout** — it
    does not carry `mutation_allowed` or any other structured field, because `fail()` only
    prints JSON when called with an explicit `payload`. This is a different output shape from
    GAP-16's `ci_outcome_invalid`/`deployment_outcome_invalid`, which are fields inside
    `classify_component`'s already-structured JSON result (`mutation_allowed: false`):
    `load_json_file` runs before any record has been parsed, so it has no structured result to
    attach a field to, and must fail the same stderr-only way its sibling `load_json_file`
    rejections already do (GAP-17).
  ...
  - `classify_component`, hub path: type-guard `ci_outcome` and `deployment_outcome` before the
    set-membership checks above (lines 343 and 345), mirroring the `isinstance(raw, str)` guard
    `evidence_state()` already applies (GAP-6/D9): read each raw value with `evidence.get(field)`
    and use it in the membership check only when `isinstance(value, str)` is true; when the key is
    present with a non-`str` JSON value (a list, object, number, boolean, or `null`), do not pass
    it to the `{"passed", "not_applicable"}` / `{"recorded", "not_applicable"}` set-membership
    test at all — append `ci_outcome_invalid` / `deployment_outcome_invalid` directly instead. This
    closes the gap where a set's `in`/`not in` operator hashes its left operand: an unguarded,
    caller-fabricated `ci_outcome: []` or `deployment_outcome: {}` raises an uncaught Python
    `TypeError` (unhashable type) instead of the documented blocker, crashing `inspect-component`
    rather than rejecting the malformed record (GAP-16). An absent key is unaffected and still
    resolves to `ci_outcome_missing` / `deployment_outcome_missing`; a present, recognized, or
    unrecognized `str` value still resolves exactly as GAP-14's narrowed check already specifies.
...
Every other numbered test must show a captured red state: T1-T28 (plus T6i) other than T6b,
T10b, T10c, T10d, T13a, T14a, T15b, T15c, and T22, including T5b, T6a, T6c, T6d, T6e, T6f, T6g,
T6h, T6i, T10a, T15a, T15d, T20b, T20c, T20d, T20e, T20f, T23, T24, T25, T26, T27, and T28, all target behavior
this plan introduces — T4's red capture is taken at step 6a rather than step 5, for the reason
given above; T27's red state, unlike T4's, is captured cleanly at step 5, before D1 lands: the
...
*present* but non-string `evidence_state` value (e.g. JSON `null`), which previously fell through
silently to the same synthesis path as an absent key; T6h proves the `routing_outcome` closed-enum
precondition rejects a third, unrecognized value on its own, before either
`selected_product_repo_key` conditional branch is even consulted; T23 and T24 prove that
`ci_outcome: "skipped"` — a value `component-release-evidence.sh`'s own `--ci-outcome` enum can
never emit — is now rejected by both consumers that read `ci_outcome` directly from the evidence
file (GAP-14); T25/T26 prove that a present, non-`str` JSON value for `ci_outcome` /
`deployment_outcome` on the hub path (for example a JSON array) is now rejected cleanly with
`ci_outcome_invalid` / `deployment_outcome_invalid` rather than crashing `classify_component` with
an uncaught `TypeError` from unguarded set membership (GAP-16); and T28 proves that
`--evidence-file`/`--delivery-manifest` pointed at a path that exists but is not an openable
regular file (a directory) is now rejected cleanly — exit `1`, `ERROR_CODE={label}_unreadable`
printed to stderr, no JSON on stdout, the same stderr-only shape `invalid_json` and
`{label}_not_found` already use — rather than crashing `load_json_file` with an uncaught
`OSError` (GAP-17); this is a different, simpler shape than GAP-16's `mutation_allowed: false`
JSON result, because `load_json_file` runs before any record exists to attach a structured
field to. T25/T26's and
T28's red captures are each the crash itself (the unmodified script raising the uncaught
exception and exiting non-zero for the wrong reason, or terminating the test harness), not a
clean rejection — the fix converts a crash into the documented blocker, so "red" here means "the
process aborts instead of returning the blocker", distinct from every other non-exempt test's
"the process exits 0 or names the wrong blocker".
...
| T6i | `test-component-release-evidence.sh` | target binding with `routing_outcome: "component_release_routed"` and `selected_product_repo_key` set to a **malformed JSON type** — a JSON array `[]` rather than a string — with `release_branch_pattern` omitted (empty) for the same structural reason as T6a: this proves the forward guard is a JSON-type check (non-empty string), not a string-emptiness check after the `jq -r '... // ""'` stringify pattern, which would otherwise render `[]` as the non-empty bash string `"[]"` and let it pass a plain "resolves non-empty" check | exit 1, `target binding is missing required identity field: selected_product_repo_key`; no record written |
...
| T19 | `test-prepare-release-tracker-cleanup.sh` | evidence whose `target_binding.contract_revision` is `""`, generated the same branch-bound way as T20c's baseline (see T20c's note): `contract_revision` is compared last among `validate_component_release_cleanup`'s six ordered `compare_component_field` calls (`routing_outcome`, `selected_product_repo_key`, `canonical_repository_identity`, `artifact_owners`, `release_correlation_key`, `contract_revision`), so a fixture whose `target_binding.release_correlation_key` was not resolved against the same real branch cleanup's re-resolution passes as `--release-branch` would fail one check earlier, on `release_correlation_key`, for the wrong reason | exit 1, `missing required identity field: contract_revision`; no branch deletion |
| T20 | `test-prepare-release-tracker-cleanup.sh` | evidence whose `target_binding.canonical_repository_identity` is `""`, otherwise a valid, branch-bound record (see T19's note) | exit 1, `Component release evidence is missing required identity field: canonical_repository_identity` — assert this exact GAP-8 diagnostic, not the pre-existing `Component release evidence mismatch for .canonical_repository_identity: current=... evidence=...` message: the freshly resolved target's real (non-empty) `canonical_repository_identity` already disagrees with the evidence's edited `""`, so the pre-existing `compare_component_field` check (unmodified, without GAP-8) would *also* exit 1 naming this field, for the wrong reason — a red capture against an assertion that does not distinguish the two messages would already be green before GAP-8 exists, defeating the red-before-green proof this test exists to provide |
| T20b | `test-prepare-release-tracker-cleanup.sh` | evidence whose `target_binding.release_correlation_key` is `""`, otherwise a valid, branch-bound record | exit 1, `Component release evidence is missing required identity field: release_correlation_key` — same exact-diagnostic requirement and same-reason rationale as T20; no branch deletion |
| T20c | `test-prepare-release-tracker-cleanup.sh` | evidence generated by re-invoking `component-release-target.sh` directly against a real `--release-branch` (**not** `setup-component-release-fixture.sh`'s own bundled `component-target.json`, whose internal `component-release-target.sh` call never passes `--release-branch` and so never binds one — `component-release-evidence.sh` copies `target_binding.release_correlation_key` from the target file verbatim rather than recomputing it, so the target itself, not just the evidence render, must be resolved against the real branch for `target_binding.release_correlation_key` to be the genuine per-attempt value that branch produces, exactly as the comment above cleanup's `evidence_branch` read describes), then the fixture's top-level `release_branch` is overwritten to `""` (or the key removed) **without** changing `target_binding.release_correlation_key` — this is deliberate: an empty `evidence_branch` means cleanup's target re-resolution omits `--release-branch`, so the freshly resolved target computes a *different* per-attempt `release_correlation_key` than the one already recorded in `target_binding`, and this fixture is the one that would surface that disagreement as a `compare_component_field '.release_correlation_key'` mismatch instead of the intended rejection if the new guard were implemented anywhere other than immediately after `evidence_branch` is read (a fixture that instead used a fixed, attempt-independent key would not expose that ordering defect) — with a positional release argument supplied that does not match any real branch | exit 1, `missing required field: release_branch`; no branch deletion; the message must **not** be a `release_correlation_key` mismatch, which would indicate the guard ran too late |
| T20d | `test-prepare-release-tracker-cleanup.sh` | evidence whose `target_binding.routing_outcome` is `""`, otherwise a valid, branch-bound record | exit 1, `Component release evidence is missing required identity field: routing_outcome` — same exact-diagnostic requirement and same-reason rationale as T20 (the freshly resolved target's real `routing_outcome` disagrees with the evidence's edited `""` regardless of GAP-8); no branch deletion |
| T20e | `test-prepare-release-tracker-cleanup.sh` | evidence whose `target_binding.selected_product_repo_key` is `""` (or the key absent), otherwise a valid, branch-bound record | exit 1, `Component release evidence is missing required identity field: selected_product_repo_key` — same exact-diagnostic requirement and same-reason rationale as T20; no branch deletion |
| T20f | `test-prepare-release-tracker-cleanup.sh` | evidence with exactly one `target_binding.artifact_owners` sub-field set to `""`, other five populated, otherwise a valid, branch-bound record — run once per sub-field (`release`, `ci`, `github_release`, `deployment`, `cleanup`, `tracker`; six separate red-then-green captures under this one test identifier, T20f, for the same reason as T6d) | each of the six runs: exit 1, `Component release evidence is missing required identity field: artifact_owners` — same exact-diagnostic requirement and same-reason rationale as T20 (the freshly resolved target's real `artifact_owners` object disagrees with the evidence's edited copy regardless of GAP-8); no branch deletion |
| T20g | `test-prepare-release-tracker-cleanup.sh` | evidence whose top-level `cleanup_outcome` is `""` (a second variant with the key removed entirely) — otherwise a valid, ready-to-clean-up record generated the same branch-bound way as T20c's baseline (see T20c's note): the `cleanup_outcome` check runs after all six ordered `compare_component_field` identity comparisons succeed, so a `target_binding.release_correlation_key` that was not resolved against the same real branch cleanup's re-resolution passes as `--release-branch` would fail on that identity comparison before this test's guard is ever reached | exit 1, `Component release evidence is missing required field: cleanup_outcome`; no branch deletion; this must **not** silently fall through to a real cleanup run (GAP-15) |
...
| T28 | `test-component-milestone-reconciliation.sh` | `inspect-component --evidence-file <path-to-a-directory>` on the hub path (a path that exists but is not a regular file, e.g. an actual directory) | exit `1`, `ERROR_CODE=evidence_unreadable message='...'` printed to stderr, **no JSON on stdout** — the same stderr-only shape as the pre-existing `invalid_json` and `evidence_not_found` failures in this function, not a `classify_component`-style structured result with `mutation_allowed` (GAP-17) |
...
```json
{
  "residual_groups": [
    ...
    {
      "summary": "Spec #1357 documents ci_outcome: skipped as valid for a ci_policy: none product, but GAP-14 narrows both consumers to reject it because no ci_policy-equivalent field exists to condition admission; deferred to issue #1742 (RESIDUAL-7)",
      "remaining_count": 1,
      "disposition": "out_of_scope"
    },
    {
    ...
    {
      "summary": "Spec #1357 requires an explicit human action for a deferred hub_tracker_reconciliation_outcome to count toward finalization, but no field or mechanism for that action exists; both delivery-bundle-manifest.sh's blocker_for_component and component-milestone-reconciliation.sh's component_is_released accept deferred identically to complete with no check at all; deferred to issue #1754 (RESIDUAL-9)",
      "remaining_count": 2,
      "disposition": "out_of_scope"
    },
    {
...
| Routed-malformed-type-key target binding | `component_release_target.v1` with `routing_outcome: "component_release_routed"`, `selected_product_repo_key: []` (a JSON array, not a string), and `release_branch_pattern` omitted (empty) for the same structural reason as the routed-null-key fixture above — proves the forward-direction guard rejects a non-string JSON type exactly like an empty value, not merely a value that stringifies non-empty via the `jq -r '... // ""'` pattern | New temp fixture inside `tests/test-component-release-evidence.sh` (T6i) |
| Release-branch-empty evidence with a real per-attempt correlation key | `component_release_evidence.v1` record produced by re-invoking `component-release-target.sh` directly against a real `--release-branch` (**not** `setup-component-release-fixture.sh`'s own bundled `component-target.json`, whose internal call never binds one — `component-release-evidence.sh` copies `target_binding.release_correlation_key` verbatim rather than recomputing it, so `target_binding.release_correlation_key` is genuinely computed from that attempt branch only if the target itself was resolved against it, not a fixed/contract-level value), then mutated to set the top-level `release_branch` to `""` (or remove the key) while leaving `target_binding.release_correlation_key` unchanged — otherwise a valid producer-shaped record. Using a fixed, attempt-independent correlation key here would let this fixture pass the new guard for the wrong reason: it would never exercise the disagreement between the evidence's recorded per-attempt key and the key cleanup's target re-resolution computes when `--release-branch` is omitted, which is exactly the case that exposes whether the guard runs before or after the six `compare_component_field` calls | New temp fixture inside `tests/test-prepare-release-tracker-cleanup.sh` (T20c) |
| Empty-identity evidence (cleanup) | `component_release_evidence.v1` records, each generated the same branch-bound way as the T20c fixture above (a plain `setup-component-release-fixture.sh` target/evidence pair is branchless and would fail one of the six ordered `compare_component_field` identity comparisons for the wrong reason before reaching the field a given test edits — see T19's and T20g's notes), whose `target_binding.contract_revision` is `""`; a second variant with `target_binding.release_correlation_key: ""`; a third with `target_binding.canonical_repository_identity: ""`; a fourth with `target_binding.routing_outcome: ""`; a fifth with `target_binding.selected_product_repo_key: ""` (or the key absent); a sixth family of six variants, each with exactly one `target_binding.artifact_owners` sub-field (`release`, `ci`, `github_release`, `deployment`, `cleanup`, `tracker`) set to `""` and the other five populated — otherwise a valid producer-shaped record | New temp fixtures inside `tests/test-prepare-release-tracker-cleanup.sh` (T19, T20, T20b, T20d, T20e, T20f — T20f exercised once per `artifact_owners` sub-field) |
| Empty/absent `cleanup_outcome` evidence (cleanup) | `component_release_evidence.v1` record generated the same branch-bound way as the T20c fixture above (see T20g's note: the `cleanup_outcome` check runs after all six identity comparisons, so a branchless fixture would fail one of those first) with a valid, non-empty `target_binding` but a top-level `cleanup_outcome` of `""`; a second variant with the `cleanup_outcome` key removed entirely — otherwise a valid, ready-to-clean-up record | New temp fixtures inside `tests/test-prepare-release-tracker-cleanup.sh` (T20g) |
...
| Non-`str` `ci_outcome`/`deployment_outcome` evidence | Producer-shaped, otherwise-valid records on the hub path with `ci_outcome: []` (a JSON array) or `deployment_outcome: []` respectively — any non-`str` JSON value reproduces the defect identically, so a JSON array is sufficient and does not need to be paired with an object/number/boolean variant | `tests/test-component-milestone-reconciliation.sh` (T25 sets `ci_outcome: []`; T26 sets `deployment_outcome: []`) |
...
- [ ] `docs/workflow/development-workflow/component-release-evidence-contract.md` — **create.**
      ...
      carrying RESIDUAL-1 through RESIDUAL-9 verbatim with their rationales, including
      RESIDUAL-7's pointer to issue #1742 for the spec #1357 `ci_outcome: skipped`
      discrepancy, RESIDUAL-8's pointer to issue #1747 for the
      `release_branch`/`hub_tracker_ref` cross-consumer enforcement question, and RESIDUAL-9's
      pointer to issue #1754 for the spec #1357 `deferred` human-action discrepancy; (7) the
...
6. **Implement the producer changes in two sub-steps**, because T4 needs an intermediate
   ...
   1. **Step 6a**: implement D1 (`--component-version` argument parsing and emission) and D4
      ...
      suite: T1, T2, T5, T5b, T6, T6a-T6i, and T27 go green (T27 goes green here because no
...
9. **Add the red tests for `component-milestone-reconciliation.sh`** (T12-T18, T24, T25, T26, and
   T28, including T15a and T15d; T15b, T15c, T13a, and T14a are the red-capture exemptions for
   this suite — T13a and T14a additionally regression-pin the pre-existing `pending`-value
   blocking behavior for the two `hub_input` flags). For T25/T26/T28, the captured red state is
   the unmodified script raising an uncaught exception (`TypeError` for T25/T26, `OSError` for
   T28) instead of returning a blocker — capture that crash, not a clean non-zero exit with the
   wrong blocker name.
10. **Implement the reconciliation changes** (GAP-4, D5, D9, D6, GAP-14, GAP-16, GAP-17,
    `trust_basis`). Re-run green: T25/T26 now exit cleanly with `ci_outcome_invalid` /
    `deployment_outcome_invalid` instead of crashing, and T28 now exits cleanly with a stable
    `{label}_unreadable`-class blocker instead of an uncaught `OSError`.
...
| Regression tests assert rejection for each fabricated-value case, each confirmed to fail before its fix | Testing Strategy AC-4 discipline; Implementation Order steps 5, 6a/6b, 7, 9, 11 capture red before green | T1-T28 (plus T6f, T6g, T6h, T6i, T10a, T13a, T14a, T20g) other than T6b, T10b, T10c, T10d, T13a, T14a, T15b, T15c, and T22 (including T5b, T6a, T6c, T6d, T6e, T6f, T6g, T6h, T6i, T10a, T15a, T15d, T20b, T20c, T20d, T20e, T20f, T20g, T23, T24, T25, T26, T27, and T28) with captured red-then-green output on the implementation PR — T6d and T20f each capture six pairs, one per `artifact_owners` sub-field; T4's red capture is taken at step 6a (after D1, before D3) rather than step 5, because `--component-version` is not a recognized flag until D1 lands (see the T4 row in Fabricated-value rejection cases); T27's red capture is taken at step 5, before D1 lands, for the same reason but with the opposite polarity — see the T27 row in Fabricated-value rejection cases; T25/T26's and T28's red captures are each the unmodified script's uncaught exception crash rather than a clean wrong-blocker exit (see the exemption paragraph above the Fabricated-value rejection cases table); T6b, T10b, T10c, T10d, T13a, T14a, T15b, T15c, and T22 are the nine declared red-capture exemptions and are recorded green-before and green-after |
| The producer's emitted-field contract is documented, so a future consumer can tell required from optional without reading the producer | New contract document section 2 (the 16-field table, each field carrying exactly one class) and section 4 (never-emitted fields); no field contradicts the class it carries, so the class alone yields the duty | Document review; T2 pins the `null` emission of an unsupplied conditional field, T6a/T6e/T6f/T6g pin the two directions of the `producer_required_nullable` boundary (`null` if and only if `single_repo_release` routing — T6a pins that `component_release_routed` requires non-`null`; T6e, T6f, and T6g together pin that `single_repo_release` requires *exactly* `null`, rejecting a non-empty string (T6e), an empty string (T6f), and a missing key (T6g) alike) while T6b pins the `null` passthrough this plan leaves unchanged, T6c/T6d pin the `producer_required` promise for `routing_outcome` and `artifact_owners`, T6h pins that the `producer_required_nullable` boundary's "if and only if" claim holds even when `routing_outcome` carries a third, unrecognized value, and T6i pins that the `component_release_routed` direction rejects a non-string JSON value (an array) exactly like an empty one, not merely a value that stringifies non-empty |
