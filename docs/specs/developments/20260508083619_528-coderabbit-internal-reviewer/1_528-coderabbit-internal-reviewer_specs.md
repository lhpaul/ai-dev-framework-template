...
- **BR-2 — Runtime-availability check for `coderabbit`**: The availability check for `coderabbit` is determined by whether the CodeRabbit GitHub App is installed and accessible on the repository (i.e., can the runner post trigger comments and observe `coderabbitai[bot]` responses via `gh`). Unlike `codex`, which is determined by runner context alone, `coderabbit` availability depends on the repository's GitHub App configuration. The check is performed by inspecting whether the CodeRabbit GitHub App is accessible (e.g., checking for the presence of `coderabbitai[bot]` activity on the PR or confirming App installation via `gh api`). The exact availability-detection mechanism is an implementation decision for the plan stage; the spec requires only that it be deterministic, fast (no multi-minute wait), and documented.
