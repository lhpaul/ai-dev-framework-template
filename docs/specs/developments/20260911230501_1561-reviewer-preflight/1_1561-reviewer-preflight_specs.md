...
Deferral notes for B-1's and B-5's partial-coverage items, B-6, and B-7 are recorded in `## Out of Scope (MVP)` entries 1, 2, 3, and 11.
...
| `prerequisite-failed`  | Prerequisite not met   | The run's set of targeted base branches was empty, unresolved, or malformed, or the set of lifecycle stages it will exercise or the pull-request state for any one of those stages was unresolved or malformed (a stage set legitimately empty by deferral to a later single-item preflight, per Triggers, is not this failure) — a failure of an input the run must resolve before the preflight runs, not a per-platform verdict. Decided before any per-platform verdict is computed; the run does not dispatch. |
...
| The lifecycle stage(s) this run will exercise, a distinct input from the pull-request state below | For each item the run will dispatch, every lifecycle stage bucket (for example `on_draft`, `on_ready`) that item's own execution will exercise before its own next preflight, unioned across every item — never one global stage set applied indiscriminately to every item, and never the whole batch's own remaining lifecycle when a per-item preflight will run again for that item before its own next mutation | Selects, from the shared workflow reviewer configuration, which platforms are expected to review at each stage each item will exercise (not only the first) |
...
**The lifecycle-stage input carries the same per-item association the base-branch input already does, never a bare Cartesian product of every targeted base against every exercised stage.** A run that dispatches several items can have one item already at `on_ready` against one base while a fresh item still needs `on_draft` against a different base; the gate evaluates each item's own stage-excluded and base-branch checks only against that item's own base and that item's own stage(s), from the item/base/stage association the run's own resolved dispatch plan already carries, not by combining every targeted base with every stage any item anywhere in the run will exercise. A base that no item in the run will reach at a given stage places no stage-excluded expectation on a platform for that base/stage pair, exactly as a base no item targets places no base-branch expectation at all (see the base-branch association rule above). This does not change the whole-run Blocked stop stated above: a platform still found Cannot review for the one item/base/stage combination it actually applies to still blocks the whole run, the same as a base-branch gap does; only the *evaluation* is scoped per association, not the *outcome*.
...
**An empty lifecycle-stage set is not a trigger failure either, under one specific condition.** Per the Gate-inputs stage row and the item/base/stage association rule above, a batch-level preflight's own stage-set input is every stage any dispatched item will exercise *before that item's own next preflight*. When every item the batch will dispatch is itself gated by its own later single-item preflight before that item's own first mutation — the ordinary case for a batch of fresh items — that interval is empty for every item, so the batch-level input correctly, deliberately resolves to the empty set: this batch-level preflight is not itself responsible for any item's stage-excluded verdict, because each item's own later preflight already is. That is a legitimate division of responsibility, resolved, not unresolved, and the batch-level preflight produces Passed (or whatever the base-branch and other checks otherwise determine) rather than Prerequisite not met on the stage input's account alone. This exemption is narrow: it applies only to the stage-set input, only at the level whose own responsibility is legitimately deferred this way, and never to the base-branch set (which is never deferred to a later preflight) or to a stage set that could not be determined at all — see below.

An empty, unresolved, or malformed set of targeted base branches is not the same case as the reviewer-list or stage-set exemptions above, and does not produce Passed by the same reasoning. The set of base branches this run targets, the lifecycle stage(s) this run will exercise (the distinct Gate-inputs input of that name — which may legitimately resolve to empty per the exemption above, but must still be resolved, not merely undetermined), and the pull-request state a platform will actually see at its own dispatch (its listed stage, from that input, as adjusted by any existing workflow behavior that changes it before dispatch) are all required inputs the run resolves before the preflight runs, not inputs the preflight resolves for itself; a run that reaches the preflight without a non-empty, resolved base-branch set, without a resolved set of lifecycle stage(s) (empty-by-deferral counts as resolved; genuinely undetermined does not), or without a resolved pull-request state for each stage that set does contain, has already failed that prerequisite. The preflight does not run against such an input and does not report Passed on its account — an empty base-branch set would otherwise vacuously satisfy every base-branch coverage check without reading any platform's own configuration at all, and an unresolved stage or pull-request state would let stage-excluded checking be silently skipped for every platform, which is exactly the false assurance this feature exists to remove. Any of these three failures is reported as the same distinct run outcome, Prerequisite not met — never folded into any of the three per-platform verdicts, and never reported as Passed, Passed some unverified, or Blocked.

### Allowed outcomes and required next actions

| Outcome                 | When it is produced                                                | Required next action                                                                     |
| ----------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| Prerequisite not met    | The run's set of targeted base branches is empty, unresolved, or malformed, or its set of lifecycle stages or the pull-request state for any one of those stages is unresolved or malformed (a stage set legitimately empty by deferral, per Triggers, is not this case) | Nothing is dispatched. No per-platform verdict is computed. The run resolves the failed input and re-runs |
...
**For every configured value that is a supported platform, a Cannot review verdict names every one of the three remaining disagreement reasons that applies, not one.** The three checks — automatic review turned off, a listed stage its own configuration excludes, and an uncovered base branch — are independent of each other, and a platform can fail more than one at once: a platform can simultaneously have automatic review off, be listed for a stage its own configuration declines, and cover a set of bases that omits one this run expects it to review against. The gate runs all applicable checks for every such platform in the resolved list and reports the complete set of disagreement reasons that apply on that platform's single Cannot review verdict. No check is skipped because another already produced Cannot review, and no precedence between the reasons is defined, because a report that names every applicable reason needs none — precedence would only be necessary to pick one reason to the exclusion of others, and this gate never does that. Across all four disagreement reasons, exactly one of two shapes applies to any given platform: Not a supported reviewer alone, or some non-empty subset of the other three.
...
- [ ] With the run's set of lifecycle stages unresolved or malformed (an empty set that legitimately reflects every dispatched item's own later single-item preflight, per the exemption in Triggers, does not count), or with the pull-request state for any one stage in a resolved set empty, unresolved, or malformed, the preflight produces Prerequisite not met rather than Passed, and does not silently skip the stage-excluded check for any platform at any stage by treating the unresolved stage or state as agreement.
