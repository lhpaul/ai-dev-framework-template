...
- [ ] When a single-item run resumes an existing workflow branch or pull request instead of starting from a state where no branch exists yet, the preflight uses whichever path matches what already exists: the existing-pull-request path, reading that pull request's own branch, when a pull request already exists; or a preflight checked against that existing branch's own state when a branch exists without a pull request yet. The "before any branch exists" pre-dispatch path applies only when resuming finds neither a branch nor a pull request already in place — once a branch exists, checking a base branch a different, earlier run already covered misses a disabling change already present on the branch the reviewer will actually read.
