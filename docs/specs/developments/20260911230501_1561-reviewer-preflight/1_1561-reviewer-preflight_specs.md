...
**Prerequisite not met is decided first, before any per-platform verdict.** The gate cannot compute base-branch coverage — or read any per-base copy of the shared reviewer configuration — without a resolved base-branch set to evaluate against, and it cannot compute the stage-excluded check without both a resolved dispatching stage and a resolved pull-request state (the stage as adjusted by any existing workflow behavior that changes it before dispatch) to compare a platform's listing against, so it checks all three prerequisites — the base-branch set, the dispatching stage, and the pull-request state — before computing any platform's verdict. Where any of the three fails, the gate reports Prerequisite not met and stops there; no platform is classified Can review, Cannot review, or Undetermined for that run, because none of those verdicts was actually computed — an unresolved stage or pull-request state is never treated as silently satisfying the stage-excluded check for every platform. Where all three prerequisites are met, the gate proceeds to per-platform verdicts as stated below.
...
**Value-not-supported is checked first and stands alone.** Before any of the other checks, the gate confirms the configured value is one of the reviewer platforms this workflow supports. When it is not, no cross-check against its own configuration is even possible — there is no defined platform to read a configuration for — so the platform is Cannot review with only the reason Not a supported reviewer, and none of the other three checks is attempted for it.
