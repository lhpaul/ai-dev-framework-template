...
| ------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `operable`          | Can review           | The surfaces agree for this platform: it will review pull requests of the kind this run opens, at the stage it is listed for, against every base this run expects it to review. |
...
- **Prerequisite failure report**: on Prerequisite not met, the report says plainly that the run's targeted base-branch set was empty, unresolved, or malformed, and that no platform was cross-checked on this run — distinct from Passed, whose report states that every platform in the resolved list was actually checked.
...
An empty, unresolved, or malformed set of targeted base branches is not the same case and does not produce Passed by the same reasoning. The set of base branches this run targets is a required input the run resolves before the preflight runs, not one the preflight resolves for itself; a run that reaches the preflight without a non-empty, resolved base-branch set has already failed that prerequisite. The preflight does not run against such an input and does not report Passed on its account — an empty base-branch set would otherwise vacuously satisfy every base-branch coverage check without reading any platform's own configuration at all, which is exactly the false assurance this feature exists to remove. This failure is reported as a prerequisite failure, not folded into any of the three per-platform verdicts or the three run outcomes.

### Allowed outcomes and required next actions

| Outcome                 | When it is produced                                                | Required next action                                                                     |
| ----------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| Prerequisite not met    | The run's set of targeted base branches is empty, unresolved, or malformed | Nothing is dispatched. No per-platform verdict is computed. The run resolves its base-branch set and re-runs |
...
**Prerequisite not met is decided first, before any per-platform verdict.** The gate cannot compute base-branch coverage — or read any per-base copy of the shared reviewer configuration — without a resolved base-branch set to evaluate against, so it checks that prerequisite before computing any platform's verdict. Where the prerequisite fails, the gate reports Prerequisite not met and stops there; no platform is classified Can review, Cannot review, or Undetermined for that run, because none of those verdicts was actually computed. Where the prerequisite is met, the gate proceeds to per-platform verdicts as stated below.
