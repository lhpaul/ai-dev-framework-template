...
| --- | --- | --- |
| B-1 | Verify before dispatch that configured reviewer platforms can actually review this repository | Partially covered: AC group "A disagreement is reported before dispatch, naming the surface"; AC group "The preflight is reachable where it is needed" verify configuration coherence. Live installation/service-availability verification — whether a platform will actually review, beyond what its configuration says — is deferred under **Out of Scope entry 11**, the same partial-coverage treatment as B-5 |
...
| B-4 | Fail loudly before a batch starts; the cost of not having the check is an entire run | Partially covered: Outcome `blocked` / Blocked; AC "On Blocked, no item is dispatched"; AC "reported before any item-level output" enforce this per item — each item's own preflight fails loudly before that item's own first mutation. A distinct batch-level preflight that aggregates every dispatched item's remaining stages into one check ahead of a batch's first dispatch is deferred under **Out of Scope entry 12** |
...
Deferral notes for B-1's, B-4's, and B-5's partial-coverage items, B-6, and B-7 are recorded in `## Out of Scope (MVP)` entries 1, 2, 3, 11, and 12.
...
**Preconditions**: The run has resolved its base branch and a resolved reviewer configuration. Every reviewer platform named for this item's stages can review pull requests of the kind this run will open, against the base this item targets.
...
3. For each platform in the resolved list, it reads whatever the platform's own configuration says about whether it will review, and compares that against the stage the platform is listed for and the base branch this item targets.
...
- What the surfaces say that is incompatible, stated as the contradiction rather than as two isolated facts: which stage the platform is listed for against which stages it will review, or which base this item targets, against which bases it covers.
- At least one action that would resolve the disagreement.

**Actions available**:

- Change the platform's own configuration so it covers this run's stage and the base this item targets.
...
- This rule is why the preflight distinguishes what it checked from what it could not check. Before a branch exists, only the base branch's copies can be read.
...
- The preflight's verdicts are determined fresh on each run. No verdict from an earlier run is an input to a later one: a record of a past run's outcome is history, not state the preflight reads or updates. Repairing the surface behind one of a platform's disagreement reasons removes that specific reason on the second run with no further action, and breaking it between two runs reintroduces it the same way; that platform's own overall verdict is then whichever the multi-reason combination rule produces from whatever reasons remain, which is Can review only when none remain.
...
- Base-branch coverage is evaluated against this item's own single targeted base — the base before a branch exists, or the base this item's own execution already resolved to target once a branch or pull request exists (per the resume rules above and below). A platform whose own configuration does not cover that base is Cannot review with the reason Base branch not covered. The Decision-Gate Consistency Matrix states this canonically.
- The preflight reports what it checked against. A preflight performed before any branch exists is checked against the base branch the run targets, and names it; it does not claim to have verified branches that do not yet exist.
...
- A preflight performed against an existing pull request has a single, well-defined base-branch input: that pull request's own target base, read from the pull request itself, not from any multi-base run context — a run that dispatched the item may no longer exist by the time an existing pull request is checked. Only the shared workflow reviewer configuration and each platform's own configuration switch source, to the copy on that pull request's own branch instead of the copy on a targeted base branch. Its dispatching-stage input is, like the branch-only path's, every lifecycle stage this item's remaining execution will still exercise from this resume point forward — not only the pull request's current stage — because a resumed run can continue through a later stage's dispatch (for example converting the pull request to ready and dispatching `review.on_ready.github`) within this same continuous run without a further preflight in between; the pull-request state input is resolved per stage from that set, adjusted by any existing workflow behavior, exactly as the branch-only path's is. The machine-local override does not switch source: it is resolved from the machine running the preflight exactly as it is for a pre-dispatch preflight, never from the pull request's branch, and it keeps the same precedence over the shared list read from that branch that it has everywhere else in this spec.
- A preflight performed against an existing workflow branch that has no pull request yet switches configuration source the same way an existing-pull-request preflight does: the shared workflow reviewer configuration and each platform's own configuration are read from that branch's own copies, not from any targeted base's copy, because that branch is what the reviewer will actually read once its pull request exists. Its base-branch input is not the branch itself — a branch has no base-branch coverage of its own to check, and treating its name as a base would falsely evaluate every platform's coverage against a base no platform's configuration is expected to name — but the single base this item's own execution already resolved to target, the same base a pre-dispatch preflight for this item would have used before the branch existed. Its dispatching-stage input and its pull-request-state input remain the two distinct inputs the canonical prerequisite rule requires, resolved the same way a pre-dispatch preflight for this item resolves them, not folded into one value and not narrowed to only the next dispatch: the dispatching-stage input is every lifecycle stage this item's remaining execution will still exercise from this resume point forward — not the next stage alone — the same set a pre-dispatch preflight for this item would have resolved before the branch existed, so a platform listed only for a later stage (for example an `on_ready.github` reviewer this item has not reached yet) is still checked now rather than left for a later resume that this rule does not require; the pull-request-state input is, separately and per stage, the state the platform will actually see once this branch reaches that platform's own listed stage — for example draft for the stage the not-yet-opened pull request will open at, or non-draft for a later stage — adjusted by any existing workflow behavior that would change it before that platform's dispatch, exactly as the pre-dispatch pull-request-state input already is. The machine-local override does not switch source here either: it is resolved from the machine running the preflight, exactly as on every other path.
...
| ------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `operable`          | Can review           | The three configuration surfaces agree for this platform: at the stage it is listed for, against the base this item expects it to review, nothing in configuration says it will decline. This is a configuration-coherence verdict, not a live probe of the platform's actual installation or service availability — a state that lives outside configuration, which a configuration cross-check cannot see. |
...
| `base-branch-unmatched` | Base branch not covered    | The base branch this item targets is not among the bases the platform's own configuration covers. Reported with the base it was expected to cover and the bases covered. |
| `value-not-supported`   | Not a supported reviewer   | The configured value is not a reviewer platform this workflow supports for the lifecycle bucket that names it — `review.on_draft.runner`, `review.on_draft.github`, and `review.on_ready.github` each have their own supported-platform list, and a value supported in one is not automatically supported in another — so no cross-check is possible and no review would happen.   |
...
| `prerequisite-failed`  | Prerequisite not met   | The base branch this item targets was empty, unresolved, or malformed, or the set of lifecycle stages it will exercise was unresolved or malformed (never merely resolved to be empty — see `no-review-remaining` below), or the pull-request state for any one of those stages was empty, unresolved, or malformed, when the preflight ran — a failure of an input the run must resolve before the preflight runs, not a per-platform verdict. Decided before any per-platform verdict is computed; the run does not dispatch. |
| `no-review-remaining`  | No review remaining    | This item's own remaining execution resolved its set of lifecycle stages to be empty because no stage still ahead of it invokes a reviewer at all — for example, a run resumed only to dispatch merge or run post-merge cleanup, after every review stage has already completed. The stage set is resolved, not unresolved or malformed; this is a legitimate, expected state, not a failure. No per-platform verdict is computed, because there is no stage for any platform to review. The run dispatches. |
...
- **Scope statement**: every report states what it was checked against — the base branch this item targets, an existing workflow branch a resumed run checked instead of a base branch, or a specific pull request's branch — so a reader never has to guess whether a branch-local divergence was covered. Naming a resumed run's base branch instead of the existing workflow branch it actually checked would conceal exactly the branch-local divergence that resume path exists to catch.
- **Prerequisite failure report**: on Prerequisite not met, the report says plainly which required input was empty, unresolved, or malformed — the targeted base branch, the set of lifecycle stages this run will exercise, or the pull-request state for any one of those stages — and that no platform was cross-checked on this run — distinct from Passed, whose report states that every platform in the resolved list was actually checked.
- **No review remaining report**: on No review remaining, the report states plainly that no remaining lifecycle stage in this item's own remaining execution invokes a reviewer, and that no platform was cross-checked for that reason — distinct from Prerequisite not met, whose report describes a failed input the run must resolve before re-running. No review remaining resolves nothing and asks for no operator action; it states an already-resolved, expected state.
...
| -------------------------------------------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| The reviewer list for each lifecycle stage               | The shared workflow reviewer configuration, read from the branch in force for this item — its own single targeted base, or its own branch or pull-request copy once one exists | Names which platforms are expected to review, and at which stage, on this item's own branch-in-force copy |
| The machine-local override, when one is in effect        | The machine-local reviewer override, resolved once from the machine running the preflight, never from this item's own branch | Replaces this item's own shared list for that stage outright — the resolved list may be a subset, a superset with platforms the shared list never named, or an entirely different list |
| Each platform's own review configuration                 | The platform's own configuration, read from the branch in force for this item | Decides whether the platform will review at all, at which stage, and against this item's own targeted base |
| The base branch this item targets                        | This item's own resolved execution base — before a branch exists — or this item's own branch or pull-request target once one exists | A base the platform does not cover means the pull request this item opens against that base is declined |
| The lifecycle stage(s) this item will exercise, a distinct input from the pull-request state below | Every lifecycle stage bucket (for example `on_draft`, `on_ready`) this item's own remaining execution will exercise from the point this preflight runs forward — resolved the same way whether the preflight runs before any branch exists, against an existing branch, or against an existing pull request (see Business Rules). Aggregating this input across every item a batch will dispatch, for a distinct batch-level preflight ahead of each item's own, is deferred — see Out of Scope entry 12 | Selects, from the shared workflow reviewer configuration, which platforms are expected to review at each stage this item will exercise (not only the first) |
| The pull request state a listed platform will actually see at its own dispatch | The nominal stage that platform is listed for, from the input above, adjusted by any existing workflow behavior that changes that state before that platform is dispatched | A draft-stage listing against a platform that declines drafts is a disagreement only when no such adjustment resolves it before that platform is dispatched |
| Whether each configured value is a supported platform, for the lifecycle bucket that names it | The supported-platform list of that value's own lifecycle bucket — `review.on_draft.runner`, `review.on_draft.github`, and `review.on_ready.github` each have a distinct list, never one list shared across all three | An unsupported value must be reported, not dropped, and a value valid only in a different bucket is still unsupported in the bucket that actually names it |

Where a platform's own configuration cannot be read, that platform is Undetermined with the reason it could not be read. Where the resolved reviewer list itself cannot be read — the shared workflow reviewer configuration or the machine-local override is unparseable, rather than merely naming zero platforms — the run's own, already-existing configuration-loading step (outside this feature, unchanged by it) fails and halts before the run ever reaches the preflight or dispatches anything; the preflight itself is never invoked against an unreadable list, so it never has the opportunity to compute a verdict from one, and it is fail-closed for the same reason a run that cannot load its own configuration does not dispatch regardless of this feature. This is distinct from, and does not fall back to, the empty-resolved-list case above: an unreadable list is never silently treated as an empty one, because that would let a corrupt configuration pass as a deliberately empty one — exactly the false assurance this feature exists to remove. The preflight adds no new handling for this case because none is needed: the run never gets far enough to ask it.

**Base-branch coverage is evaluated against this item's own single targeted base.** A branch this item creates inherits whichever copy of every repository file exists on the base it targets, and that inherited copy — not any other base's copy — is what the actual reviewer gates will read once that branch and its pull request exist (the same branch-in-force rule already stated for each platform's own configuration). A platform's own resolved list that does not name that platform for this item's targeted base places no expectation on it, and the platform's absence from that list is not itself a disagreement; a platform that is named and whose own configuration does not cover this item's targeted base is Cannot review with the reason Base branch not covered.

**A coverage gap on this item's targeted base is a Cannot review verdict for that platform, and Blocked stops this item's dispatch.** Nothing is dispatched for this item. The gate outcome and the required next action are therefore fully determined.
...
| A single-item run resumes an existing branch or pull request instead of starting fresh | The branch or pull request that already exists — not the "before any branch exists" state — is what the actual reviewer gates will read, so the preflight must be scoped to what already exists, not to a base branch a different, earlier check already covered |
...
An empty, unresolved, or malformed base branch is not the same case as the reviewer-list exemption above, and does not produce Passed by the same reasoning. The base branch this item targets, the lifecycle stage(s) this run will exercise (the distinct Gate-inputs input of that name), and the pull-request state a platform will actually see at its own dispatch (its listed stage, from that input, as adjusted by any existing workflow behavior that changes it before dispatch) are all required inputs the run resolves before the preflight runs, not inputs the preflight resolves for itself; a run that reaches the preflight without a resolved base branch, without a resolved set of lifecycle stage(s), or without a resolved pull-request state for each stage that set does contain, has already failed that prerequisite. The preflight does not run against such an input and does not report Passed on its account — an unresolved base branch would otherwise vacuously satisfy the base-branch coverage check without reading any platform's own configuration at all, and an unresolved stage or pull-request state would let stage-excluded checking be silently skipped for every platform, which is exactly the false assurance this feature exists to remove. Any of these three failures is reported as the same distinct run outcome, Prerequisite not met — never folded into any of the three per-platform verdicts, and never reported as Passed, Passed some unverified, or Blocked.

**A set of lifecycle stages that resolves to empty is not the same case as one that is unresolved or malformed, and does not produce Prerequisite not met by the same reasoning.** "Unresolved" means the run could not determine this item's own remaining set of lifecycle stages; "resolved to empty" means the run determined it, and the answer is that none remain — a single item's own remaining execution can legitimately have no lifecycle stage left that invokes a reviewer at all, for example when a run resumes only to dispatch merge or run post-merge cleanup after every review stage this item exercises has already completed. That is a definite answer, not a missing one, so treating it as Prerequisite not met would misdescribe it: there is no failed input for the run to resolve before re-running, because nothing is unresolved. Prerequisite not met's own required next action — resolve the failed input and re-run — has no available action here, which is itself the sign that the outcome does not fit. This case produces the distinct outcome No review remaining instead, so the report can say plainly "nothing is left to review" rather than "an input this run needed was missing." The exemption applies only to a stage set that itself resolves to empty; it does not extend to an unresolved or malformed stage set, nor to a resolved, non-empty stage set with an unresolved or malformed pull-request state for any one of its stages — both of those remain Prerequisite not met exactly as stated above.

A distinct batch-level preflight that runs once before a batch's first dispatch — separate from, and ahead of, each dispatched item's own single-item preflight, aggregating every item's own remaining stages redundantly with (not deferring to) that item's own later check — is deferred. See **Out of Scope entry 12** and issue #1743.

### Allowed outcomes and required next actions

| Outcome                 | When it is produced                                                | Required next action                                                                     |
| ----------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| Prerequisite not met    | The base branch this item targets is empty, unresolved, or malformed; or its set of lifecycle stages is unresolved or malformed (not merely resolved to be empty); or the pull-request state for any one stage in a resolved set is empty, unresolved, or malformed | Nothing is dispatched. No per-platform verdict is computed. The run resolves the failed input and re-runs |
| No review remaining     | This item's set of lifecycle stages resolves to empty because no stage still ahead of it invokes a reviewer | Dispatch proceeds. No per-platform verdict is computed, because no stage remains for any platform to review. No operator action, no confirmation collected |
...
**Prerequisite not met is decided first, before any per-platform verdict — but only once a resolved-empty stage set has already been separated out as No review remaining.** The gate cannot compute base-branch coverage — or read the shared reviewer configuration's own copy — without a resolved base branch to evaluate against, and it cannot compute the stage-excluded check without both a resolved set of every lifecycle stage this run will exercise and, for every stage in that set, a resolved pull-request state (that stage as adjusted by any existing workflow behavior that changes it before dispatch) to compare a platform's listing against, so it checks all three prerequisites — the base branch, the full set of lifecycle stages, and a resolved pull-request state for each stage in that set — before computing any platform's verdict. A stage set that resolves to empty is not itself a failed prerequisite: the gate reports No review remaining for that case, per the Business Rules exemption above, and does not reach the Prerequisite not met check at all, because there is no stage in the set left to check a pull-request state for. Where the stage set is unresolved or malformed, or resolves to a non-empty set with an unresolved or malformed pull-request state for any one of its stages, or the base branch itself is empty, unresolved, or malformed, the gate reports Prerequisite not met and stops there; no platform is classified Can review, Cannot review, or Undetermined for that run, because none of those verdicts was actually computed — an unresolved stage set, or an unresolved pull-request state for even one stage within an otherwise-resolved set, is never treated as silently satisfying the stage-excluded check for every platform at every stage. Where all three prerequisites are met for every stage in the set, the gate proceeds to per-platform verdicts as stated below.
...
**Value-not-supported is checked first and stands alone.** Before any of the other checks, the gate confirms the configured value is one of the reviewer platforms this workflow supports for the specific lifecycle bucket that names it — `review.on_draft.runner`'s own supported list, `review.on_draft.github`'s own supported list, or `review.on_ready.github`'s own supported list, never one list checked against all three buckets, because a value this workflow supports only as a local-runtime reviewer (`review.on_draft.runner`) is not thereby a value `review.on_ready.github` can dispatch, and the reverse holds too. When it is not supported in its own bucket, no cross-check against its own configuration is even possible — there is no defined platform to read a configuration for in that bucket — so the platform is Cannot review with only the reason Not a supported reviewer, and none of the other three checks is attempted for it.

**For every configured value that is a supported platform, a Cannot review verdict names every one of the three remaining disagreement reasons that applies, not one.** The three checks — automatic review turned off, a listed stage its own configuration excludes, and an uncovered base branch — are independent of each other, and a platform can fail more than one at once: a platform can simultaneously have automatic review off, be listed for a stage its own configuration declines, and not cover the base this item expects it to review against. The gate runs all applicable checks for every such platform in the resolved list and reports the complete set of disagreement reasons that apply on that platform's single Cannot review verdict. No check is skipped because another already produced Cannot review, and no precedence between the reasons is defined, because a report that names every applicable reason needs none — precedence would only be necessary to pick one reason to the exclusion of others, and this gate never does that. Across all four disagreement reasons, exactly one of two shapes applies to any given platform: Not a supported reviewer alone, or some non-empty subset of the other three.
...
| ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| The single-item orchestration protocol                    | That a fresh run performs the preflight before its first mutation — before any branch is created — not merely before the reviewer gates; that a resumed run with an existing branch and no pull request yet checks that existing branch's own state instead; that a resumed run with an existing pull request uses the existing-pull-request path against that pull request's own branch instead; and that the gates themselves run after a preflight that did not block, with their own behaviour otherwise unchanged |
...
**Example: Base branch not covered.** This item targets the epic integration branch. `platform-z`'s own configuration covers the main integration branch but not the epic integration branch. Contradiction: this item targets the epic integration branch; `platform-z`'s own configuration does not cover it. Report: Cannot review, reason Base branch not covered, naming the base targeted (the epic integration branch) and the bases covered (the main integration branch only), surface `platform-z`'s own configuration. Remedy: add the epic integration branch to the set `platform-z`'s own configuration covers, or narrow this machine's reviewer list through the machine-local override to exclude `platform-z` for this item, and re-run.
...
- [ ] With a platform whose own configuration does not cover the base branch this item targets, the preflight produces Blocked, classifies that platform Cannot review with the reason Base branch not covered, and names both the base it was expected to cover and the bases covered.
...
- [ ] With a platform for which more than one disagreement reason applies at once — for example automatic review turned off and a base-branch gap on the same platform — the preflight's single Cannot review verdict for that platform names every applicable reason, not only the first one found.
...
- [ ] On Blocked, no item is dispatched: no branch is created, no pull request is opened, and no tracker status changes by that run. The preflight itself posts no comment and applies no label; where the enclosing run already keeps an audit record of itself, folding the Blocked outcome into that pre-existing record (which may be a comment on an already-existing pull request from a resumed run) is the run's own pre-existing action, not something this rule prohibits.
...
- [ ] With a configured value in the resolved list that is not a supported reviewer platform for the lifecycle bucket that names it — including a value that is supported, but only in a different bucket, such as a local-runtime reviewer named in `review.on_ready.github` — the preflight classifies it Cannot review with the reason Not a supported reviewer, names the offending value, and therefore produces Blocked. It is never silently dropped, and it is never classified Undetermined merely because it happens to be a recognized name in some other bucket's list.
...
- [ ] With this item's targeted base branch empty, unresolved, or malformed, the preflight produces Prerequisite not met rather than Passed, does not compute a Can review / Cannot review / Undetermined verdict for any platform, and does not dispatch. An unresolved base branch does not vacuously pass the base-branch coverage check.
- [ ] With the run's set of lifecycle stages unresolved or malformed, or the pull-request state for any one stage in an otherwise-resolved set empty, unresolved, or malformed, the preflight produces Prerequisite not met rather than Passed, and does not silently skip the stage-excluded check for any platform at any stage by treating the unresolved stage or state as agreement.
- [ ] With the run's set of lifecycle stages resolved to be empty because no stage still ahead of this item invokes a reviewer — for example, a run resumed only to dispatch merge or run post-merge cleanup after every review stage has already completed — the preflight produces No review remaining, not Prerequisite not met and not Passed, computes no Can review / Cannot review / Undetermined verdict for any platform, and dispatches rather than stopping for the operator. A resolved-empty stage set is never reported as a failed prerequisite, and never treated as if it were the resolved-empty reviewer list that produces Passed.
...
- [ ] A preflight that produces Blocked, Passed some unverified, Prerequisite not met, or No review remaining has the same absence of side effects as a Passed one.
...
- [ ] Verdicts are determined fresh each run: on the second run, repairing the surface that made a platform Cannot review for a given reason removes that specific disagreement, and breaking it between two runs reintroduces the same disagreement, in both cases with no other change. That platform's own overall verdict on the second run is whichever the disagreement-reason rule (where more than one reason applied) produces from the reasons that remain — Can review only if repairing that one disagreement leaves none of the others outstanding and this item's branch-in-force configuration was read and agrees, Undetermined if that configuration remains unreadable with no other disagreement proven, or still Cannot review if another disagreement or an unreadable-yet-disagreeing configuration remains; repairing one disagreement is not asserted to guarantee that platform's own verdict becomes Can review by itself. The run outcome that follows is, in turn, whichever the canonical outcome rule produces from every platform's verdict in the resolved list — including the repaired platform's own resulting verdict, not only every other platform's: Blocked if the repaired platform or any other remains Cannot review; otherwise Passed, some unverified if the repaired platform or any other remains Undetermined; otherwise Passed. No earlier run's recorded outcome is consulted.
...
- [ ] The documentation states that a pre-dispatch preflight is checked against the base branch the run targets and therefore cannot see a divergence introduced by a branch that does not yet exist.
...
- [ ] When the preflight is performed against an existing pull request rather than before any branch exists, its base-branch input is that pull request's own target base — a single, well-defined value read from the pull request itself, not a multi-base run context that may no longer exist by the time an existing pull request is checked. It cross-checks each platform's own configuration as read from that pull request's own branch — not the branch it targets — against every lifecycle stage this item's remaining execution will still exercise (not only the pull request's current stage, per the same rule the branch-only resume path follows) and that one target base, resolving the pull-request state per stage in that set, and reaches the same Can review / Cannot review / Undetermined per-platform verdicts and the same Passed / Passed, some unverified / Blocked / Prerequisite not met / No review remaining run outcome that a pre-dispatch preflight with a single targeted base would reach — including Prerequisite not met when the remaining-stage set is unresolved or malformed, or the per-stage pull-request state within an otherwise-resolved set is itself empty, unresolved, or malformed, and including No review remaining when the remaining-stage set resolves to empty because no stage still ahead of this pull request invokes a reviewer, per the canonical outcome rules. The shared workflow reviewer configuration is read from that same pull request branch; the machine-local override is resolved from the machine running the preflight, exactly as it is for a pre-dispatch preflight, and keeps its usual precedence over that branch's shared list.
- [ ] A preflight performed against an existing pull request states, per the Scope statement rule, that it was checked against that specific pull request's branch rather than against the run's base branch, so a reader never has to guess which scope a given report covers.
...
- [ ] When a single-item run resumes an existing workflow branch or pull request instead of starting from a state where no branch exists yet, the preflight uses whichever path matches what already exists: the existing-pull-request path, reading that pull request's own branch, when a pull request already exists; or a preflight checked against that existing branch's own state when a branch exists without a pull request yet — reading that branch's own configuration copies, evaluated against the single base this item's own execution already resolved to target (not the branch itself) and, kept as the two distinct inputs the canonical prerequisite rule requires, every lifecycle stage this item's remaining execution will still exercise (not only the next one) and, per stage, the pull-request state a platform listed for that stage will actually see at its own dispatch, per the branch-only resume rule in Business Rules. The "before any branch exists" pre-dispatch path applies only when resuming finds neither a branch nor a pull request already in place — once a branch exists, checking a base branch a different, earlier run already covered misses a disabling change already present on the branch the reviewer will actually read.
- [ ] When a single-item run resumes with no remaining lifecycle stage that invokes a reviewer — for example, resuming only to dispatch merge or run post-merge cleanup after every review stage this item exercises has already completed — the preflight produces No review remaining and the run's own dispatch proceeds without waiting on the preflight; a legitimately empty remaining reviewer-stage set never blocks a resumed run indefinitely, and is never reported as though the run were missing an input it needed.
- [ ] The single-item orchestration protocol and the reviewer platform's integration documentation describe the same outcomes, the same verdicts, and the same display labels, with no surface naming a verdict the others do not.
...
11. **Detecting a live installation or service-availability loss that leaves every configuration surface unchanged** — for example, a platform's GitHub App removed, or its service otherwise made unavailable, after its own configuration file was last read, so `.coderabbit.yaml` (or the equivalent) still reads as enabled. Deferral rationale: the preflight's cross-check is a configuration read; detecting this failure mode needs a materially different mechanism — a live external-service probe — which Step 7a's own reachability classification already attempts and already documents as imperfect (its activity-proxy check can itself be a false Reachable after an App is removed; see `docs/workflow/development-workflow/integrations/coderabbit.md`). Extending this preflight's cross-check to the same kind of probe is deferred rather than duplicated ad hoc. Until it is added, Can review is a configuration-coherence verdict, not an absolute guarantee that a platform will actually review — including on the existing-pull-request path (the "A preflight against an existing pull request asserts operability" AC group), which also cross-checks configuration only. A platform whose installation was removed after its configuration was last read still declines silently; that decline is reported as a review failure when it happens, not as a preflight problem, exactly like the already-established "passes the preflight and then fails" case. Human confirmation requested: yes — this narrows brief objective B-1's "can actually review" framing and B-5's "asserted operability" framing, and the Overview's headline promise, for this specific failure mode, and a human should decide whether that narrowing is acceptable or whether a live probe belongs in this feature's first iteration.

12. **Batch-level preflight aggregation across items with mixed resume states.** A single batch run can dispatch several items, and each of those items may independently be fresh, resumed on a branch with no pull request yet, or resumed on an existing pull request — and each may already be at a different point in its own lifecycle (for example one item already past its draft stage while another has not opened a pull request yet). A distinct batch-level preflight — separate from, and running before, each item's own single-item preflight (the fresh, branch-only-resumed, and existing-pull-request-resumed paths this spec defines throughout) — that aggregates every dispatched item's own remaining stages, and correctly resolves each item's own configuration source (a resumed item's own branch or pull request copy, versus a fresh item's targeted-base copy) rather than a single global source for the whole batch, is deferred to a follow-up item. Deferral rationale: three consecutive review cycles on this spec (commits `fcb1ba2a` through `d0993077`, and one further finding after it) each produced a distinct, real gap in this batch-level aggregation model — an emptied stage set, an unpreserved item/base/stage association, and an unresolved per-item configuration-source distinction between resumed and fresh items sharing a base — while the single-item preflight this spec defines (fresh, branch-only resume, and existing-pull-request resume) held up under the same scrutiny with no further gaps found across that same review history. This spec ships that stable single-item core; issue #1743 tracks the batch-level aggregation this entry defers. Human confirmation requested: yes — this narrows brief objective B-4's "before a batch starts" framing to per-item enforcement (each item's own preflight still fails loudly, and still before that item's own first mutation) rather than one aggregate check for the whole batch ahead of its first dispatch.
...
2. When the disagreement affects only part of a batch — for example a base branch coverage gap that applies to items targeting an epic integration branch but not to items targeting the main integration branch — should the preflight block the whole batch, or only the affected items? Blocking everything is safer and simpler to report; blocking only the affected items preserves throughput but makes the outcome harder to state in one line. No answer is assumed; the spec currently treats Blocked as a whole-run stop. This question presumes the distinct batch-level preflight this spec defers under Out of Scope entry 12; it is carried to issue #1743 rather than answered here.
