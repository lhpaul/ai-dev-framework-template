...
Reading a platform's own configuration can succeed for one targeted base and fail — be unreadable, or not complete within the preflight's bound — for another; the two per-base results still combine into exactly one verdict for that platform, using the same severity ordering the run outcome uses (Cannot review outranks Undetermined, which outranks Can review). If a targeted base's own configuration was read and does not cover that base, the platform is Cannot review even if another targeted base's own configuration could not be read at all — a proven gap on one base is definite evidence the platform will decline pull requests opened against it, and an inconclusive reading of another base cannot undo that. If no targeted base's own configuration shows a proven gap but at least one targeted base's own configuration could not be read, the platform is Undetermined. A platform is Can review only when every targeted base's own configuration was read and every one of them is covered.
