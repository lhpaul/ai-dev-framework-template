...
- A preflight performed against an existing workflow branch that has no pull request yet switches configuration source the same way an existing-pull-request preflight does: the shared workflow reviewer configuration and each platform's own configuration are read from that branch's own copies, not from any targeted base's copy, because that branch is what the reviewer will actually read once its pull request exists. Its base-branch input is not the branch itself — a branch has no base-branch coverage of its own to check, and treating its name as a base would falsely evaluate every platform's coverage against a base no platform's configuration is expected to name — but the single base this item's own execution already resolved to target, the same base a pre-dispatch preflight for this item would have used before the branch existed. Its dispatching-stage and pull-request-state input is, correspondingly, the stage this branch is about to be dispatched into next — for example the draft stage the not-yet-opened pull request will open at — resolved and adjusted by any existing workflow behavior exactly as the pre-dispatch dispatching-stage input is. The machine-local override does not switch source here either: it is resolved from the machine running the preflight, exactly as on every other path.
...
- [ ] When a single-item run resumes an existing workflow branch or pull request instead of starting from a state where no branch exists yet, the preflight uses whichever path matches what already exists: the existing-pull-request path, reading that pull request's own branch, when a pull request already exists; or a preflight checked against that existing branch's own state when a branch exists without a pull request yet — reading that branch's own configuration copies, evaluated against the single base this item's own execution already resolved to target (not the branch itself) and the pull-request state this branch is about to be dispatched into next, per the branch-only resume rule in Business Rules. The "before any branch exists" pre-dispatch path applies only when resuming finds neither a branch nor a pull request already in place — once a branch exists, checking a base branch a different, earlier run already covered misses a disabling change already present on the branch the reviewer will actually read.
