...
- [ ] When a single-item run resumes an existing workflow branch or pull request instead of starting from a state where no branch exists yet, the preflight uses whichever path matches what already exists: the existing-pull-request path, reading that pull request's own branch, when a pull request already exists; or a preflight checked against that existing branch's own state when a branch exists without a pull request yet — reading that branch's own configuration copies, evaluated against the single base this item's own execution already resolved to target (not the branch itself) and, kept as the two distinct inputs the canonical prerequisite rule requires, the dispatching stage this item's execution already resolved to dispatch next and the pull-request state this branch's not-yet-opened pull request will have at that dispatch, per the branch-only resume rule in Business Rules. The "before any branch exists" pre-dispatch path applies only when resuming finds neither a branch nor a pull request already in place — once a branch exists, checking a base branch a different, earlier run already covered misses a disabling change already present on the branch the reviewer will actually read.
