...
| -------------------------------------------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| The reviewer list for each lifecycle stage               | The shared workflow reviewer configuration, resolved once per run from the repository's main integration branch regardless of how many bases the run targets | Names which platforms are expected to review, and at which stage                      |
| The machine-local override, when one is in effect        | The machine-local reviewer override, resolved from the machine running the preflight, never from any targeted base's branch | Replaces the shared list for that stage outright — the resolved list may be a subset, a superset with platforms the shared list never named, or an entirely different list |
...
**A run may target more than one base branch.** One run can dispatch items against an epic integration branch and items against the main integration branch, so the gate's base-branch input is the set of every base the run will target, never a single representative base. The shared workflow reviewer configuration and the machine-local override are resolved once per run, from the repository's main integration branch, never per targeted base: which platforms this workflow expects to review, and at which stage, is a decision this repository's workflow configuration makes about its review process in general, not one that depends on which base a particular item happens to target, and the machine-local override is inherently machine-scoped, not branch-scoped. There is therefore exactly one shared reviewer list and one resolved reviewer list for the whole run, with no per-base list-membership variance to combine or report. Only each platform's own configuration is read per targeted base, because whether a platform covers a particular base is a fact about that platform's own external configuration, not about this repository's workflow configuration. Base-branch coverage is evaluated for every base in that set: a platform in the resolved list is Can review only where its own configuration covers all of them, and is Cannot review with the reason Base branch not covered where it covers none of them or only some. Where a platform's own configuration differs between two targeted bases, each base is evaluated against that platform's own configuration as read from that base, and the report names every base that was checked.

Reading a platform's own configuration can succeed for one targeted base and fail — be unreadable, or not complete within the preflight's bound — for another; the two per-base results still combine into exactly one verdict for that platform, using the same severity ordering the run outcome uses (Cannot review outranks Undetermined, which outranks Can review). If a targeted base's own configuration was read and establishes any of the three disagreement reasons against that base's copy — automatic review turned off, a listed stage its own configuration excludes, or that base itself not covered — the platform is Cannot review with that reason, even if another targeted base's own configuration could not be read at all: a proven disagreement on one base's readable copy is definite evidence, and an inconclusive reading of another base cannot undo it. Only when no targeted base's own configuration establishes any disagreement reason, and at least one targeted base's own configuration could not be read, is the platform Undetermined. A platform is Can review only when every targeted base's own configuration was read and every one of them agrees.
...
Worked examples are part of the changed surface wherever a disagreement case is described. Each of the three disagreement cases below shows the same shape — the inputs, the contradiction the preflight finds, the report it produces, and the remedy it points to — so an operator can match a report to an example without translating between them. A fourth example follows the same nominal shape as one of the three but resolves to Can review, to show that the stage check is decided against the pull request state at actual dispatch, not against the nominal listed stage alone.
...
- [ ] Verdicts are determined fresh each run: on the second run, repairing the surface that made a platform Cannot review flips that platform's own verdict to Can review with no other change, and breaking it between two runs flips the platform's verdict back the same way. The run outcome that follows is whichever the canonical outcome rule produces from every platform's verdict — Passed when every other platform in the resolved list is already Can review, Passed, some unverified when another platform remains Undetermined, or still Blocked when another platform remains Cannot review; repairing one platform's surface is not asserted to guarantee Passed by itself. No earlier run's recorded outcome is consulted.
...
- [ ] When the preflight is performed against an existing pull request rather than before any branch exists, it cross-checks each platform's own configuration as read from that pull request's own branch — not the branch it targets — against the stage and every base that pull request's run targets, and reaches the same Can review / Cannot review / Undetermined per-platform verdicts and the same Passed / Passed, some unverified / Blocked run outcome that a pre-dispatch preflight would reach. The shared workflow reviewer configuration is read from that same pull request branch; the machine-local override is resolved from the machine running the preflight, exactly as it is for a pre-dispatch preflight, and keeps its usual precedence over that branch's shared list.
...
- [ ] This is the preflight's asserted-operability path against a real pull request that brief objective B-5 requires; creating a scratch pull request to exercise this same path remains deferred under Out of Scope entry 1.
