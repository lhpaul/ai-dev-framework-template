...
| The lifecycle stage(s) this run will exercise, a distinct input from the pull-request state below | The run's own resolved dispatch plan — every stage bucket (for example `on_draft`, `on_ready`) the run expects to use before the next preflight runs, resolved the same way the targeted base-branch set is | Selects, from the shared workflow reviewer configuration, which platforms are expected to review at each stage the run will exercise (not only the first) |
