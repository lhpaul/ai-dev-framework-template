...
| The lifecycle stage(s) this run will exercise, a distinct input from the pull-request state below | For each item the run will dispatch, every lifecycle stage bucket (for example `on_draft`, `on_ready`) that item's own remaining execution will exercise, unioned across every item — never one global stage set applied indiscriminately to every item. A batch-level preflight resolves this the same way a single-item preflight for each of those items would, redundantly with (not deferring to) that item's own later single-item preflight: catching a disagreement now, before the batch's own first dispatch, is the whole point of the batch-level check (see the coverage-preservation rule below), and a later, item-scoped preflight repeating the same check is not itself an error | Selects, from the shared workflow reviewer configuration, which platforms are expected to review at each stage each item will exercise (not only the first) |
...
**A batch-level preflight preserves the same reviewer cross-check every item's own later preflight would perform; it never defers or narrows it.** The batch-level stage-set input is every stage every dispatched item's own remaining execution will exercise, exactly as a single-item preflight for that item would resolve it — not merely the stage each item is about to enter next. Brief objective B-4 requires failing loudly before a batch starts, at the cost of a whole run otherwise; a batch-level check that only verified each item's immediate next stage, deferring the rest to that item's own later single-item preflight, would let a reviewer disagreement on a later stage (for example automatic review turned off on the `on_ready.github` platform) pass Passed at the batch level and surface only after the batch has already begun dispatching that item — exactly the cost B-4 exists to avoid. The batch-level check and each item's own later single-item preflight therefore check the same stages redundantly by design: the batch-level check exists to catch the disagreement early, and the item-level check existing separately, per its own AC group, is not itself in tension with that — both are required, not alternatives.
