...
| Historical artifact availability | `find docs/specs/developments -maxdepth 2 -type f -path '*201-tech-lead-parser-regex-plan-requirements*' -print` | No matching local historical artifact exists |
...
- [ ] `CHANGELOG.md` - add the exact `[Unreleased]` correction listed above.
...
8. Add under `CHANGELOG.md` `[Unreleased]`:
   `- **Make Protocol 02 parser guidance portable** (#1310): Inline the required parser-risk and suppression intent so synced repositories do not depend on an unavailable historical development fixture.`
