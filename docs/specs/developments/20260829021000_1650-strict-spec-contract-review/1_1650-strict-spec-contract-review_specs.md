...
- [ ] **AC-16b.** The checks are attempted within their own bounded time budget: **the same bound the review itself uses** — the reviewer's `--timeout`, defaulting to 300 seconds — applied independently to the checks, so a round that runs them is bounded by twice that value and never by an open interval. A failure costs at most the bound. Running them takes time a review without them would not spend; what that time can never do is change the review's outcome or make a round unbounded.
- [ ] **AC-16c.** The bound has exactly one source and **no second setting exists**: there is no configuration, environment variable or flag that sets the checks' budget separately, capped or otherwise. Overriding the review's timeout overrides the checks' by the same act, and that is the only way to change it. A second knob, even a capped one, is one more place for the two values to disagree about a bound whose whole purpose is that a round cannot outlast it.
