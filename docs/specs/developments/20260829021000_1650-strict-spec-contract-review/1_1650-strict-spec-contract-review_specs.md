...
- [ ] **AC-16c.** The bound has one source. Overriding the review's timeout overrides the checks' by the same act, and no separate value can be configured to a number the review's timeout does not already permit — a second knob would let the checks outlive the review that dispatched them.
