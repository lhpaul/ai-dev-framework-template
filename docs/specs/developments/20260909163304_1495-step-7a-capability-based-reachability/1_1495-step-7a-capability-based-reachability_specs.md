...
| The hosted Codex review configured, its app installed and the service reachable, driven from any supported runner | Reachable                       | It needs no local runtime, so the driving runner's identity does not bear on invoking it   |
...
| The hosted Codex review configured and its app installed, but the service cannot be reached      | Unreachable                     | The determination started and did not answer, so it is reported as a check that did not complete rather than as a missing prerequisite |
