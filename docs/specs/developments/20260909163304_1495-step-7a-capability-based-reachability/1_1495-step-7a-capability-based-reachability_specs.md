...
**Preconditions**: A draft pull request has reached the internal review gate. The unavailable-reviewer policy is readable. The resolved configuration names at least one supported reviewer, and every reviewer it names is present and callable in the environment the gate is running in — this is the path where nothing is unavailable, so the policy never has to decide anything. The runner driving the gate is not the reviewer's own native runner.
...
- Evaluation order is fixed, and every outcome below is stated under it: read the unavailable-reviewer policy, resolve the configured list (machine-local override first, shipped configuration otherwise), mark the reviewers the override left out as Excluded by override, determine availability for each remaining reviewer, then apply the policy to the result.
- The policy is read first because it decides what the gate does with every later outcome, so a policy the gate cannot read is not a question it can defer. Where the policy is unsupported or unreadable the gate blocks immediately, whatever the reviewer list turns out to hold — including the absent or empty list that would otherwise reach the fallback. Reading the list first and falling back would commit the gate to proceeding before it knew whether the operator had forbidden it.
...
- A reviewer invoked through a hosted review service rather than a local runtime — the Codex GitHub App review, carried by the configured value `codex-github`, alongside CodeRabbit — is a supported reviewer value, and the canonical list names it. This follows from capability rather than being an exception to it: such a reviewer needs no local runtime, so no runner is inherently barred from using it, and the identity of the driving runner tells the gate nothing about whether it can be invoked.
- For a hosted-service reviewer, availability means the service is installed for the repository and enabled for this review, decided the way CodeRabbit's already is. It does not mean the service has been confirmed responsive, because responsiveness cannot be established without asking the service to review — which the gate must not do before it has decided availability and applied the policy. A hosted service that is installed is therefore Reachable, and one that accepts the review and then fails, stalls, or never answers is a review failure like any other. The boundary is the same one drawn for local runtimes: availability asks whether the reviewer can be invoked, never whether invoking it will go well.
...
| The unavailable-reviewer policy is set to an unsupported value, or cannot be read              | Blocked                          | Dispatches nobody; names the offending policy value or the input that could not be read. Decided before the reviewer list is resolved, so it holds even where the list is absent, empty, or malformed | Correct the policy value                                     |
...
An unreadable or unsupported policy combined with an absent, empty, or malformed reviewer list resolves to Blocked on the policy, because the policy is read first and the gate stops there without resolving the list. The combination "excluded by the override and also unreachable" cannot occur: under the stated order, exclusion is settled before availability is determined, and excluded reviewers are never examined. No outcome installs software, provisions access, substitutes a reviewer, or converts the pull request out of draft.
...
| The hosted Codex review installed, dispatched, and then never answers                             | Review failure                  | Availability was settled before dispatch; a silent service afterwards is a review outcome, not a reachability verdict |
...
- [ ] With the policy unsupported or unreadable and the reviewer list absent, empty, or malformed at the same time, the gate blocks on the policy and reports it as the cause. It does not reach the fallback reviewer.
- [ ] With the unavailable-reviewer policy input present but unreadable — its file will not parse, or its value cannot be read as one of the supported forms — the gate blocks, dispatches nobody, and names the input that could not be read. It does not silently apply the default policy.
