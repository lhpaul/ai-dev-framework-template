...
- Determining availability is bounded in time. A determination that does not complete within its bound yields Unreachable with the reason that it did not complete, so the gate always reaches a verdict.
...
- The gate dispatches only reviewers the resolved configuration names. It never installs a missing runtime, never provisions a missing prerequisite, and never substitutes a different reviewer for an unreachable one.
...
| ------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------- |
| `reachable`         | Reachable            | The gate can invoke this reviewer in the current environment, and will dispatch it.                            |
