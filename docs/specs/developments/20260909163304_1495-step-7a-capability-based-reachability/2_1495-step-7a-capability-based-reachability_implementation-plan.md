...
| VL-12 — Sync manifest coverage | `grep -n "scripts/development-workflow/" sync-manifest.yaml` | Line 105 covers the directory at `hub_only` scope via `glob: "**/*"`; lines 109-129 add explicit `product_repo_injection` entries for the six runtime helpers a product repo needs a product repo needs. |
