...
| 2026-09-21 | **Live-branch probe widened to local refs.** The probe was specified over `refs/remotes/origin/` only, copying `workflow-next-action.sh:602`–`:620`, which is origin-only in both its `--verify` call and its `sed` filter. A just-cut fast-track branch is a local `refs/heads/` ref with no push and no PR, so an origin-only probe read `none` and the single-item gate would have stopped in-flight work | The probe now matches the issue-keyed pattern in `refs/heads/` **and** `refs/remotes/origin/`; a bare `git show-ref` already lists both, so this costs no extra command. `stale-backlog-open-fix-branch-no-pr-continues` now uses a local-only ref, and `stale-backlog-pushed-fix-branch-no-pr-continues` covers the origin-only case | Bugbot finding, 2026-09-21 |
...
| Live-branch probe (both callers) | `workflow-next-action.sh:602`–`:620`: `git show-ref --verify -q refs/remotes/origin/<prefix>/<slug>`, then a `git show-ref` enumeration filtered by `sed -n "s\|.*refs/remotes/origin/<prefix>/\|\|p"` | Existing probe form, no API call, and `git show-ref` exits `1` on no match (that is `none`, not a failure). **Both of its steps are origin-only**, so a just-cut unpushed branch is invisible to it; a bare `git show-ref` lists `refs/heads` and `refs/remotes` alike, so this gate matches the issue-keyed pattern in both namespaces — verified 2026-09-21 |
...
- [ ] Add `scripts/development-workflow/framework-mode-backlog-type-gate.sh` (**pinned name** —
  ...
  **Kind (1) must look at local refs as well as remote ones.** A branch that was just cut and
  not yet pushed exists only as `refs/heads/<prefix>/<issue>-<slug>`; it has no
  `refs/remotes/origin/` ref and no PR, which is precisely the "work has started" state this
  input exists to detect. `workflow-next-action.sh:602`–`:620` is **origin-only** — both its
  `git show-ref --verify -q refs/remotes/origin/<prefix>/<slug>` call and its enumeration,
  whose `sed -n "s|.*refs/remotes/origin/<prefix>/||p"` drops every local ref — so this gate
  reuses that probe's *form* but widens its namespace rather than copying the limitation.
  Origin-only is defensible for a status heuristic that only ever mislabels a stage; it is not
  defensible for a gate that can stop work. A single `git show-ref` invocation already lists
  `refs/heads` and `refs/remotes` together, so covering both costs nothing: match the
  issue-keyed pattern against `refs/heads/<prefix>/…` **and**
  `refs/remotes/origin/<prefix>/…`, and treat a hit in either as `present`.

  | Caller | (1) live branch | (2) open PR | (3) merged PR |
  | --- | --- | --- | --- |
  | Single-item (prelude `item` scope) | `git show-ref` in the form `workflow-next-action.sh:602`–`:620` uses, matched against the issue-keyed head pattern in **both** `refs/heads/<prefix>/…` and `refs/remotes/origin/<prefix>/…`. Local read, no API call | Scope JSON `pullRequests.open[]` (`run-epic-scope-resolver.sh:769`), selected by head ref at `:397` | Scope JSON `pullRequests.merged[]` (same emission, selected at `:411`). Both arrays filtered to `^(feature\|fix\|refactor\|hotfix)/`, the filter `:709` already uses for `merged_impl_count` |
  ...
  **Combining the three**: `present` if **any** kind is found — finding work is conclusive, so
  ...
  found work. Per-probe unavailability: `git show-ref` exiting `1` means *no matching ref* in either namespace
...
- [ ] **Status data flow — the gate needs the tracker status *and* the artifact stage, and
  ...
  | --- | --- | --- | --- |
  | Portfolio scan | Tracker status: `workflow-batch-plan.sh:542`. Artifact stage: the `STATUS` line of the `workflow-next-action.sh` call batch-plan already makes at `:569`–`:582` (empty when that call exits non-zero — the "no merged spec/plan PR yet" branch). Branch/PR evidence: all three kinds — the two-namespace `git show-ref` branch probe, `open_implementation_pr_metadata` (`:363`–`:433`, open PRs only) **and** the merged-PR probe in `workflow-next-action.sh:625`–`:660` form; any probe failure maps to `--branch-pr-evidence unavailable`. Type: the gate's own `get_tracker_type_for_issue` read, since batch-plan retains no Type | In-process shell variables `$issue_number` / `$tracker_status` / the parsed `$status`; no new env var, no file | `framework-mode-backlog-type-gate.sh --issue "$issue_number" --status "$tracker_status" --artifact-stage "$status" --branch-pr-evidence "$pr_evidence" --caller scan --repo-root "$repo_root"`, invoked **after** the next-action call (`:582`) and before the block is printed (`:634`). Costs one extra GraphQL request per scanned non-terminal folder — see "Tracker-read cost" below |
  | Single-item run (bounded prelude scope `item` only) | Tracker status and Type: `run-epic-scope-resolver.sh:678` / `:687`, which `run-item-scope-resolver.sh` delegates to. Artifact stage: `workflow-next-action.sh --development <folder>` for the item's development folder, or empty when the item has no folder. Branch/PR evidence: `pullRequests.open[]` / `pullRequests.merged[]` already in the scope JSON (`run-epic-scope-resolver.sh:769`) **plus** a `git show-ref` branch probe over `refs/heads/` and `refs/remotes/origin/`, because the scope JSON lists no branches and a fast-track branch may be unpushed and have no PR yet | The resolved scope JSON that `run-bounded-prelude.sh` writes to `$scope_file` (`:460`–`:481`), whose item object already carries `status` and `type` (`:766`), plus the next-action call for the folder | Same script with `--caller single`, `--status` / `--type` / `--branch-pr-evidence` from `$scope_file` and `--artifact-stage` from next-action — **no extra tracker API call**. The `items` and `epic` scope modes are **not** wired to this gate; see Out of Scope |
  ...
  - **Resolving the item's development folder (single-item path).** The scope JSON carries no
    ...
    3. **Zero matches** → `--artifact-stage ''`, but that is **not** on its own a decision.
       ...
       `^(feature|fix|refactor|hotfix)/`. The branch probe is not optional here, and it must
       cover `refs/heads/` as well as `refs/remotes/origin/`: the scope JSON lists no branches,
       so without it a fast-track branch that has no PR yet would read as `none`, and an
       origin-only probe would still miss one that has not been pushed. Only when all three are empty does the stop fire — an item with no folder,
...
9g-ii. **`stale-backlog-open-fix-branch-no-pr-continues`** (single-item): the item has a live
    `fix/<issue>-<slug>` branch that exists **only as a local `refs/heads/` ref — not pushed,
    no `refs/remotes/origin/` ref, and no PR at all** — the normal state right after a
    fast-track branch is cut. The scope JSON's `pullRequests` arrays are empty here, so this
    scenario fails unless the caller runs the `git show-ref` branch probe, and it fails again
    if that probe looks only at `refs/remotes/origin/`. Expected `RESULT=pass`,
    `REASON=branch_or_pr_in_flight`.
9g-ii-b. **`stale-backlog-pushed-fix-branch-no-pr-continues`** (single-item): the same item
    with the branch present **only** as `refs/remotes/origin/fix/<issue>-<slug>` and no local
    ref (the state on a fresh clone, or after the local branch is deleted post-push). Same
    expected outcome. The pair pins both namespaces: neither fixture may be satisfied by a
    probe that reads only the other.
...
| Evidence source sees only one kind of work | A single source misses the others: the scope JSON lists no branches (an open `fix/` branch with no PR would read `none`), and `open_implementation_pr_metadata` lists only open PRs (a merged implementation PR would read `none`). Each caller must therefore probe all three kinds, and the branch probe must cover local **and** remote refs — an origin-only probe misses a branch that has been cut but not pushed. `stale-backlog-open-fix-branch-no-pr-continues` (local-only ref), `stale-backlog-pushed-fix-branch-no-pr-continues` (origin-only ref) and `scan-merged-implementation-pr-continues` fail if any of those blind spots returns |
...
| Effective-stage inputs | Three: tracker status, artifact stage, and branch/PR evidence. The evidence itself has **three kinds** — live branch, open PR, merged PR — and every caller covers all three: single-item uses `git show-ref` over `refs/heads/` **and** `refs/remotes/origin/` plus the scope JSON's `pullRequests.open[]`/`.merged[]`; the scan uses the same two-namespace `git show-ref`, `open_implementation_pr_metadata` (open only) and the merged-PR probe. `present` outranks `unavailable`; the gate holds only when all three inputs say Backlog-with-no-work |
