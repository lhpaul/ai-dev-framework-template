...
- **Duplicate detection**: A Discussion is considered a duplicate of an existing open issue when the Discussion title and body share 3 or more significant keywords with the issue title or body, or when the affected file paths or protocol names are identical. Keyword matching excludes common stopwords ("the", "a", "is", "in", "to", "of", "and", "or", "for"). When match confidence is ambiguous, the triage runner presents both the potential match and "No strong existing item found" and asks the maintainer to decide.
...
- **Promoted issue format**: Issues created by the triage protocol must follow a standard format (see Acceptance Criteria) and include a link back to the originating Discussion.
