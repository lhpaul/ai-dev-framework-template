...
2. The review loop script queries all review threads via GraphQL `reviewThreads(first: 100) { nodes { id isResolved } }`
...
5. A fixer agent is dispatched; it either replies with a decision rationale and resolves the thread via `resolveReviewThread` mutation, or pushes a code fix to the current HEAD
6. After the fix, the loop re-runs and confirms `isResolved: true` for all previously unresolved threads
...
2. The loop script re-queries all review threads via GraphQL
...
**Preconditions**:
- Step 7 completed with `clean` but the GraphQL thread check was not performed (e.g., from a prior run before this feature shipped)
- One or more review threads on the PR are still `isResolved: false`

**Steps**:
1. Agent runs Step 8c independent verification
2. Step 8c queries `reviewThreads(first: 100) { nodes { id isResolved } }` directly
...
- [ ] **AC3**: Protocol 91 Step 8c verification queries `reviewThreads(first: 100) { nodes { id isResolved } }` and rejects the PR (removes `ready-for-human-review`, applies `needs-fixes`) if any automated-reviewer thread is `isResolved: false`.
...
- [ ] **AC7**: Protocol 90 Step 5.1 verification table includes a row for "All automated-reviewer review threads resolved (`isResolved: true`)" as a hard-gate check before declaring a PR ready for human review.
