...
5b. An **early match followed by a long tail**: a changed-file list whose first
    ...
    output exceeds **2 MiB**. The Workflow Policy checklist is still added.

    The size is a hard requirement, not a round number, and the test asserts it
    with `wc -c` on the generated fixture before using it. A pipe's capacity is
    64 KiB by default on Linux and macOS; macOS caps there, and Linux allows up
    to `fs.pipe-max-size`, 1 MiB on a default kernel. Anything below that
    ceiling makes the proof platform-dependent — the planted pipeline would
    pass wherever the buffer happened to swallow the whole list. 2 MiB clears
    the highest supported capacity with margin. The runbook's generator reaches
    2,428,900 bytes at 40,000 paths of the stated shape, measured rather than
    estimated; the test asserts the **byte count**, not the path count, because
    path length is the variable that silently shrinks the fixture when someone
    shortens the generated names.
...
| P10 | Replace the buffered decode with `jq -r '.[]?' \| reviewer_changed_files_touch_workflow_policy` | a scratch copy of the merge | scenario 5b fails under `set -o pipefail`: the predicate returns on the first path, `jq` takes SIGPIPE writing the remaining 2 MiB, the pipeline reports 141, and the checklist is dropped. The proof is deterministic only because 5b asserts its fixture exceeds 2 MiB — above the highest pipe capacity any supported platform allows — so the plant cannot pass by fitting in a buffer. Scenario 5a still passes on its two-path list, which is the point: the plant is invisible to any test that stays under the buffer; restoring the buffered decode passes both |
...
3. Add `reviewer_resolve_review_stage` and call it after `changed_files_json` is
   ...
   `changed_files_json` value, the >2 MiB list that exposes a pipeline decode, all eight rows of the decision table, both
