...
5b. An **early match followed by a long tail**: a changed-file list whose first
    entry is `REVIEW.md` and which then carries 500 further paths — more than a
    pipe buffer holds. The Workflow Policy checklist is still added. Piping
    `jq` straight into the predicate passes this on a three-path list and fails
    here, which is why the length is part of the scenario rather than left to
    the implementer's fixture.
...
| Changed-file fixture | One path per `case` entry — nine — the four non-matching controls of scenario 3, a mixed list for scenario 4, the empty list of scenario 5, and a 501-path list beginning with `REVIEW.md` for scenario 5b, generated by a loop rather than written out | inline in the same suite |
...
| P10 | Replace the buffered decode with `jq -r '.[]?' \| reviewer_changed_files_touch_workflow_policy` | a scratch copy of the merge | scenario 5b fails under `set -o pipefail`: the predicate returns on the first path, `jq` takes SIGPIPE writing the remaining 500, the pipeline reports 141, and the checklist is dropped. Scenario 5a still passes on its short list — the plant is invisible to every test that does not go past the pipe buffer; restoring the buffered decode passes both |
...
3. Add `reviewer_resolve_review_stage` and call it after `changed_files_json` is
   ...
   `changed_files_json` value, the 501-path list that exposes a pipeline decode, all eight rows of the decision table, both
