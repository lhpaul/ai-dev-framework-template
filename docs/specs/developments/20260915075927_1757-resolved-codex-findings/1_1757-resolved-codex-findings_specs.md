...
- Changing Codex GitHub App rate limits, usage limits, polling budgets, or external service availability behavior. The availability outcomes, reason codes, and the usage-limit and account-not-connected immediate-termination contract documented in this spec describe existing shipped behavior that this item preserves; they introduce no behavior change.
...
blocker outranks a usage-limit or account-not-connected availability hard stop,
which in turn outranks the cleared-findings wait and clean; an availability
response never outranks an escalation or an actionable blocker, and a retained
environment-setup response yields only when strictly newer terminal or review
evidence exists. The decision-gate matrix below lists rows in
...
| A genuine usage-limit notice or account-not-connected refusal is present in the same fetch as a clean or otherwise superseding review verdict, and no blocking or `CHANGES_REQUESTED` evidence is present | Unavailable, with the shipped availability reason code | Terminate the current run immediately with that availability outcome; the notice is never superseded by newer or same-fetch clean evidence | Protocol 93 immediate-termination contract, `codex-github-reviewer.sh` immediate-precedence block, reviewer loop | Codex returns a usage-limit root response and a clean review in the same poll, and the loop still reports unavailable |
