...
scope only when it contains exactly one hexadecimal commit token that is an
unambiguous prefix of the live head. A root comment also must be authored after
...
When no such trigger exists, the first marker-pinned clean root comment for that
head may be canonical only when its marker is valid. Empty, non-hex, ambiguous,
multiple-token, non-prefix, or superstring marker matches are invalid revision
evidence and escalate with the malformed-marker reason, including when no
...
| Root pull-request comment evidence for the live head has a `Reviewed commit` marker that is empty, non-hex, ambiguous, contains multiple commit tokens, does not prefix-match the live head, or only matches by substring or superstring; no newer valid terminal evidence covers the same live head | `escalate` with reason `codex_current_verdict_malformed_revision_marker` | Stop the current run for human review; do not wait, claim clean, or infer an older-revision verdict from invalid marker evidence | Codex review adapter, reviewer loop, PR summary | A root comment says `Reviewed commit: abcdef1` when another live-head candidate shares that prefix, or embeds the live OID inside a longer token |
