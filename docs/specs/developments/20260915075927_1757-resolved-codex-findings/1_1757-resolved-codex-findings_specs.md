...
- [ ] A current terminal Codex verdict that matches neither an approved clean template nor the documented blocking markers, is not a response the shipped adapter recognizes as an availability outcome, and is not a `CHANGES_REQUESTED` submitted review, is recorded as `codex_current_verdict_unrecognized`; a recognized availability response keeps its shipped availability handling instead, a `CHANGES_REQUESTED` review is recorded as an actionable blocker instead; and a current terminal Codex finding with no stable review-thread identifier or no identifiable matching conversation is recorded as `codex_finding_thread_correlation_missing`. For each, the loop stops for human review, preserves the shipped availability path, or returns `needs_fixes` respectively, instead of waiting, returning `needs_fixes`, or claiming clean.
