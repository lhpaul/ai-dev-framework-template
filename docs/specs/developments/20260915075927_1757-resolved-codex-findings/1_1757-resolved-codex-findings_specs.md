...
- [ ] A current terminal Codex verdict that matches neither an approved clean template nor the documented blocking markers, and is not a response the shipped adapter recognizes as an availability outcome, is recorded as `codex_current_verdict_unrecognized`; a recognized availability response keeps its shipped availability handling instead; and a current terminal Codex finding with no stable review-thread identifier or no identifiable matching conversation is recorded as `codex_finding_thread_correlation_missing`. For each, the loop stops for human review or preserves the shipped availability path respectively, instead of waiting, returning `needs_fixes`, or claiming clean.
...
| A current-head submitted Codex review carries GitHub review state `CHANGES_REQUESTED` and contains no finding that is unthreaded or uncorrelated | Actionable blocker | Return `needs_fixes` and route to the fix loop; never approve from body wording, and do not record it as unrecognized or clean | Protocol 93 verdict classification, `codex-github-reviewer.sh` structured-state short-circuit, reviewer loop, PR summary | Codex submits a `CHANGES_REQUESTED` review whose body happens to reproduce a clean template; the structured state still blocks readiness |
