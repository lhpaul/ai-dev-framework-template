...
- After a pull-request update, a clean readiness path requires terminal Codex evidence for the live revision: either a submitted Codex review whose full commit SHA equals the live head and whose body is classified clean, or a Codex root pull-request comment authored after the latest review trigger that names the live head and whose `Reviewed commit` marker contains exactly one full 40-character commit OID equal to the live head and whose body is classified clean. A root comment with the same timestamp second as that trigger is fresh only when its comment ID orders after the trigger comment; when no live-head review trigger exists yet, the first marker-pinned clean root comment for that head may serve as terminal clean evidence. Clean or finding evidence from an older revision, and a root comment without the required freshness boundary, exact full-OID marker, and clean body, are stale or incomplete. Empty, abbreviated, malformed, or superstring marker matches are invalid revision evidence and escalate rather than authorizing readiness.
...
3. Require terminal clean Codex evidence for the live revision after a push: a submitted review with the full live commit SHA and a clean body, or a fresh Codex root comment whose `Reviewed commit` marker contains exactly one matching full 40-character OID and whose body is clean.
...
live head. A root comment also must be authored after the latest review trigger
that names the live head to authorize readiness. If GitHub records the trigger
and root comment in the same timestamp second, the root comment is fresh only
when its comment ID orders after the trigger comment. When no such trigger
exists, the first marker-pinned clean root comment for that head may be canonical.
Empty, abbreviated, malformed, or superstring marker matches are invalid
revision evidence and escalate with the malformed-marker reason. Even with a
valid and fresh marker, the comment body must still be classified clean, and a
