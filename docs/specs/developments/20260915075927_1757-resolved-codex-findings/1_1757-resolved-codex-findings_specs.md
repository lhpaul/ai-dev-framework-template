...
- [ ] A syntactically-unusable-marker root comment escalates the live head with `codex_current_verdict_malformed_revision_marker` only when it falls inside that head's evidence window; a malformed comment authored while an earlier head was current is stale-head evidence that never escalates, waits on, or authorizes readiness for a later head.
