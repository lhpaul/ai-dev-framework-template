...
- [ ] Per-run and lifetime cycle-limit outcomes are recorded as explicit escalations and never as clean or ready outcomes. Omitting the settings keeps the documented 10/25 defaults; only an explicitly invalid value escalates as a configuration error.
...
head may be canonical only when its marker is valid. Marker evidence splits into
...
**malformed marker** — empty, non-hex, multiple-token, ambiguous, substring-only,
or superstring-only — is syntactically unusable revision evidence: it escalates
...
| Root pull-request comment evidence has a `Reviewed commit` marker that is syntactically unusable: empty, non-hex, contains multiple commit tokens, ambiguous between candidates, or only matches the live head by substring or superstring; no newer valid terminal evidence covers the same live head | `escalate` with reason `codex_current_verdict_malformed_revision_marker` | Stop the current run for human review; do not wait, claim clean, or infer an older-revision verdict from malformed marker evidence | Codex review adapter, reviewer loop, PR summary | A root comment says `Reviewed commit: abcdef1` when another candidate shares that prefix, or embeds the live OID inside a longer token |
