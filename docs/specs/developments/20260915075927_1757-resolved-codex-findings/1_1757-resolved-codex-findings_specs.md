...
- After a pull-request update, a clean readiness path requires terminal Codex evidence for the live revision: either a submitted Codex review whose commit SHA equals the live head, or a Codex root pull-request comment whose `Reviewed commit` marker contains that same head SHA. Clean or finding evidence from an older revision, and a root comment without that exact marker, are stale or incomplete.
...
- [ ] After a pull-request update, the workflow requires terminal Codex evidence for the live revision before permitting a clean readiness path: a submitted review with that commit SHA or a Codex root pull-request comment whose `Reviewed commit` marker contains it.
...
3. Require terminal Codex evidence for the live revision after a push: a submitted review with the live commit SHA or a Codex root comment whose `Reviewed commit` marker contains that SHA.
...
between waiting and routing a fixer.
...
| No unresolved current conversation; terminal clean evidence covers the live revision as either a submitted Codex review with the live commit SHA or a Codex root pull-request comment whose `Reviewed commit` marker contains that SHA | Clean current evidence | Continue to readiness | Codex review adapter, reviewer loop, readiness checks | A current submitted clean review or marker-pinned root comment authorizes the Codex phase |
