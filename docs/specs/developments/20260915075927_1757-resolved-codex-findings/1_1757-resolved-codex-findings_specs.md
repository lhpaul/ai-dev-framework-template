...
- Acknowledgement-only evidence for the live-head review attempt — a thumbs-up reaction, a draft review, or a clean-looking root comment that omits any explicit reviewed revision — is not terminal evidence. When no unresolved current conversation exists and no terminal evidence covers the live revision, the loop records `waiting_on_reviewer` with reason `codex-github-reaction-without-review` and awaits or requests terminal evidence for the unchanged head; it does not claim clean and does not dispatch a fixer.
...
- [ ] When no unresolved current conversation exists and the live-head review attempt shows only acknowledgement evidence — a reaction, a draft review, or a clean-looking root comment without an explicit reviewed revision — the loop records `waiting_on_reviewer` with reason `codex-github-reaction-without-review` and awaits terminal evidence instead of claiming clean.
...
9. Treat acknowledgement-only evidence and trigger-less clean evidence conservatively: reaction-only evidence waits with `codex-github-reaction-without-review`, and a trigger-less clean comment is superseded by newer non-dismissed terminal evidence for the same head.
...
| Add resolved-finding regression coverage | Acceptance criterion 11; Operational Visibility |
| Share the invariant without rate-limit coupling | Acceptance criterion 12; Out of Scope |
...
| Conservative acknowledgement and trigger-less evidence | Acceptance criteria 9-10; Business Rules 8, 13 |
...
exists for that head, the head follows the `codex-github-review-pending` wait path.
...
| Successful bounded evidence query; every actionable finding in the current terminal verdict has an identifiable matching Codex review conversation for the live head, every matching conversation is resolved, and no other applicable current-head Codex conversation remains unresolved | `waiting_on_reviewer` with reason `codex-github-review-pending` | Request or await a fresh current-head Codex review through the cleared-findings retrigger path; do not dispatch a fixer, claim clean, or escalate correlation-missing | Protocol 93, `codex-github.md`, reviewer loop, readiness checks | An operator resolves all Codex findings as false positives without changing HEAD and no other current-head Codex thread is open, so the next action is to obtain the clean verdict for that same head |
