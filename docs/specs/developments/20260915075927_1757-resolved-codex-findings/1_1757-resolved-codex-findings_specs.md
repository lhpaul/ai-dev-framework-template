...
- After a pull-request update, a clean readiness path requires terminal Codex evidence for the live revision: either a submitted Codex review whose commit SHA equals the live head and whose body is classified clean, or a Codex root pull-request comment whose `Reviewed commit` marker contains that same head SHA and whose body is classified clean. Clean or finding evidence from an older revision, and a root comment without both the exact marker and a clean body, are stale or incomplete.
...
3. Require terminal clean Codex evidence for the live revision after a push: a submitted review with the live commit SHA and a clean body, or a Codex root comment whose `Reviewed commit` marker contains that SHA and whose body is clean.
...
scope only; the comment body must still be classified clean, and a marker-pinned
root comment with recognizable unthreaded findings follows the incomplete-finding
escalation path.
...
| No unresolved current conversation; terminal clean evidence covers the live revision as either a submitted Codex review with the live commit SHA and a clean body or a Codex root pull-request comment whose `Reviewed commit` marker contains that SHA and whose body is clean | Clean current evidence | Continue to readiness | Codex review adapter, reviewer loop, readiness checks | A current submitted clean review or marker-pinned clean root comment authorizes the Codex phase |
| No unresolved current conversation; otherwise clean-looking or acknowledgement evidence has no submitted review marker and no explicit reviewed SHA | `waiting_on_reviewer` with reason `codex_current_verdict_unsubmitted` | Await or request terminal evidence for the unchanged head; do not claim clean | Codex review adapter, reviewer loop, readiness checks | A draft review is visible or a root clean comment omits any reviewed SHA |
| No Codex conversations, or only resolved or unresolved historic conversations; terminal verdict is absent, or the newest submitted or SHA-marked terminal evidence covers an earlier revision | `waiting_on_reviewer` with reason `codex_current_verdict_pending` | Request or await a submitted current review for the unchanged head; do not dispatch a fixer or claim clean | Codex review adapter, reviewer loop, readiness checks | A newly opened pull request has no Codex review yet, a push occurs after the last Codex review, or a root clean comment names an older SHA |
