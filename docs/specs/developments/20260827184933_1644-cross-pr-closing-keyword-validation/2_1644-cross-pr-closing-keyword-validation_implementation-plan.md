...
- **Shared mutable state guards**: the only shared mutable state is the published report comment and the check run for one pull request. The comment is located by the `closing-keyword-scope:v1` marker and updated in place, so a run never creates a second one; concurrent writes are guarded by the workflow's per-PR concurrency group plus the script's own ordering stamp.
