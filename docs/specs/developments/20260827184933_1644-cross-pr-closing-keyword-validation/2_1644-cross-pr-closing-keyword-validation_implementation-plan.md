...
| A persistently unreadable check-run listing accumulates neutral checks on one head SHA | Low — it needs a sustained API failure | Low — the checks are non-blocking and age out with the SHA | Not prevented, and the plan says so instead of claiming a bound. The alternative — declining to publish — trades a visible non-conclusion for an invisible one, which is the failure the indeterminate design exists to prevent |
