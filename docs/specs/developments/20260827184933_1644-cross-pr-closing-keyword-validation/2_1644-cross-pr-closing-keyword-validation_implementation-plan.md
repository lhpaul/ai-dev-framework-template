...
- [ ] **The sentinel's value, and what happens when it collides.** The sentinel is `CLOSING-KEYWORD-SCOPE-BOUNDARY-<nonce>` alone on its line, where `<nonce>` is 32 hex characters generated per invocation. A fixed string would be forgeable: a title or description containing it would move the boundary, and the run would report keywords from the title or suppress keywords from the description — parser-critical behaviour decided by whoever wrote the pull request. The generator is named rather than left to the implementer, because there is no reusable one in this repository to point at — `auto-tag-release.yml` uses `dd`/`base64`, which produces neither hex nor 32 characters. The library defines:

  ```bash
  closing_keyword_scope_new_nonce() {
  ...
  Unpredictable is not the same as impossible, so the collision is also **detected rather than assumed away**: before filtering, the run counts occurrences of the sentinel line in `title + "\n" + sentinel + "\n" + description`. Exactly one is the expected case. More than one means the text already contained it; the run generates a fresh nonce and retries **once**. A second collision is treated as an unreadable input, making the run indeterminate — it leaves any existing report untouched and writes the neutral check — rather than reporting from a boundary it cannot trust. Zero occurrences after insertion is impossible and is a defect, not an input condition; it stops the run the same way.

  After filtering, a surviving sentinel must still appear exactly once. More than one survivor would mean the filter duplicated text, which it does not do, so that too is indeterminate rather than a guess. *Acceptance criteria: The indeterminate outcome; the title-fence rows.*
...
| The indeterminate outcome | Single-accessor input reads; check-run identity; sentinel collision detection | `unreadable_description_leaves_report`, `unreadable_pr_list_leaves_report`, `unreadable_label_is_indeterminate`, `unreadable_base_is_indeterminate`, `unreadable_existing_report_is_indeterminate`, `unreadable_head_branch_is_indeterminate`, `unreadable_check_run_list_is_indeterminate`, `unreadable_check_run_list_still_publishes_neutral_check`, `unreadable_check_run_list_posts_no_second_report`, `indeterminate_writes_neutral_check_and_log`, `indeterminate_does_not_change_mergeability`, `conclusive_rerun_updates_same_check_run`, `silent_conclusive_rerun_replaces_neutral_check`, `inherited_duplicate_check_runs_update_the_oldest`, `earlier_run_does_not_overwrite_later_check_run`, `unparseable_check_run_stamp_is_adopted`, `repeated_unreadable_listings_each_add_a_neutral_check`, `sentinel_collision_retries_once_with_a_fresh_nonce`, `second_sentinel_collision_is_indeterminate`, `duplicate_surviving_sentinel_is_indeterminate`; runbook Step 7 covers only that the check publishes and does not block; the indeterminate path itself is unit-only, for the reason the runbook states |
