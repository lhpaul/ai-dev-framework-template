...
- No rule requires tooling that does not exist. Each check is performed by a reader holding the plan, the repository at the recorded revision, and a search tool, with the one narrow exception Group A names for a cited protected artifact under Rule 1.
...
| A plan enumerates a population in place of sampling it, but the population is not finite and closed, no cited provenance establishes that closure, or no single command reproduces its complete current membership | Blocking finding       | Names the population and why the escape hatch does not apply  | Sample real occurrences and record the sampling, or cite closure provenance and show the population is finite and closed |
...
| A design matches against a partner service's webhook payloads for an integration being decommissioned; the plan cites the partner's decommissioning notice fixing the channel's shutoff date as closure provenance, and the only source of these payloads is a fixed, versioned archive of every payload it ever sent before that date. A single directory listing over the archive reproduces its complete current membership, and the plan states what the design does if a payload outside the archive is later discovered | Check passed | The cited decommissioning notice establishes the archive's population as finite and closed; a directory listing alone, without that cited provenance, would prove only the archive's present contents, not that no more payloads can arrive. With no contract cited fixing the output format, the plan still states behavior beyond the archived set, exactly as Rule 1 requires when no contract fixes the set |
