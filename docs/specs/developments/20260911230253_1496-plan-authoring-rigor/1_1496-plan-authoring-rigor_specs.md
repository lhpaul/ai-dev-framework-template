...
- When the sampling record states that further occurrences had not stopped yielding new variants, the plan does not name the full set of observed variants as the stable part. It names a narrower part instead — a field, delimiter, position, or structural boundary present in every occurrence examined — and treats every variant value, seen or unseen, as subject to change. A design that relies on an unsaturated sample's enumerated values as if they were the complete or stable set does not satisfy this rule, however many occurrences were examined.
...
| A design with no cited contract names an unsaturated sample's full set of observed variant values as its stable part | Blocking finding       | Names the record's saturation state and the design's over-broad stable-part claim | Name a narrower, independently justified stable part, or continue sampling toward saturation |
...
| The same design, with dozens of real occurrences sampled, saturation not reached, the stable part named as a structural boundary present in every reply rather than the observed reply text itself, and behavior on an unseen reply stated | Check passed         | The set is open, the design says so, and the stable part it relies on does not depend on which variants were actually observed |
| The same sampling, but the design instead treats every reply text observed in the sample as the complete set the system must recognize | Blocking finding     | The sample is not saturated; treating its observed variants as complete or stable does not satisfy the rule, however many occurrences were examined |
...
- [ ] When the sampling record shows saturation was not reached, the stated stable part passes only when it is narrower than the full set of observed variant values — a field, delimiter, position, or structural boundary present in every occurrence — rather than the enumerated values themselves. A plan that names an unsaturated sample's observed values as the stable part fails.
