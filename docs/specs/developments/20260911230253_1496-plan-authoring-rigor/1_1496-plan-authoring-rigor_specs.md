...
- A locator that points only to a source known to be short-retention, deletable, or access-restricted does not satisfy this rule unless the record also embeds the occurrence's own text; a locator's value is judged by whether a later reader can still use it, not by whether one existed at capture time.
...
| A sampling record's locator points only to a short-retention, deletable, or access-restricted source, with no embedded occurrence text | Blocking finding       | Names the locator and the durability gap                       | Embed the occurrence's own text, or cite a durable locator      |
...
- [ ] A locator pointing only to a short-retention, deletable, or access-restricted source passes only when the record also embeds the occurrence's own text. A locator that later readers cannot use fails, regardless of whether it resolved at capture time.
