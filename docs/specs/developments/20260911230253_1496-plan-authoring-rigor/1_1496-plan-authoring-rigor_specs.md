...
- No rule requires tooling that does not exist. Each check is performed by a reader holding the plan, the repository at the recorded revision, the per-rule outcome record on the plan pull request, and a search tool, with the one narrow exception Group A names for a cited protected artifact under Rule 1.
...
- [ ] Every check can be performed with the plan, the repository at the recorded revision, the per-rule outcome record on the plan pull request, and a search tool, with one narrow, explicitly-scoped exception: verifying a sampling record's cited durable protected artifact under Rule 1 requires access to the protected system it names, because an access-restricted source cannot be captured into the plan or the repository without defeating the access control or retention policy it exists to enforce. That exception applies only to this one evidence element and to no other check in this feature. No check names a tool that would have to be built first.
