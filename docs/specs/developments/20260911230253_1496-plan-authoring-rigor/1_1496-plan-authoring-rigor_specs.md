...
- **Re-verifying evidence against the repository's current state, or against a cited external source's current state, on every review round.** Repository-derived evidence continues to be judged against the revision it was recorded at, Rule 1's sampling records against the population and window they cover, and Rule 1's enumeration records against their cited closure provenance and recorded command — not against the repository's state at review time. The same snapshot model applies to every external source Rule 1's Group A exception permits evidence to reference; none is required to carry an immutable or versioned locator, and none is re-inspected against its own current state on a later round with no corresponding plan-text change. This is a deliberate rejection rather than a deferral — see the deferral notes below.
...
Known gap deferred to plan stage — recorded, not resolved here.

1. Rule 4 requires a reproducible search recorded in the plan before a claim that something exists, does not exist, or is already covered or already handled enters the plan (Business Rules, Rule 4; Use Case 3; Group E). A reproducible search proves that matching text or a candidate artifact is present at the searched location. It does not, by itself, prove that the matched artifact executes on the path the "handled" or "already covered" claim relies on, or that it actually produces the claimed behavior — a keyword or file-name match can be present and unreachable, dead, superseded, or behaviorally unrelated to the concern the plan names. Rule 4 as stated in this feature accepts a direct search as sufficient support for a "handled" or "already covered" claim, the same evidentiary standard it applies to a plain existence claim; this feature does not distinguish the two. Whether a "handled" or "already covered" claim should instead require stronger evidence — for example, a passing test that exercises the matched artifact on the claimed path, or a traced call path from the concern's trigger to the matched code — is an open, genuinely new evidentiary-depth question. It is not a consequence of any of the confirmed decisions on issue #1496, and it is deliberately left open here rather than patched with a partial rule: the implementation-plan stage must decide whether Rule 4 needs a second, stricter evidentiary tier for "handled" claims specifically, or whether the existence-level standard is accepted as sufficient for this feature's scope, and record that decision where the plan states its approach to Rule 4.
