...
- A locator that points only to a source known to be short-retention or deletable does not satisfy this rule unless the record also embeds the occurrence's own text; a locator's value is judged by whether a later reader can still use it, not by whether one existed at capture time. When the source is access-restricted because it carries secrets, PII, or customer data, the record embeds an appropriately redacted capture that preserves the output's relevant shape instead of the raw occurrence, or cites a durable protected artifact with documented access and retention guarantees. This rule never requires copying protected content into the plan unredacted.
...
- A sampling record that has not reached saturation states why the plan considers the observed sample adequate to the design being proposed, given the population and window it covers. A record that reports non-saturation and states no such rationale does not satisfy this rule — the same standard this feature applies everywhere else a rationale is required: silence is not a rationale.
...
| A sampling record reports non-saturation with no stated rationale for the sample's adequacy      | Blocking finding       | Names the record and the missing adequacy rationale            | State why the sample is adequate, or continue sampling toward saturation |
...
| A design matches against a partner service's webhook payloads for an integration being decommissioned; the channel has been shut off, and the only source of these payloads is a fixed, versioned archive of every payload it ever sent before shutoff. A single directory listing over the archive reproduces its complete membership, and the plan states what the design does if a payload outside the archive is later discovered | Check passed | The channel is permanently closed, so the archive's population is finite and closed by that fact, not by a directory listing alone or by a promise about the partner's future output; with no contract cited fixing the output format, the plan still states behavior beyond the archived set, exactly as Rule 1 requires when no contract fixes the set |
| The same design, applied instead to a live, still-operating partner integration, and described as covering "the common payload shapes seen so far," with no command that lists the complete set and no closure event that would make the population provably finite | Blocking finding     | A live, ongoing integration is not closed, and a directory of what was captured so far proves only that directory's own membership, not that the partner's real output space cannot produce more; the escape hatch requires the population itself to be finite and closed, not merely a captured subset called complete |
...
- [ ] A sampling record that reports non-saturation passes only when the plan states why the observed sample is adequate to the design being proposed. A record that reports non-saturation with no stated rationale fails.
