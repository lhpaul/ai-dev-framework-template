...
| A design matches against a partner service's webhook payloads, all of which are captured in a fixed, versioned set of fixture files, a single directory listing reproduces the complete set, and the plan states what the design does if a payload outside that set ever arrives | Check passed | The population is finite, closed, and one recorded command reproduces its complete membership; with no producer contract cited, the plan still states behavior beyond the enumerated set, exactly as Rule 1 requires when no contract fixes the set |
