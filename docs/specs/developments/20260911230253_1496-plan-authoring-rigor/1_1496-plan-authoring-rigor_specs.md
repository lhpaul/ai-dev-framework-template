...
- Unsatisfied → Satisfied when a later plan revision supplies the required evidence for a claim that still fires the rule, or corrects an outcome record wrongly labeled Unsatisfied whose evidence in the plan was already complete, reproducible, and satisfying — with no new evidence needed — and the outcome is determined again.
- Unsatisfied → Not applicable when a later plan revision removes the claim that made the rule fire, or supplies the rationale a Not-applicable record recorded with no rationale was missing, with no claim to remove because the rule never fired, and the record states the rationale this outcome requires.
