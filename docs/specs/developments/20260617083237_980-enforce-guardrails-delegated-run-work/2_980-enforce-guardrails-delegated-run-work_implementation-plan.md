...
| Delegated gate already consumes a policy + evidence object | read `scripts/development-workflow/run-epic-delegated-gate.sh` (lines ~98–179) | Gate reads `policy.delegateReview`, `policy.mayMerge`, `policy.mayStartBacklog`, plus `pr`/`risk`/`reviewer`/`statusChecks` evidence and emits `merge_allowed` / `fix_required` / `human_required` / `blocked`. The guardrails config maps directly onto these fields — no gate change needed. |
| Risk classifier interface | `sed -n '60,75p' docs/workflow/development-workflow/protocols/95-run-epic-protocol.md` | `run-epic-risk-classifier.sh --pr <n> --max-risk <low\|medium\|high>` returns `merge_permitted`; risk scale `low\|medium\|high` matches the #979 model exactly. |
| Audit helpers and stable markers | `sed -n '76,97p' docs/workflow/development-workflow/protocols/95-run-epic-protocol.md` | `run-epic-audit-trail.sh render/apply pr-disposition` (marker `<!-- run-epic:pr-disposition -->`) and `render/apply epic-ledger` (marker `<!-- run-epic:epic-ledger -->`) already exist; reruns update existing markers. Reused for audit-evidence recording. |
...
| README index location for the new reference page | `sed -n '527,565p' docs/workflow/development-workflow/README.md` | "Core Protocols" lists protocols; "Tooling And Configuration" lists `.ai-dev-workflow.yaml` and helpers — the new `guardrails-enforcement.md` belongs in "Tooling And Configuration" next to the #979 `guardrails.md` reference. |
