...
| A7 | Delivered test scope is equal to or larger than projected | Any | Not required | Not applicable | No finding from this gate | Ordinary Pass 2 quality review still applies |
...
| A reviewer uses "no harm statement available" to wave through a real coverage gap | Med | High | A missing record is `important` (row A4), binding enumerations stay blocking (A5/A6), and the unchanged rules — planted-violation proof, E2E fixture contract, filter-schema canary, scope-residual evidence — continue to require their specific coverage kinds regardless of counts |
