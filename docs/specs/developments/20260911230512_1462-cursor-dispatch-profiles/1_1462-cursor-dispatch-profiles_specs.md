...
- A declared profile that contradicts a handoff fact already known to the run at the point of declaration — declaring native handoff while initial handoff is already known unavailable, declaring native handoff while onward-handoff capability is already known unavailable, or declaring parent-orchestrated while initial handoff is already known unavailable — does not put that declared profile in force. The profile-selection Business Rule requires the declaration to be informed by the run's own known facts, so a declaration contradicting them is not an informed declaration; it is treated as a missing declaration for that checkpoint, exactly as an unrecognized profile value is. The run stops (or, for a read-only checkpoint, stops before reporting) with the `dispatch_profile_declaration_missing` named stop condition, reporting which known fact the declaration contradicted, rather than proceeding under the contradicted profile or inventing acceptance or correction semantics.
...
| Declared profile is native handoff, but initial handoff is already known unavailable at the point of declaration (onward-handoff capability is therefore also unevaluated, per the evaluation order) | None in force (treated as a missing declaration) | Stop before mutating, or before reporting for a read-only checkpoint, with the `dispatch_profile_declaration_missing` named stop condition, the affected work item, and the known fact the declaration contradicted | Proceeding under the declared profile; treating the generic no-initial-handoff rows below as inapplicable because a (contradicted) profile was declared; inventing correction or acceptance semantics instead of stopping |
| Declared profile is native handoff, but onward-handoff capability is already known unavailable at the point of declaration | None in force (treated as a missing declaration) | Stop before mutating, or before reporting for a read-only checkpoint, with the `dispatch_profile_declaration_missing` named stop condition, the affected work item, and the known fact the declaration contradicted | Proceeding under the declared profile; treating a known fact as irrelevant to declaration validity; inventing correction or acceptance semantics instead of stopping |
| Declared profile is parent-orchestrated, but initial handoff is already known unavailable at the point of declaration | None in force (treated as a missing declaration) | Stop before mutating, or before reporting for a read-only checkpoint, with the `dispatch_profile_declaration_missing` named stop condition, the affected work item, and the known fact the declaration contradicted | Proceeding under the declared profile; treating a known fact as irrelevant to declaration validity; inventing correction or acceptance semantics instead of stopping |
...
| --- | --- | --- | --- |
| Declaration missing at the first mutating action; declared profile value outside the three defined profiles; declared profile with no accountable orchestrator role named; declaration missing, invalid, or missing an accountable orchestrator role at a read-only checkpoint — a portfolio scan under any profile (checkpoint: before the scan reports its findings), or any single-item, multi-item, or epic command operating under the inline-fallback profile before it stops or completes (checkpoint: before that run's next read-only action or report); a declared posture that does not match the run's checkpoint (`observing` declared at a mutating action, or `absorbed`/`handed off intact` declared at a read-only checkpoint); a declared profile that contradicts a handoff fact already known at declaration (native handoff declared while initial handoff or onward-handoff capability is already known unavailable, or parent-orchestrated declared while initial handoff is already known unavailable) | `dispatch_profile_declaration_missing` | The branch, pull request, or development-folder path the bounded run was invoked against; for a read-only portfolio scan, the scan invocation itself, since the declaration is checked before any candidate item is resolved | Supply a valid declaration before the run is allowed to continue past this point: one of the three defined profile values; a named accountable orchestrator role; a posture appropriate to the run's checkpoint (absorbed or handed off intact for a mutating action, observing for a read-only checkpoint); and, when the prior declaration was rejected for contradicting a known handoff fact, a profile consistent with that fact — inline-fallback when initial handoff is the unavailable fact, or parent-orchestrated when only onward-handoff capability is the unavailable fact |
