...
- [ ] An operator who has never run the workflow under a constrained environment can reach the canonical guidance from any bounded command entrypoint or orchestration role document, and can run a single item, an explicit multi-item batch, and an epic to a terminal condition using the documentation alone, with no gate dropped, no human rescue, and no invented scheduling behavior for the explicit multi-item batch beyond the two dispatch paths `90-batch-orchestrate-work-protocol.md` Step 4 already defines: parallel Work Item Runner dispatch when the runner supports true concurrent subagents, or one-item-at-a-time processing in the current context following Protocol 91 when it does not. The parent-orchestrated profile at the portfolio layer is declared precisely when the latter path applies (onward handoff to a per-item Work Item Runner is unavailable), and selects between Protocol 90's existing paths rather than adding a third.
