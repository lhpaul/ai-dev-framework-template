...
- [ ] An operator who has never run the workflow under a constrained environment can reach the canonical guidance from any bounded command entrypoint or orchestration role document, and can run a single item, an explicit multi-item batch, and an epic to a terminal condition using the documentation alone, with no gate dropped, no human rescue, and no unexplained change to the explicit multi-item batch's existing parallel-dispatch behavior under `90-batch-orchestrate-work-protocol.md` (the parent-orchestrated profile changes who performs the portfolio-supervisory function, not whether items are dispatched and supervised concurrently).
...
> Resolved — this note closes the **former** Open Question 3 (batch concurrency: under the parent-orchestrated profile, one context is accountable for the portfolio layer for an entire explicit multi-item batch — should such a batch still run its items concurrently, or run one at a time, given that a single context is supervising every item itself?): choosing the parent-orchestrated profile changes who performs the portfolio-supervisory function for an explicit multi-item batch — the current context that absorbed the portfolio layer, instead of a separately dispatched portfolio orchestrator context — and does not relax `90-batch-orchestrate-work-protocol.md` Step 4's existing parallel-dispatch requirement: launch the full batch in parallel when the runner supports true concurrent subagents, and fall back to serial execution only when it does not. This follows directly from the Business Rule that choosing a profile never relaxes an existing guardrail, gate, stop condition, permission requirement, or human decision point, and determines who performs the work rather than whether the rules apply. Under parent-orchestrated, the absorbing context dispatches and supervises each item's stage work concurrently exactly as Protocol 90 already requires, from within the same context that also performs the absorbed portfolio-orchestration decisions; runner-capability-based serialization (a runner that cannot execute Work Item Runners concurrently) applies exactly as it does under native handoff and is not introduced or expanded by this profile.
