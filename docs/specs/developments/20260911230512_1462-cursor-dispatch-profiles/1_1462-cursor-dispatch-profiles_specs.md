...
| `cursor-inline-fallback`     | Inline fallback     | No handoff is available for the current mutating action — either because no handoff of any kind is available to the run at all, or because, under parent-orchestrated, the specific stage handoff the current action requires has become unavailable even though the current context's own initial handoff remains available for other actions. The run is read-only: it reports what it can determine and stops before any mutating action.                                       |
...
| BO-2 | AC4, AC19 | Full-contract assumption under parent-orchestrated, no inline authorship/review authority, and the carve-out against other documents' inline-fix provisions. |
...
All three open questions raised during this spec's review cycles are resolved; the numbered list is empty and the notes below are retained for traceability, each identified by its original number from the first draft.

> Resolved — this note closes the **former** Open Question 2 (Cursor Cloud Agents' profile-per-layer assignment), not the former Open Question 3 (batch concurrency), which is resolved separately below: the former question is answered by the new Business Rule on unobserved environments and by Acceptance Criterion 15 — the agent model configuration document records Cloud Agents' assignment as an explicit assumption under the same conservative-default pattern used elsewhere, unless and until it is confirmed by observation, rather than leaving it undecided.
...
> Resolved — this note closes the **former** Open Question 3 (batch concurrency: under the parent-orchestrated profile, one context is accountable for the portfolio layer for an entire explicit multi-item batch — should such a batch still run its items concurrently, or run one at a time?): the parent-orchestrated profile is declared for the portfolio layer precisely because onward handoff is unavailable there — the current context cannot hand orchestration to a separate portfolio-orchestrator context that could in turn dispatch a Work Item Runner subagent per item. That is exactly the condition `90-batch-orchestrate-work-protocol.md` Step 4 already names: "If the runner does **not** support Work Item Runner handoff natively, continue in the current session by following `91-orchestrate-work-protocol.md` for each item one at a time." So under parent-orchestrated at the portfolio layer, items are processed **one at a time**, following Protocol 91 in the current context for each: this is not a new rule invented for this profile, it is Protocol 90's own existing no-handoff fallback, and the parent-orchestrated declaration is the named reason that fallback applies. This is independent of Step 4's separate true-concurrent-subagents question ("If the runner supports true concurrent subagents, launch the full batch in parallel"), which governs scheduling only among Work Item Runners actually dispatched under native handoff — onward-handoff availability and concurrent-subagent capability are separate conditions, and this profile's declaration is not a proxy for the latter. This follows directly from the Business Rule that choosing a profile never relaxes an existing guardrail, gate, stop condition, permission requirement, or human decision point — it determines which of Protocol 90's existing dispatch paths governs, not whether either applies.
