...
| Declared profile is native handoff, but onward-handoff capability is already known unavailable at the point of declaration | None in force (treated as a missing declaration) | Stop before mutating with the `dispatch_profile_declaration_missing` named stop condition, the affected work item, and the known fact the declaration contradicted | Proceeding under the declared profile; treating a known fact as irrelevant to declaration validity; inventing correction or acceptance semantics instead of stopping |
