...
- When onward-handoff capability — whether the orchestration role the current context hands off to can hand stage work onward in turn — cannot be confirmed as available, the run treats it as unavailable rather than assuming it works. This is the environment's more conservative capable profile: the run declares parent-orchestrated, not native handoff, until onward-handoff capability is confirmed by a later declaration.
...
| Onward-handoff capability — whether the receiving role can hand stage work onward — cannot be confirmed as available or unavailable | Parent orchestrated (conservative default until confirmed) | Declare the profile as parent-orchestrated, record that onward-handoff capability is unconfirmed, perform the absorbed contract in full, hand off stage work only | Declaring native handoff, or any profile more permissive than parent-orchestrated, based on unconfirmed onward-handoff capability |
...
| Declared profile value does not match one of the three defined profiles                                                    | None in force (treated as a missing declaration)          | Stop before mutating and report the invalid value alongside the three valid profile values                                                    | Assuming a profile and continuing; treating an unrecognized value as any defined profile                   |
| Declared profile has no accountable orchestrator role named, including an empty or blank value                            | None in force (treated as a missing declaration)          | Stop before mutating and report that the accountable role is missing                                                                          | Assuming an accountable role and continuing                                                                |
...
- [ ] The canonical document and its mirror surfaces state the outcome for each of: onward-handoff capability that cannot be confirmed (treated as unavailable, declared parent-orchestrated as the conservative default until confirmed), a declared profile value outside the three defined profiles, and a declared profile with no accountable orchestrator role named — with the latter two stated explicitly as equivalent to a missing declaration that stops the run before mutation, matching the Decision-Gate Consistency Matrix.
