...
- [ ] Invoking the single-item command, the explicit multi-item command, or the epic command produces a declaration of the dispatch profile, the accountable orchestration role, and the role's accountability posture (absorbed or handed off intact, since these commands can mutate) before any mutating action, and a run that reaches a mutating action without one stops with a named reason. Invoking the portfolio-scan command produces the same declaration — always with the observing posture for the named role, since the scan never mutates — before the scan reports its findings, and a scan that reaches that checkpoint without a valid declaration, including one naming no role, stops with the `dispatch_profile_declaration_missing` named stop condition instead of reporting.
