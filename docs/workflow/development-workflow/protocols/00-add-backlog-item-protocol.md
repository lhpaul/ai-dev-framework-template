...
3. For **Linear**, use the Linear API/MCP per [`linear.md`](../integrations/linear.md). `add-backlog-item.sh create` now emits `TRACKER_ACTION_REQUIRED=create_item title=<title>` to stdout and exits 0 (not non-zero) when the configured provider is Linear. The orchestrator must capture this output and use the Linear MCP `createIssue` tool to create the item. Guidance referencing `linear.md` is also written to stderr for operator visibility.
