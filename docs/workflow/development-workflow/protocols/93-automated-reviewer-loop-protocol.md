...
3. **Maximum cycle count**: As specified in `91-orchestrate-work-protocol.md`, escalate when `cycle >= max_cycles` (default: 10). This is a hard limit independent of finding counts. `pr-review-loop.sh` itself also enforces this cap directly (issue #1502): it reads the cumulative cycle count from the PR's persisted `reviewer_loop_history.v1` ledger (not reset when the HEAD SHA changes — see the comment above `reviewer_loop_history_entries_count` in the script for the reset-semantic rationale) and exits `RESULT=escalate` / `REASON=max_cycles_exceeded` on its own once the cap is reached and the loop would otherwise still report `needs_fixes` or `needs_rerun`. The heuristic above remains a useful earlier-warning signal (it can fire before the hard cap is reached), but orchestrators and fixer agents must not rely on their own manual cycle counting as the enforcement mechanism — the script is now the source of truth. Configurable via `PR_REVIEW_LOOP_MAX_CYCLES` or `review.max_cycles` in `.ai-dev-workflow.yaml`.
