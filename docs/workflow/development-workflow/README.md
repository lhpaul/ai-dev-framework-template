...
- `review.on_draft.github` and `review.on_ready.github` are consumed by `scripts/development-workflow/pr-review-loop.sh` for external automated PR review (Step 7). If the config file is absent, or both lists are omitted or empty, automated PR review is treated as not configured and the review loop reports `skipped`. New repositories default ready-phase review to `codex-github`; CodeRabbit (`coderabbit` or `coderabbit-cli`) remains supported when explicitly listed by a repository or local override.
