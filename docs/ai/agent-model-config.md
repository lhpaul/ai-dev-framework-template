...
| `product-manager` | `claude-sonnet-4-6` | Spec writing requires creativity and structured thinking, but the spec template provides strong scaffolding. Sonnet handles this well at a reasonable cost. |
| `spec-reviewer` | `claude-sonnet-4-6` | Review against a checklist. Sonnet is well within the capability required. |
| `tech-lead` | `claude-opus-4-6` | Architecture decisions and implementation planning are the highest-reasoning tasks in the workflow. A weak plan is expensive to fix downstream — this is where Opus pays for itself. |
| `implementation-plan-reviewer` | `claude-sonnet-4-6` | Validating a plan against a spec and codebase is review work; no architectural invention required. |
| `developer` | `claude-sonnet-4-6` | Code generation at scale. Sonnet is the cost/quality sweet spot for coding tasks, especially when it receives a strong spec + plan. |
| `code-reviewer` | `claude-sonnet-4-6` | Code review against known standards and a completed spec. Sonnet is capable here. |
| `project-setup` | `claude-sonnet-4-6` | Structured onboarding conversation with clear protocol guidance. Sonnet is sufficient. |
