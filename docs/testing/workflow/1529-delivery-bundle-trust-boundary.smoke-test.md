...
Then repeat with an evidence file generated by the normal fixture path against
a real `--release-branch` (so `target_binding.release_correlation_key` is the
genuine per-attempt value that branch produces, not a fixed or contract-level
key), then edit **only** the top-level `release_branch` to `""` (or remove the
key) while leaving `target_binding.release_correlation_key` untouched, and pass
a positional release argument that does not correspond to any real branch
(T20c). Do not substitute a fixed/attempt-independent correlation key here: an
empty `release_branch` means cleanup's target re-resolution omits
`--release-branch`, so the freshly resolved target computes a release
correlation key from an empty attempt-branch input, which will disagree with
the genuine per-attempt key already recorded in `target_binding` — this
disagreement is what proves the new `release_branch` guard runs before, not
after, cleanup's `compare_component_field '.release_correlation_key'` check.
