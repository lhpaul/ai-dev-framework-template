...
usage() {
  cat >&2 <<'EOF'
Usage: component-release-evidence.sh --target-file PATH --binding-file PATH --release-outcome OUTCOME --ci-outcome OUTCOME --deployment-outcome OUTCOME --cleanup-outcome OUTCOME --hub-tracker-ref REF [--output PATH] [--json]
EOF
}
