...
query_epic_subissues() {
  ...
  while [ "$has_next" = "true" ]; do
    if ! response="$(gh api graphql \
      ...
      ' 2>/dev/null)"; then
      echo "ERROR: failed to read native sub-issues for epic #${epic_number}." >&2
      exit 1
    fi

    if [ -z "$response" ]; then
      echo "ERROR: empty GraphQL response while reading epic #${epic_number}." >&2
      exit 1
    fi

    printf '%s\n' "$response" | jq -r '.data.repository.issue.subIssues.nodes[]?.number' \
      >> "$subissues_file" || {
        echo "ERROR: failed to parse sub-issues for epic #${epic_number}." >&2
        exit 1
      }

    has_next="$(printf '%s\n' "$response" | jq -r '.data.repository.issue.subIssues.pageInfo.hasNextPage // false')"
    after="$(printf '%s\n' "$response" | jq -r '.data.repository.issue.subIssues.pageInfo.endCursor // ""')"
  done
...
verify_parent() {
  ...
  if ! response="$(gh api graphql \
    ...
    ' 2>/dev/null)"; then
    echo "ERROR: failed to verify parent relationship for issue #${child}." >&2
    exit 1
  fi
  parent="$(printf '%s\n' "$response" | jq -r '.data.repository.issue.parent.number // empty')"
  if [ "$parent" != "$epic_number" ]; then
    echo "ERROR: issue #${child} does not point back to epic #${epic_number}." >&2
    exit 1
  fi
}
...
linked_pr_json() {
  ...
  open_prs="$(gh pr list --state open --limit 1000 --search "$issue" \
    --json number,title,headRefName,baseRefName,isDraft,labels 2>/dev/null \
    | jq --arg issue "$issue" '
        [ .[]
  ...
  merged_prs="$(gh pr list --state merged --limit 1000 --search "$issue" \
    --json number,title,headRefName,baseRefName,mergedAt 2>/dev/null \
    | jq --arg issue "$issue" '
        [ .[]
          | select(.headRefName | test("^(spec|implementation-plan|feature|fix|refactor|hotfix)/" + $issue + "(-|$)"))
          | {
...
dependency_json() {
  ...
  while IFS= read -r dep; do
    ...
    if dep_view="$(gh issue view "$dep" --json number,title,state 2>/dev/null)"; then
      ...
      jq -n --argjson number "$dep" '{number:$number,title:"",state:"UNKNOWN"}' >> "$dep_file"
    fi
  done <<< "$deps"
...
enrich_item() {
  local issue="$1"
  local issue_json title state labels integration_label status type priority pr_json dep_json group body

  if ! issue_json="$(gh issue view "$issue" --json number,title,state,body,labels,projectItems 2>/dev/null)"; then
  ...
  labels="$(printf '%s\n' "$issue_json" | labels_csv)"
  integration_label="$(printf '%s\n' "$issue_json" | integration_label_for_issue_json)"
  status="$(get_tracker_status_for_issue "$issue" 2>/dev/null || true)"
  type="$(get_tracker_type_for_issue "$issue" 2>/dev/null || true)"
  priority="$(printf '%s\n' "$issue_json" | jq -r '.projectItems[0].priority.name // .projectItems[0].priority // ""')"
  pr_json="$(linked_pr_json "$issue")"
  dep_json="$(dependency_json "$body")"

  group="eligible"
  if terminal_status "$status" || [ "$state" = "CLOSED" ] \
    || [ "$(printf '%s\n' "$pr_json" | jq '[.merged[] | select(.headRefName | test("^(feature|fix|refactor|hotfix)/"))] | length')" -gt 0 ]; then
    group="already_merged"
  elif [ "$(printf '%s\n' "$pr_json" | jq '[.open[] | select((.labels | index("ready-for-human-review")) or .isDraft == false)] | length')" -gt 0 ] \
    || [ "$status" = "Spec in Review" ] \
    || [ "$status" = "Plan in Review" ] \
    || [ "$status" = "Development in Review" ]; then
    group="in_review"
  elif [ "$(printf '%s\n' "$dep_json" | jq -r '.state')" = "blocked" ]; then
    group="blocked"
  fi
