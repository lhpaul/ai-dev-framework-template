...
if [ "$RETRIGGER_COUNT" -lt "$MAX_RETRIGGERS" ]; then
  ...
  _refreshed_sha=$(gh pr view "$PR_NUMBER" --repo "$OWNER/$REPO" --json headRefOid \
    --jq '.headRefOid' 2>/dev/null | cut -c1-12)
  if [ -z "$_refreshed_sha" ]; then
    echo "WARN: could not refresh HEAD SHA — using previous value ($CURRENT_SHA)" >&2
  else
    CURRENT_SHA="$_refreshed_sha"
  fi
  if ! TRIGGER_TIME=$(gh api "repos/$OWNER/$REPO/issues/$PR_NUMBER/comments" \
    --method POST \
