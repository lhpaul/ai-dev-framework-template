...
run_test "regression_workflow_constant_present" "yes" "$(contains 'REGRESSION_WORKFLOW: "e2e-regression.yml"')"
run_test "regression_workflow_dispatches_after_label" "yes" "$(contains 'gh workflow run "$REGRESSION_WORKFLOW"')"
...
run_test "label_lookup_error_preserves_label" "yes" "$(contains "dispatch-failure cleanup will preserve the label")"
run_test "regression_dispatch_failure_removes_new_label" "yes" "$(contains "Removing '\${LABEL_NAME}' because regression dispatch failed after this workflow applied it.")"
