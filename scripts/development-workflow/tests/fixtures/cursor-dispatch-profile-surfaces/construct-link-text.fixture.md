...
See [Evaluation order: initial handoff is evaluated first; onward-handoff capability is evaluated only once initial handoff is confirmed. A profile decision never evaluates onward-handoff capability before initial handoff is confirmed.](https://example.com/evaluation-order) for the rule.
