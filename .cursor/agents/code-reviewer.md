...
When dispatched for **Pass 1 (Spec Compliance)**: evaluate only the `### Pass 1: Spec Compliance` sub-checklist from `REVIEW.md`. Do not evaluate code quality items. A delta confined to test scaffolding is blocking only when you can state a Coverage-Harm Statement (the specific coverage lost and the defect class it lets through) — see `docs/workflow/development-workflow/test-scope-proportionality.md`; a behaviour or acceptance-criterion delta stays blocking under the unchanged rule regardless of test counts.
