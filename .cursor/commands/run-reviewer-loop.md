...
- Use the helper scripts `scripts/development-workflow/pr-review-loop.sh` and `scripts/development-workflow/pr-ci-loop.sh`. `pr-review-loop.sh` may evaluate more than one configured platform in sequence. Do not run Step 7 in the background; run it to completion before Step 8.
